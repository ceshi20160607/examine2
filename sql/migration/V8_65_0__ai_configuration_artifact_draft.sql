-- Confirmation-gated dictionary/selection-field and page-layout draft artifacts.
-- Actionable previews are fixed redactions; executable owner commands remain
-- authenticated ciphertext and never appear as plaintext in AI persistence.
...
CREATE TABLE un_ai_config_artifact_proposal (
    ...
    UNIQUE KEY uk_ai_config_artifact_proposal_turn (
        system_id, tenant_id, turn_id),
    KEY idx_ai_config_artifact_member (
        system_id, tenant_id, member_id, state, expires_at, id),
    KEY idx_ai_config_artifact_module (
        system_id, tenant_id, module_code, artifact_kind, state, created_at, id),
    CONSTRAINT fk_ai_config_artifact_session FOREIGN KEY (
        system_id, tenant_id, session_id)
        REFERENCES un_ai_agent_session (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_ai_config_artifact_turn FOREIGN KEY (
        system_id, tenant_id, turn_id)
        REFERENCES un_ai_agent_turn (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_ai_config_artifact_policy FOREIGN KEY (
        system_id, tenant_id, policy_version_id)
        REFERENCES un_ai_agent_policy_version (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_ai_config_artifact_provider FOREIGN KEY (
        system_id, tenant_id, provider_id)
        REFERENCES un_ai_provider (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT ck_ai_config_artifact_identity CHECK (
    ...
    CONSTRAINT ck_ai_config_artifact_kind CHECK (
        artifact_kind IN ('SELECTION_FIELD','PAGE_LAYOUT')),
    CONSTRAINT ck_ai_config_artifact_module CHECK (
        module_code IS NULL
        OR module_code REGEXP '^[a-z][a-z0-9_]{1,63}$'),
    CONSTRAINT ck_ai_config_artifact_hash CHECK (
        plan_hash REGEXP '^[0-9a-f]{64}$'),
    CONSTRAINT ck_ai_config_artifact_json CHECK (
        (preview_json IS NULL OR JSON_TYPE(preview_json) = 'OBJECT')
        AND (result_json IS NULL OR JSON_TYPE(result_json) = 'OBJECT')),
    CONSTRAINT ck_ai_config_artifact_confidence CHECK (
        confidence BETWEEN 0.00000 AND 1.00000),
    CONSTRAINT ck_ai_config_artifact_payload CHECK (
    ...
    CONSTRAINT ck_ai_config_artifact_state CHECK (
        state IN ('CLARIFICATION_REQUIRED','PENDING','EXECUTING',
                  'SUCCEEDED','FAILED','REJECTED','EXPIRED')),
    CONSTRAINT ck_ai_config_artifact_result CHECK (
        ...
        AND ((state IN ('CLARIFICATION_REQUIRED','PENDING')
          ...
          OR (state = 'EXPIRED' AND acted_by IS NULL
              AND result_json IS NULL AND finished_at IS NOT NULL))),
    CONSTRAINT ck_ai_config_artifact_time CHECK (
...
CREATE TABLE un_ai_config_artifact_attempt (
    ...
    UNIQUE KEY uk_ai_config_artifact_attempt_key (
        system_id, tenant_id, proposal_id, action, request_key),
    CONSTRAINT fk_ai_config_artifact_attempt_root FOREIGN KEY (
        system_id, tenant_id, proposal_id)
        REFERENCES un_ai_config_artifact_proposal (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT ck_ai_config_artifact_attempt_identity CHECK (
    ...
    CONSTRAINT ck_ai_config_artifact_attempt_action CHECK (action = 'CONFIRM'),
    CONSTRAINT ck_ai_config_artifact_attempt_key CHECK (
        request_key REGEXP '^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$'),
    CONSTRAINT ck_ai_config_artifact_attempt_hash CHECK (
        request_hash REGEXP '^[0-9a-f]{64}$'
        AND (result_hash IS NULL OR result_hash REGEXP '^[0-9a-f]{64}$')),
    CONSTRAINT ck_ai_config_artifact_attempt_state CHECK (
        (status = 'EXECUTING' AND finished_at IS NULL
            AND result_hash IS NULL)
        OR (status IN ('SUCCEEDED','FAILED') AND finished_at IS NOT NULL
            AND result_hash IS NOT NULL)),
    CONSTRAINT ck_ai_config_artifact_attempt_result CHECK (
        result_code REGEXP '^[A-Z][A-Z0-9_]{1,63}$')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE un_ai_config_artifact_event (
    ...
    KEY idx_ai_config_artifact_event_root (
        system_id, tenant_id, proposal_id, revision, id),
    CONSTRAINT fk_ai_config_artifact_event_root FOREIGN KEY (
        system_id, tenant_id, proposal_id)
        REFERENCES un_ai_config_artifact_proposal (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_ai_config_artifact_event_attempt FOREIGN KEY (
        system_id, tenant_id, attempt_id)
        REFERENCES un_ai_config_artifact_attempt (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT ck_ai_config_artifact_event_identity CHECK (
        id > 0 AND account_id > 0 AND member_id > 0 AND session_id > 0
        AND turn_id > 0 AND policy_version_id > 0 AND provider_id > 0
        AND provider_version >= 0 AND proposal_id > 0
        AND (attempt_id IS NULL OR attempt_id > 0) AND actor_member_id > 0
        AND revision >= 0),
    CONSTRAINT ck_ai_config_artifact_event_type CHECK (
        event_type IN ('CLARIFICATION_REQUIRED','PROPOSED','CONFIRMING',
                       'SUCCEEDED','FAILED','REJECTED','EXPIRED')),
    CONSTRAINT ck_ai_config_artifact_event_state CHECK (
        ...
        AND (from_state IS NULL OR from_state IN (
             ...
             'SUCCEEDED','FAILED','REJECTED','EXPIRED'))),
    CONSTRAINT ck_ai_config_artifact_event_result CHECK (
        result_code REGEXP '^[A-Z][A-Z0-9_]{1,63}$'
