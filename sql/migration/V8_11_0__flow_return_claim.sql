-- Durable approval cursor plus open/claimed task ownership.

ALTER TABLE un_flow_instance
    ADD COLUMN current_step_index INT UNSIGNED NULL AFTER approver_ids_json,
    ADD COLUMN claim_state VARCHAR(8)
        CHARACTER SET ascii COLLATE ascii_bin NULL AFTER current_step_index;

UPDATE un_flow_instance instance_row
SET instance_row.current_step_index = (
        SELECT COUNT(*)
        FROM un_flow_history_event history_row
        WHERE history_row.system_id = instance_row.system_id
          AND history_row.tenant_id = instance_row.tenant_id
          AND history_row.instance_id = instance_row.instance_id
          AND history_row.event_type = 'APPROVED'
          AND history_row.to_status = 'PENDING'
    ),
    instance_row.claim_state = 'CLAIMED';

ALTER TABLE un_flow_instance
    MODIFY COLUMN current_step_index INT UNSIGNED NOT NULL,
    MODIFY COLUMN claim_state VARCHAR(8)
        CHARACTER SET ascii COLLATE ascii_bin NOT NULL;

ALTER TABLE un_flow_instance
    DROP CHECK ck_flow_instance_approver_snapshot;
...
ALTER TABLE un_flow_instance
    ...
    ADD CONSTRAINT ck_flow_instance_claim_state CHECK (
        claim_state IN ('CLAIMED', 'OPEN')
    ),
    ADD KEY idx_flow_instance_claim_pool (
        system_id, tenant_id, status, claim_state, started_at, instance_id
    );
...
CROSS JOIN (
    SELECT 'flow.instance.return' AS permission_code,
           'Return current flow task to the previous step' AS name
    UNION ALL
    SELECT 'flow.instance.claim', 'Claim an open flow approval task'
    UNION ALL
    SELECT 'flow.instance.cancel-claim', 'Release a flow task to the claim pool'
