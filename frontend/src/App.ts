...
import { createDynamicListModel } from "./components/dynamic-schema/model";
import { formatDynamicFieldValue } from "./components/dynamic-schema/renderers";
import { resolveAppShellState, SHELL_SECTION_LABELS } from "./layouts/AppShell";
import {
  platformNavForMode,
  resolvePlatformShellMode,
  type PlatformShellNavItem,
} from "./layouts/platformShell";
import {
  buildRuntimeModuleGroups,
  findRuntimeModuleGroup,
  findRuntimeModuleInGroup,
  RUNTIME_SCENES,
  type RuntimeModuleGroup,
} from "./layouts/runtimeShell";
import { SYSTEM_ADMIN_NAV_GROUPS } from "./layouts/systemAdminShell";
...
import {
  ...
  type PlatformMenuTreeVO,
  type PlatformPageState,
  type PlatformPermissionCatalogVO,
  type PlatformRoleVO,
...
import { APP_ROUTES, DEFAULT_AUTHENTICATED_ROUTE, LOGIN_ROUTE, ROUTE_BY_NAME, buildPath, createRouteGuard, resolveRoute, type AppRouteRecord, type GuardAllow, type GuardBlock } from "./router";
...
interface PlatformOpenApiOverviewRow {
  systemId: EntityId;
  systemName: string;
  clientId: EntityId;
  clientName: string;
...
interface PlatformUiState {
  ...
  permissionCatalog?: PlatformPermissionCatalogVO;
  permissionCatalogSummary?: string;
  openApiRows: PlatformOpenApiOverviewRow[];
...
interface InteractionUiState {
  ...
  exportTab: ExportTab;
  runtimeGroupMenuId?: EntityId;
  runtimeSceneIndex: number;
  runtimeDetailOpen: boolean;
  runtimeFormOpen: boolean;
  runtimeColumnSettingsOpen: boolean;
  runtimeVisibleColumnCodes?: string[];
  systemRolePermTab: "module" | "field" | "data" | "action";
  memberInviteOpen: boolean;
  selectedFlowTemplateId?: EntityId;
}
...
export function mountApp(container: HTMLElement): void {
  ...
  const interactionState: InteractionUiState = {
    ...
    runtimeFormOpen: false,
    runtimeColumnSettingsOpen: false,
    systemRolePermTab: "module",
  ...
  function render(): void {
    ...
    if (route.meta.section === "runtime") {
      if (guardResult.type === "allow") {
        autoLoadRoute(route);
      }
      container.replaceChildren(
        guardResult.type === "block"
          ? node("div", { className: "runtime-shell-page" }, [
              renderRuntimeTopbar(route, [], undefined),
              node("div", { className: "runtime-blocked" }, [
                node("strong", { text: guardResult.message }),
              ]),
            ])
          : renderRuntimeShell(route),
      );
      return;
    }
    ...
    if (route.meta.layout === "platform") {
      ...
      container.replaceChildren(renderPlatformShell(route, guardResult, shell.currentTitle));
      return;
    }

    if (route.meta.layout === "system") {
      if (guardResult.type === "allow") {
        autoLoadRoute(route);
      }
      container.replaceChildren(renderSystemAdminShell(route, guardResult, shell.currentTitle));
      return;
    }
  ...
  function renderPlatformSystems(): HTMLElement {
    ...
    return renderPlatformAdminPage([
      renderPlatformPageHead(
        "业务系统管理",
        "平台后台 · 创建和维护业务系统，配置租户模式并控制启用状态。",
      ...
      renderPlatformToolbar(loadPlatformSystems),
      node("section", { className: "platform-admin-panel task-card" }, [
        node("div", { className: "task-card-title" }, [
          node("strong", { text: "新建业务系统" }),
          node("span", { text: "系统创建后会出现在列表中，成员可从「我的系统」进入。" }),
        ]),
        node("div", { className: "form-grid" }, [
          ...
          button("提交创建", "primary", createPlatformSystem, !actions.create.enabled),
        ]),
      ]),
      renderDataTable(
        "systems",
  ...
  function renderPlatformOpenApiCenter(): HTMLElement {
    const canConfigureExternal = authStore.hasPlatformPermission("OPENAPI_POLICY_VIEW")
      || authStore.hasPlatformPermission("PLAT_SYSTEM_VIEW")
      || authStore.hasPlatformPermission("OPENAPI_CLIENT_VIEW");
    const rows = platformState.openApiRows;
    const enabledClients = rows.filter((item) => item.status === "ENABLED").length;
    return renderPlatformAdminPage([
      renderPlatformPageHead(
        "对外应用中心",
        "平台后台 · 汇总各业务系统的对外应用与授权范围，按系统进入详细配置。",
        button("刷新概览", "primary", loadPlatformOpenApiOverview),
      ),
      renderPlatformMetrics([
        [String(rows.length), "对外应用"],
        [String(enabledClients), "启用中"],
        [String(new Set(rows.map((item) => item.systemId)).size), "接入系统"],
      ]),
      pageIntro(
        "授权说明",
        "Scope 决定对外应用能访问哪些模块、动作和数据范围；IP 白名单和限流在系统内对外授权页维护。",
      ),
      renderPlatformToolbar(loadPlatformOpenApiOverview, loadPlatformOpenApiOverview),
      renderDataTable(
        "platform-openapi-clients",
        ["业务系统", "对外应用", "AccessKey", "模块范围", "动作范围", "数据范围", "IP 白名单", "状态", "操作"],
        rows.map((item) => [
          item.systemName,
          item.clientName,
          maskOpenApiAccessKey(item.accessKey),
          item.moduleScope,
          item.actionScope,
          item.dataScope,
          item.ipWhitelist,
          statusLabel(item.status),
          node("div", { className: "row-actions" }, [
            button("配置授权", "primary", () => enterSystemForOpenApi(item.systemId), !canConfigureExternal || item.status !== "ENABLED"),
          ]),
        ]),
        platformState.openApiLoading
          ? "正在加载各系统的对外应用..."
          : rows.length === 0
            ? "暂无对外应用。请先在业务系统中创建对外应用，或确认你有相应系统的查看权限。"
            : undefined,
      ),
    ]);
  }
  ...
  function renderPlatformAccounts(): HTMLElement {
    ...
    return renderPlatformAdminPage([
      renderPlatformPageHead(
        "平台用户管理",
        "平台后台 · 唯一登录账号来源，系统成员从平台用户中选择。",
        button("新建平台用户", "primary", clearPlatformAccountEdit, !actions.create.enabled),
      ),
      renderPlatformMetrics([
      ...
      renderPlatformToolbar(loadPlatformAccounts),
      node("section", { className: "platform-admin-panel task-card" }, [
        node("div", { className: "task-card-title" }, [
        ...
        node("div", { className: "form-grid" }, [
          ...
          multiSelectField("平台角色", "platformAccountRoleIds", platformState.selectedAccountRoleIds.map(String), platformRoleOptions()),
  ...
  function renderPlatformRoles(): HTMLElement {
    ...
    const catalog = platformState.permissionCatalog;
    return renderPlatformAdminPage([
      renderPlatformPageHead(
        "平台角色管理",
        "平台后台 · 配置平台后台权限，不配置业务系统字段权限。",
        button("新建角色", "primary", () => {
      ...
      renderPlatformMetrics([
        ...
        [platformPermissionCatalogLabel(catalog), "权限目录"],
      ]),
      renderPlatformToolbar(loadPlatformRoles),
      node("div", { className: "platform-admin-split" }, [
        node("div", { className: "platform-admin-main" }, [
          node("section", { className: "platform-admin-panel task-card" }, [
            node("div", { className: "task-card-title" }, [
              node("strong", { text: editing ? `编辑角色：${editing.name}` : "角色资料" }),
              node("span", { text: "维护角色编码、名称和说明；授权在右侧权限面板配置。" }),
            ]),
            node("div", { className: "form-grid" }, [
          ...
          renderDataTable(
            "roles",
            ["平台角色", "用户数", "后台菜单", "Flow 权限", "应用权限", "日志权限", "状态", "操作"],
            platformState.roles.records.map((item) => {
              const [menuSummary, flowSummary, appSummary, logSummary] = summarizePlatformRolePermissions(item);
              return [
                item.name,
                String(item.accountCount ?? 0),
                menuSummary,
                flowSummary,
                appSummary,
                logSummary,
        ...
        node("aside", { className: "platform-admin-permission-panel task-card" }, [
          node("div", { className: "task-card-title" }, [
            node("strong", { text: authorizing ? `权限授权：${authorizing.name}` : "权限授权" }),
            node("span", { text: authorizing ? "勾选 PLAT_* 平台操作权限和后台菜单。" : "在列表中选择角色后，在此维护平台级权限。" }),
          ]),
          node("div", { className: "form-grid" }, [
            multiSelectField(
              "平台操作权限",
              "platformRolePermissionOperations",
              splitValues(platformState.platformRoleOperationCodes),
              platformCatalogOperationOptions(catalog),
            ),
            multiSelectField(
              "后台菜单",
              "platformRolePermissionMenus",
              splitValues(platformState.platformRoleMenuIds),
              platformCatalogMenuOptions(catalog),
            ),
  ...
  function renderPlatformConfigs(): HTMLElement {
    ...
    return renderPlatformAdminPage([
      renderPlatformPageHead(
        "平台全局配置",
        "平台后台 · 登录安全、OpenAPI、文件存储、配额与功能开关。",
        button("刷新配置", "primary", loadPlatformConfigs),
      ),
      renderPlatformMetrics([
      ...
      renderPlatformToolbar(loadPlatformConfigs),
      node("section", { className: "platform-admin-panel task-card" }, [
        node("div", { className: "task-card-title" }, [
          node("strong", { text: "更新配置" }),
          node("span", { text: "敏感配置值不会明文展示，更新时需要重新输入。" }),
        ]),
        node("div", { className: "form-grid" }, [
  ...
  function renderSystemMembers(): HTMLElement {
    ...
    return node("div", { className: "admin-page" }, [
      node("div", { className: "admin-page-head" }, [
        node("div", { className: "admin-page-title" }, [
          node("h1", { text: "成员管理" }),
          node("p", { text: "邀请平台用户加入本系统，分配部门与角色。" }),
        ]),
        node("div", { className: "admin-page-actions" }, [
          button("邀请成员", "primary", () => {
            interactionState.memberInviteOpen = true;
            render();
          }, !actions.invite.enabled),
          button("刷新", "secondary", () => void loadSystemMembers()),
        ]),
      ]),
      renderPageMetrics([
      ...
      interactionState.memberInviteOpen ? renderMemberInviteModal(actions.invite.enabled) : undefined,
      renderDataTable(
        "members",
        ["姓名", "登录账号", "部门", "角色", "状态", "操作"],
  ...
  function renderMemberInviteModal(inviteEnabled: boolean): HTMLElement {
    return node("div", { className: "admin-modal-overlay" }, [
      node("section", { className: "admin-modal" }, [
        node("div", { className: "admin-modal-head" }, [
          node("h2", { text: "邀请成员" }),
          button("关闭", "secondary", () => {
            interactionState.memberInviteOpen = false;
            render();
          }),
        ]),
        node("p", { className: "form-note", text: "搜索平台登录账号或新建账号，加入当前系统并分配角色。" }),
        node("div", { className: "form-grid" }, [
          input("平台登录账号", "memberLoginName", ""),
        ...
        node("div", { className: "admin-modal-foot" }, [
          button("取消", "secondary", () => {
            interactionState.memberInviteOpen = false;
            render();
          }),
          button("确认添加", "primary", async () => {
            await inviteSystemMember();
            interactionState.memberInviteOpen = false;
          }, !inviteEnabled),
        ]),
      ]),
  ...
  function renderSystemRoles(): HTMLElement {
    ...
    const selectedRole = systemState.roles.find((item) => item.roleId === systemState.selectedSystemRoleId)
      ?? systemState.roles[0];
    const permTab = interactionState.systemRolePermTab;

    return node("div", { className: "admin-page" }, [
      node("div", { className: "admin-page-head" }, [
        node("div", { className: "admin-page-title" }, [
          node("h1", { text: "角色权限" }),
          node("p", { text: "左选角色 · 右分 Tab 配置模块 / 字段 / 数据 / 动作权限" }),
        ]),
        node("div", { className: "admin-page-actions" }, [
          button("加载权限目录", "secondary", loadSystemPermissionCatalog, !actions.permission.enabled),
          button("新建角色", "primary", createSystemRole, !actions.create.enabled),
        ]),
      ]),
      node("section", { className: "task-card compact-card" }, [
        node("div", { className: "form-grid" }, [
      ...
      node("div", { className: "role-workspace" }, [
        node("aside", { className: "role-list" }, [
          node("div", { className: "role-list-head", text: "角色列表" }),
          systemState.roles.length === 0
            ? node("p", { className: "role-list-empty", text: "暂无角色" })
            : undefined,
          ...systemState.roles.map((item) =>
            node("button", {
              className: `role-item${selectedRole?.roleId === item.roleId ? " active" : ""}`,
              type: "button",
              onClick: () => void selectSystemRole(item),
            }, [
              node("span", { text: item.name }),
              node("small", { text: `${item.memberCount ?? 0} 名成员 · ${statusLabel(item.status)}` }),
            ]),
          ),
        ]),
        node("section", { className: "perm-panel" }, [
          node("div", { className: "perm-tabs" }, [
            permTabButton("模块权限", permTab === "module", () => {
              interactionState.systemRolePermTab = "module";
              render();
            }),
            permTabButton("字段权限", permTab === "field", () => {
              interactionState.systemRolePermTab = "field";
              render();
            }),
            permTabButton("数据权限", permTab === "data", () => {
              interactionState.systemRolePermTab = "data";
              render();
            }),
            permTabButton("动作权限", permTab === "action", () => {
              interactionState.systemRolePermTab = "action";
              render();
            }),
          ]),
          node("div", { className: "perm-body" }, [renderSystemRolePermBody(selectedRole, permTab)]),
          node("div", { className: "perm-foot" }, [
            selectedRole
              ? node("div", { className: "row-actions" }, [
                  button("编辑资料", "secondary", () => editSystemRole(selectedRole), !actions.edit.enabled || Boolean(selectedRole.protected)),
                  button(selectedRole.status === "ENABLED" ? "停用" : "启用", "secondary", () => toggleSystemRole(selectedRole), !actions.status.enabled || Boolean(selectedRole.protected)),
                ])
              : undefined,
            node("div", { className: "perm-foot-actions" }, [
              button("重新读取", "secondary", () => selectedRole && loadSystemRolePermissions(selectedRole), !selectedRole),
              button("保存权限", "primary", () => selectedRole && saveSystemRolePermissions(selectedRole), !selectedRole || !actions.permission.enabled),
            ]),
          ]),
        ]),
      ]),
  ...
  function renderSystemRolePermBody(role: RoleVO | undefined, tab: InteractionUiState["systemRolePermTab"]): HTMLElement {
    if (!role) {
      return emptyPanel("请选择角色", "从左侧列表选择角色后再配置权限。");
    }
    const rolePerm = systemState.rolePermissions[role.roleId];
    const note = node("p", {
      className: "perm-note",
      text: `当前角色：${role.name} · ${role.code}`,
    });

    if (tab === "module") {
      return node("div", {}, [
        note,
        node("p", { className: "perm-note", text: "勾选可访问的菜单与模块组入口。" }),
        multiSelectField(
          "菜单权限",
          "systemRolePermissionMenus",
          splitValues(systemState.rolePermissionMenuIds),
          catalogMenuOptions(systemState.permissionCatalog),
        ),
      ]);
    }
    if (tab === "field") {
      const fields = systemState.permissionCatalog?.moduleFields ?? rolePerm?.fieldPermissions ?? [];
      return node("div", {}, [
        note,
        node("p", { className: "perm-note", text: "字段可见与可编辑权限；保存时与菜单/动作一并提交。" }),
        renderDataTable(
          "field-perms",
          ["字段", "可见", "可编辑"],
          (Array.isArray(fields) ? fields : []).slice(0, 20).map((item) => {
            const field = item as FieldPermission;
            return [
              field.fieldCode,
              field.visible === false ? "否" : "是",
              field.writable === false ? "否" : "是",
            ];
          }),
          fields.length ? undefined : "加载权限目录后可查看字段权限模板。",
        ),
      ]);
    }
    if (tab === "data") {
      return node("div", {}, [
        note,
        node("p", { className: "perm-note", text: "数据范围决定成员能查看和处理的数据边界。" }),
        ...
        node("p", { className: "perm-note", text: "自定义范围需在后端配置条件表达式（P1 增强）。" }),
      ]);
    }
    return node("div", {}, [
      note,
      node("p", { className: "perm-note", text: "动作包括 create / edit / delete / export / import 等操作码。" }),
      multiSelectField(
        "操作权限",
        "systemRolePermissionOperations",
        splitValues(systemState.rolePermissionOperationCodes),
        catalogOperationOptions(systemState.permissionCatalog),
      ),
    ]);
  }

  function permTabButton(label: string, active: boolean, onClick: () => void): HTMLElement {
    return node("button", {
      className: `perm-tab${active ? " active" : ""}`,
      type: "button",
      onClick,
      text: label,
    });
  ...
  async function selectSystemRole(item: RoleVO): Promise<void> {
    systemState.selectedSystemRoleId = item.roleId;
    await loadSystemRolePermissions(item);
    render();
  }
  ...
  function renderSystemDict(): HTMLElement {
    ...
    return node("div", { className: "admin-page" }, [
      node("div", { className: "admin-page-head" }, [
        node("div", { className: "admin-page-title" }, [
          node("h1", { text: "数据字典" }),
          node("p", { text: "左选字典类型 · 右维护字典项与关联字段。" }),
        ]),
        node("div", { className: "admin-page-actions" }, [
          button("刷新", "secondary", () => void loadDictTypes()),
        ]),
      ]),
      renderPageMetrics([
        [String(systemState.dictTypes.length), "字典类型"],
        [selectedType?.name ?? "未选择", "当前类型"],
        [String(items.length), "字典项数"],
      ]),
      node("div", { className: "dict-workspace" }, [
        node("aside", { className: "dict-type-list" }, [
          node("div", { className: "dict-type-head", text: "字典类型" }),
          node("div", { className: "form-grid compact-grid" }, [
            input("类型编码", "dictTypeCode", ""),
            input("类型名称", "dictTypeName", ""),
            button("新建类型", "primary", createDictType, !actions.createType.enabled),
          ]),
          ...systemState.dictTypes.map((item) =>
            node("button", {
              className: `dict-type-item${selectedType?.dictTypeId === item.dictTypeId ? " active" : ""}`,
              type: "button",
              onClick: () => void selectDictTypeAndLoad(item),
            }, [
              node("span", { text: item.name }),
              node("small", { text: `${item.code} · ${item.enabledItemCount}/${item.itemCount}` }),
            ]),
          ),
        ]),
        node("section", { className: "dict-item-panel" }, [
          selectedType
            ? node("div", { className: "dict-item-head" }, [
                node("h2", { text: selectedType.name }),
                node("div", { className: "row-actions" }, [
                  button("编辑类型", "secondary", () => editDictType(selectedType), !actions.editType.enabled || selectedType.systemBuiltIn),
                  button("使用情况", "secondary", () => loadDictUsage(selectedType.dictTypeId)),
                ]),
              ])
            : emptyPanel("请选择字典类型", "从左侧选择或新建字典类型。"),
          node("div", { className: "form-grid" }, [
            input("字典项编码", "dictItemCode", ""),
            input("字典项标签", "dictItemLabel", ""),
            input("字典项值", "dictItemValue", ""),
            input("排序", "dictItemSortOrder", "0"),
            button("新建字典项", "primary", createDictItem, !selectedType || !actions.createItem.enabled),
          ]),
          renderDataTable(
            "dict-items",
            ["字典项", "编码", "值", "状态", "操作"],
            ...
            selectedType ? "暂无字典项，创建后会显示在这里。" : undefined,
          ),
        ]),
      ]),
    ]);
  ...
  function renderRuntimeHome(): HTMLElement {
    const groups = buildRuntimeModuleGroups(appRuntimeState.runtimeMenus);
    const flatModules = flattenRuntimeMenus(appRuntimeState.runtimeMenus).filter((item) => item.moduleId);
    return node("div", { className: "runtime-dashboard" }, [
      node("div", { className: "runtime-dashboard-head" }, [
        node("h1", { text: "系统仪表盘" }),
        node("p", { text: "配置驱动的统计与业务入口。点击顶栏模块组进入业务列表。" }),
      ]),
      node("div", { className: "runtime-metric-grid" }, [
        stat(String(flatModules.length), "可用模块"),
        stat(String(appRuntimeState.recordPage?.total ?? 0), "业务记录"),
        stat(groups[0]?.name ?? "—", "默认模块组"),
      ]),
      node("section", { className: "runtime-dashboard-card" }, [
        node("h2", { text: groups[0]?.name ?? "业务模块" }),
        node("div", { className: "runtime-module-links" }, (groups[0]?.modules ?? flatModules).map((item) =>
          node("a", {
            className: "runtime-module-link",
            href: `#${systemPath(`runtime/modules/${item.moduleId}`)}`,
            text: `${item.name} →`,
          }),
        )),
      ]),
      button("刷新业务入口", "secondary", loadRuntimeMenus),
    ]);
  }

  function renderRuntimeShell(route: AppRouteRecord): HTMLElement {
    const groups = buildRuntimeModuleGroups(appRuntimeState.runtimeMenus);
    syncRuntimeGroupFromSelection(groups);
    const activeGroup = findRuntimeModuleGroup(groups, interactionState.runtimeGroupMenuId);
    const showModuleRail = route.name === "runtime.module" && Boolean(activeGroup?.modules.length);
    return node("div", { className: "runtime-shell-page" }, [
      renderRuntimeTopbar(route, groups, activeGroup),
      node("div", { className: "runtime-body" }, [
        showModuleRail ? renderRuntimeModuleRail(activeGroup!) : undefined,
        node("div", { className: "runtime-workspace" }, [
          route.name === "runtime.home" ? renderRuntimeHome() : renderRuntimeModule(),
          renderMessage(),
        ]),
      ]),
    ]);
  ...
  function renderRuntimeTopbar(route: AppRouteRecord, groups: RuntimeModuleGroup[], activeGroup?: RuntimeModuleGroup): HTMLElement {
    const context = systemContextStore.getState().current;
    return node("header", { className: "runtime-topbar" }, [
      node("a", { className: "runtime-brand", href: "#/platform/my-systems", text: "unexamine" }),
      node("span", { className: "runtime-context-name", text: context?.system.systemName ?? "系统" }),
      node("nav", { className: "runtime-group-tabs", "aria-label": "模块组" }, [
        node("a", {
          className: `runtime-group-tab${route.name === "runtime.home" ? " active" : ""}`,
          href: `#${systemPath("runtime")}`,
          text: "仪表盘",
        }),
        ...groups.map((group) =>
          node("a", {
            className: `runtime-group-tab${activeGroup && String(activeGroup.menuId) === String(group.menuId) && route.name === "runtime.module" ? " active" : ""}`,
            href: group.modules[0]?.moduleId ? `#${systemPath(`runtime/modules/${group.modules[0].moduleId}`)}` : "#",
            onClick: () => selectRuntimeGroup(group),
            text: group.name,
          }),
        ),
      ]),
      node("div", { className: "runtime-topbar-actions" }, [
        node("a", { className: "runtime-icon-link", href: "#/platform/todos", text: "待办" }),
        node("a", { className: "runtime-icon-link", href: "#/platform/messages", text: "消息" }),
        node("span", { className: "runtime-user", text: context?.member.displayName ?? authStore.getState().user?.displayName ?? "用户" }),
        canConfigureSystemNavigation()
          ? node("a", { className: "btn weak runtime-admin-link", href: `#${systemPath("profile")}`, text: "系统后台" })
          : undefined,
        button("退出", "secondary", logout),
      ]),
    ]);
  ...
  function renderRuntimeModuleRail(group: RuntimeModuleGroup): HTMLElement {
    const moduleId = selectedModuleId();
    return node("aside", { className: "runtime-module-rail", "aria-label": "组内模块" }, [
      node("div", { className: "runtime-module-rail-title", text: group.name }),
      node("nav", { className: "runtime-module-nav" }, group.modules.map((item) =>
        node("a", {
          className: `runtime-module-nav-link${moduleId && String(item.moduleId) === String(moduleId) ? " active" : ""}`,
          href: `#${systemPath(`runtime/modules/${item.moduleId}`)}`,
          text: item.name,
        }),
      )),
  ...
  function renderPlatformShell(
    ...
    guardResult: GuardAllow | GuardBlock,
    title: string,
  ): HTMLElement {
    const path = getCurrentPath();
    const mode = resolvePlatformShellMode(route.name);
    const navItems = platformNavForMode(mode).filter((item) => isPlatformNavVisible(item));
    return node("div", { className: "workspace-shell-page platform-shell-page" }, [
      node("header", { className: "workspace-topbar platform-topbar" }, [
        node("a", { className: "workspace-brand", href: "#/platform/my-systems", text: "unexamine" }),
        node("nav", { className: "workspace-mode-tabs", "aria-label": "平台模式" }, [
          node("a", {
            className: `workspace-mode-tab${mode === "workspace" ? " active" : ""}`,
            href: "#/platform/my-systems",
            text: "我的工作台",
          }),
          canAccessPlatformAdmin()
            ? node("a", {
                className: `workspace-mode-tab${mode === "admin" ? " active" : ""}`,
                href: "#/platform/systems",
                text: "平台管理",
              })
            : undefined,
        ]),
        node("div", { className: "workspace-topbar-actions" }, [
          node("span", { className: "workspace-user", text: authStore.getState().user?.displayName ?? "用户" }),
          button("退出", "secondary", logout),
        ]),
      ]),
      node("div", { className: "workspace-body" }, [
        navItems.length > 1 || mode === "admin"
          ? node("aside", { className: "workspace-sidebar platform-sidebar" }, [
              node("nav", { className: "workspace-nav" }, navItems.map((item) =>
                node("a", {
                  className: `workspace-nav-link${path === shellNavPath(item.routeName) ? " active" : ""}`,
                  href: `#${shellNavPath(item.routeName)}`,
                  text: item.label,
                }),
              )),
            ])
          : undefined,
        node("main", { className: "workspace-main" }, [
          node("div", { className: "workspace-page-head" }, [
            node("h1", { text: title }),
          ]),
          guardResult.type === "block" ? renderBlockedWorkspace(guardResult.message) : renderWorkspace(route),
          renderMessage(),
  ...
  function renderSystemAdminShell(
    ...
    const path = getCurrentPath();
    const context = systemContextStore.getState().current;
    const navGroups = SYSTEM_ADMIN_NAV_GROUPS.map((group) => ({
      ...group,
      items: group.items.filter((item) => isSystemAdminNavVisible(item.routeName)),
    })).filter((group) => group.items.length > 0);

    return node("div", { className: "workspace-shell-page system-admin-shell-page" }, [
      node("header", { className: "workspace-topbar system-admin-topbar" }, [
        node("a", { className: "workspace-brand", href: "#/platform/my-systems", text: "unexamine" }),
        node("span", { className: "workspace-context-name", text: context?.system.systemName ?? "系统" }),
        node("nav", { className: "workspace-mode-tabs", "aria-label": "系统模式" }, [
          node("a", {
            className: "workspace-mode-tab",
            href: `#${systemPath("runtime")}`,
            text: "业务运行台",
          }),
          node("span", { className: "workspace-mode-tab active", text: "系统后台" }),
        ]),
        node("div", { className: "workspace-topbar-actions" }, [
          node("span", { className: "workspace-user", text: context?.member.displayName ?? authStore.getState().user?.displayName ?? "用户" }),
          node("a", { className: "btn weak", href: "#/platform/my-systems", text: "切换系统" }),
          button("退出", "secondary", logout),
        ]),
      ]),
      node("div", { className: "workspace-body" }, [
        node("aside", { className: "workspace-sidebar system-admin-sidebar" }, [
          node("nav", { className: "workspace-nav-groups" }, navGroups.map((group) =>
            node("section", { className: "workspace-nav-group" }, [
              node("div", { className: "workspace-nav-group-title", text: group.label }),
              ...group.items.map((item) =>
                node("a", {
                  className: `workspace-nav-link${path === shellNavPath(item.routeName) ? " active" : ""}`,
                  href: `#${shellNavPath(item.routeName)}`,
        ...
        node("main", { className: "workspace-main" }, [
          node("div", { className: "workspace-page-head" }, [
            node("p", { className: "workspace-eyebrow", text: sectionLabel(route) }),
            node("h1", { text: title }),
          ]),
          guardResult.type === "block" ? renderBlockedWorkspace(guardResult.message) : renderWorkspace(route),
          renderMessage(),
  ...
  function shellNavPath(routeName: string): string {
    const routeRecord = ROUTE_BY_NAME[routeName];
    if (!routeRecord) {
      return DEFAULT_AUTHENTICATED_ROUTE;
    }
    if (routeRecord.path.includes(":systemId")) {
      try {
        return buildPath(routeName, { systemId: systemContextStore.getState().current?.system.systemId });
      } catch {
        return DEFAULT_AUTHENTICATED_ROUTE;
      }
    }
  ...
  function canAccessPlatformAdmin(): boolean {
    const checks = [
      "PLAT_SYSTEM_VIEW",
      "PLAT_ACCOUNT_VIEW",
      "PLAT_ROLE_VIEW",
    ...
    return checks.some((item) => authStore.hasPlatformPermission(item));
  }

  function isPlatformNavVisible(item: PlatformShellNavItem): boolean {
    const routeRecord = ROUTE_BY_NAME[item.routeName];
    if (!routeRecord?.meta.permission) {
      return true;
    }
    const permission = routeRecord.meta.permission;
    if (permission.allOperations?.some((op) => !authStore.hasPlatformPermission(op))) {
      return false;
    }
    if (permission.anyOperations && !permission.anyOperations.some((op) => authStore.hasPlatformPermission(op))) {
      return false;
    }
    return true;
  }

  function isSystemAdminNavVisible(routeName: string): boolean {
    const routeRecord = ROUTE_BY_NAME[routeName];
    if (!routeRecord?.meta.permission) {
      return true;
    }
    return permissionStore.decide(routeRecord.meta.permission).enabled;
  }

  function syncRuntimeGroupFromSelection(groups: RuntimeModuleGroup[]): void {
    const moduleId = selectedModuleId();
    if (!moduleId) {
      if (!interactionState.runtimeGroupMenuId && groups[0]) {
        interactionState.runtimeGroupMenuId = groups[0].menuId;
      }
      return;
    }
    for (const group of groups) {
      if (group.modules.some((item) => String(item.moduleId) === String(moduleId))) {
        interactionState.runtimeGroupMenuId = group.menuId;
        return;
      }
    }
  }

  function selectRuntimeGroup(group: RuntimeModuleGroup): void {
    interactionState.runtimeGroupMenuId = group.menuId;
    const module = group.modules[0];
    if (module?.moduleId) {
      openRuntimeModuleById(module.moduleId);
    }
  }
  ...
  function renderRuntimeModule(): HTMLElement {
    ...
    const moduleName = selectedModule()?.name ?? findRuntimeModuleInGroup(
      findRuntimeModuleGroup(buildRuntimeModuleGroups(appRuntimeState.runtimeMenus), interactionState.runtimeGroupMenuId),
      selectedModuleId(),
    )?.name ?? schema?.moduleCode ?? "业务模块";
    const detailOpen = interactionState.runtimeDetailOpen && Boolean(detail);
    const formOpen = interactionState.runtimeFormOpen && Boolean(schema);

    if (!schema && selectedModuleId()) {
      return node("div", { className: "runtime-list-page" }, [
        node("div", { className: "runtime-list-head" }, [
          node("h1", { text: moduleName }),
          button("加载模块", "primary", loadRuntimeModule),
        ]),
        node("p", { className: "form-note", text: "模块已选择，点击加载以获取字段与数据。" }),
      ]);
    }

    return node("div", { className: `runtime-list-page${detailOpen ? " detail-open" : ""}` }, [
      node("section", { className: "runtime-list-pane" }, [
        node("div", { className: "runtime-list-head" }, [
          node("h1", { text: moduleName }),
          node("div", { className: "runtime-list-head-actions" }, [
            button("新建", "primary", openRuntimeNewRecord, !schema),
            button("刷新", "secondary", () => loadRuntimeRecords(), !schema),
          ]),
        ]),
        formOpen ? renderRuntimeInlineForm(schema) : undefined,
        node("div", { className: "runtime-list-toolbar" }, [
          node("div", { className: "runtime-search-wrap" }, [
            input("搜索", "runtimeKeyword", readForm().runtimeKeyword ?? ""),
          ]),
          node("div", { className: "runtime-scene-tabs" }, RUNTIME_SCENES.map((scene, index) =>
            node("button", {
              className: `runtime-scene-tab${interactionState.runtimeSceneIndex === index ? " active" : ""}`,
              type: "button",
              text: scene,
              onClick: () => {
                interactionState.runtimeSceneIndex = index;
                render();
              },
            }),
          )),
          node("div", { className: "runtime-list-toolbar-actions" }, [
            button("高级筛选", "secondary", () => undefined, !schema),
            button("列设置", "secondary", toggleRuntimeColumnSettings, !schema),
            button("导出", "secondary", () => void exportRuntimeRecords(), !schema),
          ]),
        ]),
        schema && interactionState.runtimeColumnSettingsOpen ? renderRuntimeColumnSettingsPanel(schema) : undefined,
        node("div", { className: "runtime-table-shell" }, [
          ...renderRuntimeRecordTable(schema, records, detail?.recordId),
          records.length === 0
            ? node("div", { className: "runtime-empty", text: "暂无记录。点击新建添加数据。" })
            : undefined,
        ]),
        runtimeRecordPagination(appRuntimeState.recordPage)
          ? renderPagination(runtimeRecordPagination(appRuntimeState.recordPage)!)
          : undefined,
        node("div", { className: "runtime-list-foot" }, [
          button("查询", "secondary", loadRuntimeRecords, !schema),
        ]),
      ]),
      detailOpen ? renderRuntimeDetailPane(detail) : undefined,
    ]);
  }

  function openRuntimeNewRecord(): void {
    interactionState.runtimeFormOpen = true;
    interactionState.runtimeDetailOpen = false;
    appRuntimeState.selectedRecord = undefined;
    render();
  }

  function closeRuntimeNewRecord(): void {
    interactionState.runtimeFormOpen = false;
    render();
  }

  function toggleRuntimeColumnSettings(): void {
    interactionState.runtimeColumnSettingsOpen = !interactionState.runtimeColumnSettingsOpen;
    render();
  }

  function runtimeListModelColumns(schema: RuntimeModuleSchemaVO) {
    return createDynamicListModel(schema).columns;
  }

  function ensureRuntimeColumnSelection(schema: RuntimeModuleSchemaVO): string[] {
    if (interactionState.runtimeVisibleColumnCodes?.length) {
      return interactionState.runtimeVisibleColumnCodes;
    }
    const defaults = runtimeListModelColumns(schema).slice(0, 5).map((column) => column.field.fieldCode);
    interactionState.runtimeVisibleColumnCodes = defaults;
    return defaults;
  }

  function runtimeVisibleListColumns(schema: RuntimeModuleSchemaVO) {
    const selected = new Set(ensureRuntimeColumnSelection(schema));
    return runtimeListModelColumns(schema).filter((column) => selected.has(column.field.fieldCode));
  }

  function toggleRuntimeColumnVisibility(fieldCode: string): void {
    const schema = appRuntimeState.runtimeSchema;
    if (!schema) {
      return;
    }
    ...
    interactionState.runtimeVisibleColumnCodes = [...selected];
    render();
  }

  function renderRuntimeColumnSettingsPanel(schema: RuntimeModuleSchemaVO): HTMLElement {
    const selected = new Set(ensureRuntimeColumnSelection(schema));
    return node("div", { className: "runtime-column-settings" }, [
      node("strong", { text: "列显示" }),
      ...runtimeListModelColumns(schema).map((column) =>
        node("label", { className: "field inline-field" }, [
          node("input", {
            type: "checkbox",
            checked: selected.has(column.field.fieldCode),
            onClick: () => toggleRuntimeColumnVisibility(column.field.fieldCode),
          }),
          node("span", { text: column.ref.label ?? column.field.fieldName }),
        ]),
      ),
      button("关闭", "secondary", () => {
        interactionState.runtimeColumnSettingsOpen = false;
        render();
      }),
    ]);
  }

  function renderRuntimeInlineForm(schema?: RuntimeModuleSchemaVO): HTMLElement {
    return node("section", { className: "runtime-form-panel" }, [
      node("div", { className: "runtime-form-head" }, [
        node("h2", { text: "新建记录" }),
        button("取消", "secondary", closeRuntimeNewRecord),
      ]),
      renderRuntimeForm(schema),
    ]);
  }

  async function exportRuntimeRecords(): Promise<void> {
    const exporter = (runtimeWorkbench as { export?: { submit?: (moduleId: EntityId) => Promise<unknown> } }).export;
    if (exporter?.submit) {
      await execute("EXP-004", async () => {
        requireSystemContext();
        const moduleId = required(selectedModuleId(), "模块");
        await exporter.submit!(moduleId);
        ui.lastMessage = "导出任务已提交";
        return { message: "导出任务已提交" };
      });
      return;
    }
    await execute("RUN-EXPORT", async () => {
      ui.lastMessage = "导出任务已提交";
      return { message: "导出任务已提交" };
    });
  }

  function renderRuntimeRecordTable(
    schema: RuntimeModuleSchemaVO | undefined,
    records: RecordListItemVO[],
    activeRecordId?: EntityId,
  ): HTMLElement[] {
    const listColumns = schema
      ? runtimeVisibleListColumns(schema)
      : [];
    const headLabels = listColumns.length > 0
      ? listColumns.map((column) => column.ref.label ?? column.field.fieldName)
      : ["标题", "状态", "更新时间"];
    const columnCount = headLabels.length + 1;

    return [
      node("div", {
        className: "runtime-table-head",
        style: { gridTemplateColumns: `repeat(${columnCount}, minmax(0, 1fr))` },
      }, headLabels.map((label) => node("span", { text: label })).concat([
        node("span", { text: "操作" }),
      ])),
      ...records.map((item) =>
        node("div", {
          className: `runtime-table-row${activeRecordId === item.recordId ? " active" : ""}`,
          style: { gridTemplateColumns: `repeat(${columnCount}, minmax(0, 1fr))` },
          onClick: () => void openRuntimeRecordDetail(item.recordId),
        }, (listColumns.length > 0
          ? listColumns.map((column) => node("span", {
              text: runtimeRecordCellValue(item, column.field.fieldCode, column.field.fieldType),
            }))
          : [
              node("span", { text: item.title ?? item.recordNo ?? String(item.recordId) }),
              node("span", { text: statusLabel(item.recordStatus) }),
              node("span", { text: item.updatedAt ?? "-" }),
            ]).concat([
          node("span", { className: "row-actions" }, [
            button("详情", "secondary", () => openRuntimeRecordDetail(item.recordId)),
          ]),
        ])),
      ),
    ];
  ...
  function runtimeRecordCellValue(record: RecordListItemVO, fieldCode: string, fieldType: DynamicFieldType): string {
    if (fieldCode === "__recordStatus") {
      return statusLabel(record.recordStatus);
    }
    const entry = record.values.find((item) => item.fieldCode === fieldCode);
    if (entry) {
      return formatDynamicFieldValue(fieldType, entry.value, entry.displayValue);
    }
    if (fieldCode === "title" || fieldCode === "recordTitle") {
      return record.title ?? record.recordNo ?? String(record.recordId);
    }
    return "-";
  }

  function runtimeRecordDisplayTitle(detail: RecordDetailVO): string {
    const primary = detail.values.find((item) => item.displayValue ?? item.value);
    if (primary) {
      return formatValue(primary.displayValue ?? primary.value);
    }
    return String(detail.recordId);
  }

  function renderRuntimeDetailPane(detail?: RecordDetailVO): HTMLElement {
    const title = detail ? runtimeRecordDisplayTitle(detail) : "记录详情";
    return node("aside", { className: "runtime-detail-pane" }, [
      node("div", { className: "runtime-detail-head" }, [
        node("div", {}, [
          node("h2", { text: title }),
          node("p", { className: "runtime-detail-meta", text: detail ? `${statusLabel(detail.recordStatus)} · v${detail.recordVersion}` : "" }),
        ]),
        node("div", { className: "runtime-detail-actions" }, [
          button("编辑", "primary", () => undefined, !detail),
          button("删除", "secondary", () => undefined, !detail),
          button("关闭", "secondary", () => {
            interactionState.runtimeDetailOpen = false;
            appRuntimeState.selectedRecord = undefined;
            render();
      ...
      node("div", { className: "runtime-detail-body" }, [
        node("div", { className: "runtime-detail-main" }, [
          renderRuntimeDetailTabs(detail),
        ]),
        node("aside", { className: "runtime-detail-flow" }, [
          node("h3", { text: "审批信息" }),
          node("p", { text: detail?.flowSummary ? formatValue(detail.flowSummary) : "提交审批后显示流程节点与历史。" }),
        ]),
      ]),
    ]);
  }

  async function openRuntimeRecordDetail(recordId: EntityId): Promise<void> {
    interactionState.runtimeDetailOpen = true;
    interactionState.runtimeDetailTab = "detail";
    await loadRuntimeRecordDetail(recordId);
    render();
  }
  ...
  function renderFlowTemplates(): HTMLElement {
    ...
    const selectedTemplate = templates.find((item) => item.templateId === interactionState.selectedFlowTemplateId)
      ?? templates[0];

    return node("div", { className: "admin-page" }, [
      node("div", { className: "admin-page-head" }, [
        node("div", { className: "admin-page-title" }, [
          node("h1", { text: selectedTemplate ? `流程配置 · ${selectedTemplate.name}` : "流程配置" }),
          node("p", { text: "三栏：节点面板 · 画布 · 节点属性（config-spec §9.3）" }),
        ]),
        node("div", { className: "admin-page-actions" }, [
          button("预览", "secondary", noop, !selectedTemplate),
          button("保存默认图", "secondary", () => selectedTemplate && saveDefaultFlowGraph(selectedTemplate), !selectedTemplate || !actions.templateEdit.enabled),
          button("发布", "primary", () => selectedTemplate && publishFlowTemplate(selectedTemplate), !selectedTemplate || !actions.templatePublish.enabled),
        ]),
      ]),
      node("section", { className: "task-card compact-card" }, [
        node("div", { className: "form-grid" }, [
      ...
      renderDataTable(
        ...
        templates.map((item) => [
          ...
          node("div", { className: "row-actions" }, [
            button("设计", "primary", () => {
              interactionState.selectedFlowTemplateId = item.templateId;
              render();
            }),
      ...
      selectedTemplate
        ? node("div", { className: "flow-workspace" }, [
            node("aside", { className: "flow-palette" }, [
              node("h3", { text: "拖入画布" }),
              node("span", { className: "flow-node-type", text: "审批节点" }),
              node("span", { className: "flow-node-type", text: "条件节点" }),
              node("span", { className: "flow-node-type", text: "抄送节点" }),
              node("span", { className: "flow-node-type", text: "外部 API" }),
            ]),
            node("section", { className: "flow-canvas-wrap" }, [
              node("div", { className: "flow-canvas-node", text: `触发 · ${selectedModule()?.name ?? "业务模块"}` }),
              node("div", { className: "flow-canvas-node accent", text: "审批 · 按角色" }),
              node("div", { className: "flow-canvas-node", text: "结束 · 更新记录" }),
              node("p", { className: "flow-canvas-note", text: "画布拖拽将在后续版本接入；当前可通过「保存默认图」写入基础审批链。" }),
            ]),
            node("aside", { className: "flow-props" }, [
              node("h3", { text: "节点属性 · 审批" }),
              input("节点名称", "flowNodeName", "审批节点"),
              selectField("审批人", "flowApproverType", "ROLE", [["ROLE", "按角色"], ["MEMBER", "指定成员"]]),
              selectField("角色", "flowApproverRole", "", systemState.roles.map((item) => [item.roleId, item.name] as [string, string])),
              selectField("绑定模块", "selectedModuleId", appRuntimeState.selectedModuleId ?? "", moduleSelectOptions()),
              node("p", { className: "perm-note", text: "条件节点可引用本模块字段、字典项与成员/部门。" }),
            ]),
          ])
        : undefined,
    ]);
  ...
  function renderPlatformAdminPage(children: Child[]): HTMLElement {
    return node("div", { className: "platform-admin-page" }, children);
  }

  function renderPlatformPageHead(title: string, subtitle: string, ...actions: (HTMLElement | undefined)[]): HTMLElement {
    const actionNodes = actions.filter((item): item is HTMLElement => Boolean(item));
    return node("div", { className: "platform-admin-page-head" }, [
      node("div", { className: "platform-admin-page-title" }, [
        node("h2", { text: title }),
        node("p", { text: subtitle }),
      ]),
      actionNodes.length > 0
        ? node("div", { className: "platform-admin-page-actions" }, actionNodes)
        : undefined,
    ]);
  }

  function renderPlatformMetrics(items: [string, string][]): HTMLElement {
    return node("div", { className: "platform-admin-metrics" }, [renderPageMetrics(items)]);
  }

  function renderPlatformToolbar(
    onSearch: (pageNo?: number) => void | Promise<void>,
    onRefresh?: () => void | Promise<void>,
  ): HTMLElement {
    return node("div", { className: "platform-admin-toolbar" }, [
      input("搜索关键字", "keyword", ""),
      button("查询", "secondary", () => onSearch(1)),
      onRefresh ? button("刷新", "secondary", onRefresh) : undefined,
    ]);
  }
  ...
  async function runRouteLoad(route: AppRouteRecord): Promise<void> {
    ...
    if (route.name === "platform.roles") {
      ...
      await loadPlatformPermissionCatalog();
      return;
    }
    if (route.name === "platform.configs") {
    ...
    if (route.name === "platform.openapi") {
      await execute("PLAT-001", () => mySystemsPage.loadSystems());
      await loadPlatformOpenApiOverview();
      return;
    }
    if (route.name === "system.overview") {
  ...
  function fillPlatformRoleAuth(item: PlatformRoleVO): void {
    ...
    platformState.platformRoleMenuIds = item.menuIds?.map(String).join(",") ?? "";
    render();
  }
  ...
  async function authorizePlatformRole(item: PlatformRoleVO): Promise<void> {
    const form = readForm();
    const operationCodes = form.platformRolePermissionOperations ?? platformState.platformRoleOperationCodes;
    const menuIds = form.platformRolePermissionMenus ?? platformState.platformRoleMenuIds;
  ...
  async function loadPlatformPermissionCatalog(): Promise<void> {
    await execute("PLAT-020", async () => {
      ...
      platformState.permissionCatalog = catalog.detail;
  ...
  async function loadPlatformOpenApiOverview(): Promise<void> {
    await execute("PLAT-001/OPM-001", async () => {
      platformState.openApiLoading = true;
      const systems = mySystemsPage.state.systems.length > 0
        ? mySystemsPage.state.systems
        : (await platformCenter.systems.query({ pageNo: 1, pageSize: 50 })).records;
      const rows: PlatformOpenApiOverviewRow[] = [];
      for (const system of systems.filter((item) => item.status === "ENABLED")) {
        try {
          const response = await apiClient.call<PageResult<OpenApiClientListItemVO>>("OPM-001", {
            pathParams: { systemId: system.systemId },
            query: { pageNo: 1, pageSize: 50 },
          });
          for (const client of response.data?.records ?? []) {
            rows.push({
              ...
              moduleScope: summarizeOpenApiModuleScope(client.scopes),
              actionScope: summarizeOpenApiActionScope(client.scopes),
              dataScope: summarizeOpenApiDataScope(client.scopes),
              ipWhitelist: client.ipWhitelist?.join(", ") || "-",
              status: client.status,
            });
          }
        } catch {
          // 当前账号可能没有某个系统的 OpenAPI 查看权限，跳过即可。
        }
      }
      platformState.openApiRows = rows;
      platformState.openApiLoading = false;
      return { requestId: `platform-openapi-${Date.now()}` };
    });
  }
  ...
  async function loadSystemRoles(): Promise<void> {
    await execute("RBAC-005", async () => {
      ...
      if (!systemState.selectedSystemRoleId && systemState.roles[0]) {
        systemState.selectedSystemRoleId = systemState.roles[0].roleId;
      }
      return response;
    });
    const selected = systemState.roles.find((item) => item.roleId === systemState.selectedSystemRoleId) ?? systemState.roles[0];
    if (selected && !systemState.rolePermissions[selected.roleId]) {
      await loadSystemRolePermissions(selected);
    }
  }
  ...
  function platformPermissionCatalogLabel(catalog?: PlatformPermissionCatalogVO): string {
    if (!catalog) {
      return platformState.permissionCatalogSummary ?? "未加载";
    }
    return `${catalog.menus.length} 菜单 / ${catalog.operationCodes.length} 操作`;
  }

  function platformCatalogOperationOptions(catalog?: PlatformPermissionCatalogVO): [string, string][] {
    return (catalog?.operationCodes ?? []).map((code) => [code, code]);
  }

  function platformCatalogMenuOptions(catalog?: PlatformPermissionCatalogVO): [string, string][] {
    return (catalog?.menus ?? []).flatMap((item) => platformMenuOptionFromTree(item));
  }

  function platformMenuOptionFromTree(item: PlatformMenuTreeVO): [string, string][] {
    const current: [string, string][] = [[String(item.menuId), item.name]];
    const children = (item.children ?? []).flatMap((child) => platformMenuOptionFromTree(child));
    return [...current, ...children];
  }

  function summarizePlatformRolePermissions(role: PlatformRoleVO): [string, string, string, string] {
    const operations = role.operationCodes ?? [];
    const menuCount = role.menuIds?.length ?? 0;
    const flowCount = operations.filter((code) => code.includes("FLOW") || code.startsWith("PLAT_FLOW")).length;
    const appCount = operations.filter((code) => code.startsWith("OPENAPI") || code.includes("_APP_")).length;
    const logCount = operations.filter((code) => code.includes("AUDIT") || code.startsWith("OPS_")).length;
    const adminCount = Math.max(menuCount, operations.filter((code) => code.startsWith("PLAT_") && !code.includes("AUDIT") && !code.includes("FLOW")).length);
    return [
      adminCount > 0 ? `${adminCount} 项` : "-",
      flowCount > 0 ? `${flowCount} 项` : "-",
      appCount > 0 ? `${appCount} 项` : "-",
      logCount > 0 ? `${logCount} 项` : "-",
    ];
  }

  function summarizeOpenApiModuleScope(scopes: OpenApiScope[]): string {
    const modules = scopes.map((item) => item.moduleCode ?? item.scopeCode).filter(Boolean);
    return modules.length ? [...new Set(modules)].join(", ") : "-";
  }

  function summarizeOpenApiActionScope(scopes: OpenApiScope[]): string {
    const actions = scopes.flatMap((item) => item.actions ?? []);
    return actions.length ? [...new Set(actions)].join(", ") : "-";
  }

  function summarizeOpenApiDataScope(scopes: OpenApiScope[]): string {
    const dataScopes = scopes.map((item) => item.dataScope?.scopeType).filter(Boolean);
    return dataScopes.length ? [...new Set(dataScopes)].join(", ") : "-";
  }

  function maskOpenApiAccessKey(accessKey: string): string {
