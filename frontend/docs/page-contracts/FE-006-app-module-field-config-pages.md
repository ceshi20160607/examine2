...
| APP-003 | GET | `/api/v1/systems/{systemId}/apps/{appId}` | 查看应用详情 | `systemId`、`appId` | 无 | 无 | Authorization / X-Tenant-Id / X-Request-Id | `appId`、`latestVersionId`、`publishedVersionId`、`versionStatus` | `appStatus`、`versionStatus` | `MODULE_APP_NOT_FOUND`、`PERM_DENIED` | 详情只读系统上下文。 |
...
| APP-006 | POST | `/api/v1/systems/{systemId}/apps/{appId}/copy` | 复制应用 | `systemId`、`appId` | 无 | 无 | Authorization / X-Tenant-Id / X-Request-Id | `AppDetailVO` | `appStatus` | `MODULE_APP_NOT_FOUND`、`PERM_DENIED` | 仅调用冻结复制接口。 |
| APP-007 | GET | `/api/v1/systems/{systemId}/apps/templates` | 读取应用模板 | `systemId` | 无 | 无 | Authorization / X-Tenant-Id / X-Request-Id | 模板数组 | `appStatus` | `PERM_DENIED` | 只做模板选择，不做配置导入执行。 |
| MOD-001 | GET | `/api/v1/systems/{systemId}/apps/{appId}/modules` | 加载模块列表 | `systemId`、`appId` | 可选分页/关键字 | 无 | Authorization / X-Tenant-Id / X-Request-Id | `moduleId`、`appId`、`name`、`code`、`status`、`fieldCount`、`publishedVersionId` | `moduleStatus`、`versionStatus` | `MODULE_APP_NOT_FOUND`、`PERM_DENIED` | 空列表展示模块空态。 |
...
| MOD-003 | GET | `/api/v1/systems/{systemId}/modules/{moduleId}` | 查看模块详情 | `systemId`、`moduleId` | 无 | 无 | Authorization / X-Tenant-Id / X-Request-Id | `moduleId`、`name`、`code`、`status`、`versionStatus`、`flowBindingId` | `moduleStatus`、`versionStatus` | `MODULE_NOT_FOUND`、`PERM_DENIED` | 详情用于发布前摘要。 |
...
| MOD-006 | POST | `/api/v1/systems/{systemId}/modules/{moduleId}/publish-check` | 发布检查 | `systemId`、`moduleId` | 无 | `idempotencyKey` | Authorization / X-Tenant-Id / X-Request-Id / X-Idempotency-Key | `pass`、`issues[]`、`draftVersion` | `versionStatus` | `MODULE_PUBLISH_CHECK_FAILED`、`FIELD_UNIQUE_EXISTING_CONFLICT`、`MODULE_PAGE_FIELD_MISSING` | `issues[].fieldCode/schemaPath` 用于定位配置项。 |
| MOD-007 | POST | `/api/v1/systems/{systemId}/modules/{moduleId}/publish` | 发布模块 | `systemId`、`moduleId` | 无 | 可选 `draftVersion`、`publishRemark` | Authorization / X-Tenant-Id / X-Request-Id | `publishedVersionId`、`versionStatus`、`moduleStatus`、`publishedAt` | `PUBLISHED` | `MODULE_PUBLISH_CHECK_FAILED`、`MODULE_CONFIG_VERSION_CONFLICT`、`PERM_DENIED` | 成功后刷新 RUN-001/RUN-002 运行菜单和 schema 状态；不伪造未声明幂等语义。 |
| FIELD-001 | GET | `/api/v1/systems/{systemId}/modules/{moduleId}/fields` | 加载字段列表 | `systemId`、`moduleId` | 无 | 无 | Authorization / X-Tenant-Id / X-Request-Id | `fieldId`、`fieldCode`、`fieldName`、`fieldType`、`required`、`unique`、`status`、`fieldPermissions` | `fieldStatus`、`DynamicFieldType` | `MODULE_NOT_FOUND`、`PERM_DENIED` | 保存字段后刷新字段列表和 UI 预览。 |
| FIELD-002 | POST | `/api/v1/systems/{systemId}/modules/{moduleId}/fields` | 新建字段 | `systemId`、`moduleId` | 无 | `name`、`code`、`fieldType`；可选 `required`、`unique`、`defaultValue`、`options`、`dictTypeId`、`relationConfig`、`subTableConfig`、`fieldPermissions` | Authorization / X-Tenant-Id / X-Request-Id / X-Idempotency-Key | `FieldDefinitionVO` | `fieldStatus`、`DynamicFieldType` | `FIELD_CODE_DUPLICATED`、`FIELD_TYPE_UNSUPPORTED`、`FIELD_RELATION_INVALID`、`FIELD_SERIAL_RULE_INVALID` | 字段编码重复先本地提示，后端仍兜底。 |
| FIELD-003 | PUT | `/api/v1/systems/{systemId}/modules/{moduleId}/fields/{fieldId}` | 保存字段 | `systemId`、`moduleId`、`fieldId` | 无 | 同 FIELD-002；可选 `version` | Authorization / X-Tenant-Id / X-Request-Id | `FieldDefinitionVO` | `fieldStatus`、`DynamicFieldType` | `FIELD_TYPE_UNSUPPORTED`、`FIELD_DELETE_HAS_DATA`、`PERM_FIELD_WRITE_DENIED` | 字段类型切换时保留契约字段并重新校验 options/relation/subTable。 |
| FIELD-004 | PATCH | `/api/v1/systems/{systemId}/modules/{moduleId}/fields/{fieldId}/status` | 启停/删除字段 | `systemId`、`moduleId`、`fieldId` | 无 | `targetStatus`、可选 `reason`、`version` | Authorization / X-Tenant-Id / X-Request-Id | `FieldDefinitionVO` | `DRAFT`、`ENABLED`、`DISABLED`、`DELETED` | `FIELD_DELETE_HAS_DATA`、`MODULE_PAGE_FIELD_MISSING`、`PERM_DENIED` | 被 schema 引用字段禁用删除入口。 |
| FIELD-005 | GET | `/api/v1/systems/{systemId}/field-types` | 加载字段类型 | `systemId` | 无 | 无 | Authorization / X-Tenant-Id / X-Request-Id | `DynamicFieldType[]` 或字段类型元数据 | `DynamicFieldType` | `FIELD_TYPE_UNSUPPORTED`、`PERM_DENIED` | 页面模型也引用 `DYNAMIC_FIELD_TYPES` 兜底展示。 |
...
| UI-002 | PUT | `/api/v1/systems/{systemId}/modules/{moduleId}/ui/list-views/default` | 保存列表 schema | `systemId`、`moduleId` | 无 | `columns`；可选 `filters`、`sorters`、`fieldVisibility`、`schemaVersion` | Authorization / X-Tenant-Id / X-Request-Id | `UiSchemaVO` | 无 | `MODULE_PAGE_FIELD_MISSING`、`MODULE_CONFIG_VERSION_CONFLICT` | 只提交当前模块字段编码/ID。 |
...
| UI-004 | PUT | `/api/v1/systems/{systemId}/modules/{moduleId}/ui/forms/default` | 保存表单 schema | `systemId`、`moduleId` | 无 | `formSections`；可选 `fieldWritable`、`schemaVersion` | Authorization / X-Tenant-Id / X-Request-Id | `UiSchemaVO` | 无 | `MODULE_PAGE_FIELD_MISSING`、`PERM_FIELD_WRITE_DENIED` | 表单必填只引用字段定义，不写动态记录值。 |
...
| UI-006 | PUT | `/api/v1/systems/{systemId}/modules/{moduleId}/ui/details/default` | 保存详情 schema | `systemId`、`moduleId` | 无 | `detailBlocks`；可选 `fieldVisibility`、`schemaVersion` | Authorization / X-Tenant-Id / X-Request-Id | `UiSchemaVO` | 无 | `MODULE_PAGE_FIELD_MISSING`、`MODULE_CONFIG_VERSION_CONFLICT` | 无效字段由本地和后端双重校验。 |
| UI-007 | PUT | `/api/v1/systems/{systemId}/modules/{moduleId}/ui/menu` | 保存运行菜单 | `systemId`、`moduleId` | 无 | `menuCode`、`menuName`；可选 `menuParentId`、`visible`、`sortOrder` | Authorization / X-Tenant-Id / X-Request-Id | `UiSchemaVO` | 无 | `MODULE_MENU_CODE_DUPLICATED`、`PERM_DENIED` | 发布成功后通过 RUN-001 刷新可见菜单。 |
| UI-008 | PUT | `/api/v1/systems/{systemId}/modules/{moduleId}/ui/actions` | 保存动作按钮 | `systemId`、`moduleId` | 无 | `actionCodes`、`actions`、可选 `schemaVersion`、`idempotencyKey` | Authorization / X-Tenant-Id / X-Request-Id / X-Idempotency-Key | `UiSchemaVO` | 无 | `PERM_FIELD_WRITE_DENIED`、`MODULE_CONFIG_VERSION_CONFLICT` | 使用 `AvailableAction` 语义，不散落魔法按钮。 |
...
| 字段名称/编码 | `name`、`code` / 回显 `fieldName`、`fieldCode` | 字段表单 / FIELD-001 | 是 | 提交 BO 使用 `name/code`；schema 引用使用 `fieldCode` 或 `fieldId` | 编码重复由 `duplicateCodes()` 本地提示并由后端兜底 |
...
| 发布检查定位 | `issues[].fieldCode/schemaPath/targetType` | MOD-006 响应 | 否 | `locatePublishIssue()` 转为配置定位字符串 | blocking issue 阻止发布 |
...
| 版本状态 | `versionStatus` | 模块详情、发布结果 | `PUBLISHED` 后刷新 RUN-001/RUN-002 |
...
| 发布检查失败 | MOD-006 `pass=false` 或错误码 `MODULE_PUBLISH_CHECK_FAILED` | `issues[]`，用 `fieldCode/schemaPath/targetType` 定位 | 必须展示 MOD-006 requestId | 修复配置后重新检查 |
