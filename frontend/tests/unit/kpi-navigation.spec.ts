...
function applySession(permissions: string[], shells: Array<'SYSTEM_RUNTIME' | 'SYSTEM_ADMIN'> = ['SYSTEM_RUNTIME', 'SYSTEM_ADMIN']) {
...
describe('KPI admin and runtime navigation', () => {
  beforeEach(() => {
  ...
  it('registers the ordinary member KPI route and top entry without an admin permission gate', async () => {
    applySession([], ['SYSTEM_RUNTIME'])
    const routeRecord = router.getRoutes().find(item => item.name === 'system-kpis')
    expect(routeRecord?.path).toBe('/systems/:systemId/kpis')
    expect(routeRecord?.meta.requiredPermissions).toBeUndefined()
    const wrapper = mount(SystemLayout, { global: { stubs } })
    await flushPromises()
    expect(wrapper.get('a[data-to="/systems/10/kpis"]').text()).toContain('KPI')
  })

  it('guards the management editor route and navigation with both configuration permissions', () => {
    const routeRecord = router.getRoutes().find(item => item.name === 'system-admin-kpis')
    expect(routeRecord?.path).toBe('/systems/:systemId/admin/kpis')
    ...
    expect(mount(SystemAdminLayout, { global: { stubs } }).findAll('a[data-to="/systems/10/admin/kpis"]')).toHaveLength(1)
    applySession(['system.admin.access'])
    expect(mount(SystemAdminLayout, { global: { stubs } }).find('a[data-to="/systems/10/admin/kpis"]').exists()).toBe(false)
  })
})
