import { beforeEach, describe, expect, it, vi } from 'vitest'

import { quickCreateApi } from '@/services/quickCreate'

const envelope = (data: unknown) => JSON.stringify({
  code: 'OK',
  message: '',
  data,
  requestId: 'request-1',
  traceId: 'trace-1',
  errors: [],
})

describe('runtime quick create API', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    document.cookie = 'EXAMINE_CSRF=csrf-token; path=/'
  })

  it('loads the frozen ordered module DTO without mutation headers', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(envelope({
      items: [{
        moduleCode: 'work_order',
        moduleName: 'Work orders',
        schemaVersionId: '21',
      }],
    }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
    vi.stubGlobal('fetch', fetchMock)

    await quickCreateApi.list('system / 10')

    expect(fetchMock.mock.calls[0]![0]).toBe(
      '/api/v1/systems/system%20%2F%2010/runtime/quick-create-modules',
    )
    const options = fetchMock.mock.calls[0]![1] as RequestInit
    const headers = options.headers as Headers
    expect(options.method).toBeUndefined()
    expect(headers.get('X-CSRF-Token')).toBeNull()
    expect(headers.get('Idempotency-Key')).toBeNull()
  })
})
