...
* PM 必须把 `docs/product/final-user-goal.md` 作为最高优先级的产品目标基线来理解和执行；若该文件缺失，必须先补齐“最终用户目标基线”，再允许 UI/UX 冻结、前端重构或最终打包。
* PM 每次裁决“是否完成、是否打包、是否可上线”前，必须逐条对照 `docs/product/final-user-goal.md` 的主流程、UI 最低标准、不得通过条件和连续验收剧本；不能只引用 PRD、接口、构建或局部 E2E。
* PM 如果不能用普通用户语言回答“这个系统给谁用、登录后第一屏是什么、管理员如何搭建业务系统、普通用户如何处理业务数据、对外应用如何开放数据、出错后如何恢复”，必须判定当前阶段 `fail/blocked`，不得进入最终包。
* 当用户明确要求“这是最后一次”或同等意思时，PM 必须先冻结 `docs/product/p16-final-rework-charter.md` 或同等最终返工章程，明确不允许行为、页面范围、交付顺序、验收矩阵和最终包闸门；缺少该章程时不得直接进入实现或打包。
* PM 必须先维护“产品运行模型”，再拆 UI 和开发任务。产品运行模型必须主动定义目标用户、角色工作空间、信息架构、关键用户剧本、权限边界、错误恢复、日志分层和可用性验收，不得只是把用户指出的问题追加成清单。缺少 `docs/product/product-vision-and-operating-model.md` 或同等产物时，UI/UX 不得冻结，frontend 不得进入实现，reviewer 必须判定 `fail(pm-product-model-missing)`。
* 产品运行模型中的“角色”默认是用户画像和权限模板，不得直接等同于写死角色名。实现必须通过可配置角色、权限点、菜单可见性、字段权限、数据范围和真实系统成员上下文控制页面；只识别固定角色名称或前端伪造权限时，reviewer 必须判定失败。
* 当前 UI 若被用户或 reviewer 判定为“普通人不可用/接口调试台/视觉粗糙”，PM 不得继续要求 frontend 小修小补。必须由 UI/UX 重新输出信息架构、页面布局、视觉层级、状态和中文文案，frontend 保留可复用 API、store 和动态渲染能力，但按新 UI 重组页面。
* PM 在开发前必须输出问题预判：角色权限、平台/系统上下文、术语边界、主线流程、空态异常、自定义角色、真实数据、部署链路。预判未在 UI/UX 和任务拆分中落点时，不得进入实现。
* 如果同一项目在部署试用后连续出现 2 次以上“普通人不能正常使用、PM 过度验收、页面割裂或主流程不清”的反馈，必须暂停碎片化分期补丁，进入“完整系统基线重整”。重整产物必须包含 `docs/product/integrated-system-baseline.md`，冻结普通用户视角的一体化系统目标、导航、核心流程、页面归属、接口依赖、权限边界、验收剧本和不打包条件。
* 部署试用反馈出现“用户不知道怎么用、登录后入口不对、页面像工程调试台、系统账号/平台账号割裂、主流程需要用户反复指出”等问题时，PM 必须先执行全项目可用性重整，不能立即让 backend/frontend 局部补丁。重整产物必须明确产品定位、用户画像、登录落点、普通用户默认工作台、管理员搭建路径、信息架构、页面主动作、列表分页、新建入口、退出入口和真实验收剧本。
* PM 不得把“已经写过完整系统基线”当作通过依据；如果用户部署反馈和基线结论矛盾，必须立即撤回 `fullProjectDeployable=true`、撤回对应 accepted 结论，并把进度状态改为 retracted/in_progress。未撤回前，validator/reviewer 不得允许继续打最终包。
* PM 在重整后必须先回答“这个系统到底给谁用、用户登录后第一屏看到什么、普通成员如何只处理自己系统的数据、管理员如何不用理解底层账号模型就开通成员、用户如何退出、列表如何翻页、在哪里新建业务数据”。这些问题没有答案时，frontend 不得继续实现页面，test 不得只跑接口或管理员链路。
* 完整系统基线重整通过前，不得继续新增最终包，不得把局部修复、局部 E2E、构建通过或某个角色 pass 解释为“系统完整可用”。PM 必须先回答“普通人登录后怎么建系统、怎么建模块数据、怎么发布、怎么使用、怎么对外开放、出错后怎么恢复”。
* 完整系统验收必须使用一个连续的端到端剧本，而不是多个割裂页面 smoke 拼接：平台管理员登录 -> 创建业务系统 -> 进入系统总览 -> 完成系统设置 -> 建业务模块和字段 -> 发布页面和菜单 -> 普通成员使用运行台新增/查询/编辑/提交数据 -> 配置应用并授权系统数据/流程/能力 -> 外部或其它系统调用成功 -> 审计/日志能追踪。任一环节缺页面、缺权限、缺反馈、缺测试证据，`fullProjectDeployable=false`。
* 普通业务用户验收必须单独执行：普通用户登录后只能看到自己所在业务系统、被授权菜单/模块、被授权字段和自己数据范围内的数据；不得看到管理配置、字段设计、页面发布、权限配置等管理员入口。只用平台管理员跑通不能代表业务用户可用。
* 应用验收必须单独执行：平台级“应用”是平台能力域，和平台级“流程”、统一登录/入口、平台组织权限等同属平台能力，用于系统间调用、系统对外调用、外部调用流程或后续其它平台能力；系统内可以存在“应用配置”，但它表示某个自建系统对平台级应用能力的启用、功能范围、事件绑定、授权范围和运行入口配置，不等于模块或模块分组。系统内用于承载数据的对象仍称为“模块”，模块多时可用“模块分组”组织页面和菜单。
* 日志验收必须分层执行：平台日志覆盖平台账号、系统、平台配置、对外应用、平台级流程和运维；业务系统日志覆盖成员/角色、模块/字段/页面/菜单、运行数据、系统内流程、文件、导出和该系统内 OpenAPI 调用。平台日志和业务系统日志混在一起时，reviewer 必须判定为信息架构失败。
* PRD、API、UI/UX 和前端实现前必须冻结产品术语表；术语表必须明确“平台”“自建系统”“平台级应用”“系统内应用配置”“平台级流程”“系统内流程绑定”“模块”“模块分组”“运行台”“外部接入/调用方”的层级关系：平台类似统一登录/SSO 入口，但还承载平台组织权限、平台级应用、平台级流程和其它平台能力；自建系统是用户搭建业务后台的空间，承载系统组织架构、用户部门、角色权限、字典、模块、模块分组、页面、菜单、系统内应用配置、流程事件绑定和运行台；模块承载字段、页面和业务数据。术语未冻结或页面文案混用时，frontend 不得进入实现，reviewer 必须判定 `fail(term-ambiguity)`。
* 平台用户只能在平台内由有权限的管理员创建、导入或受控开通，不能提供普通账号自助开通入口；面向外部或新用户的入口应表达为“创建系统/创建业务系统”，创建流程同时完成系统、系统创建人和系统管理员身份初始化。系统创建人默认成为该自建系统管理员，但不因此获得平台组织、平台权限、平台应用或平台流程的管理权限。
* UI/UX 设计必须包含“原型流程追踪矩阵”：原始需求/原型中的每条关键流程图或用户链路，必须映射到页面、路由、接口、权限、状态反馈和测试用例。若流程图未生成、未映射，或当前页面无法解释普通用户如何完成该流程，PM 不得验收，test 不得只用局部 E2E 代替全流程验收。
* 涉及对外或系统间调用能力时，必须明确平台级应用属于平台能力管理域：应用用于管理调用方、appKey/secret、scope、IP 白名单、限流、调用日志，以及把系统数据、模块动作、流程或文件能力授权给外部或其它系统。系统内“应用配置”用于配置该系统如何使用平台级应用能力，例如启用哪些功能、开放哪些模块/字段/动作、绑定哪些事件或流程、生成哪些运行入口；不得把系统内应用配置和模块/模块分组混成同一概念。两者菜单、表、接口、权限和 UI 文案必须分离。
* 平台当前固定基础能力至少包含：平台组织架构、平台权限、平台级 Flow、平台级应用、平台日志、平台审计、平台待办、平台消息和统一入口；其它主流平台能力可以作为后续扩展，但不得在当前 MVP 中抢占这些基础能力的位置。
* 每个自建系统也必须具备系统级基础能力：系统组织/用户部门、系统权限、字典、模块/模块分组、系统内应用配置、流程事件绑定、系统日志、系统审计、系统待办、系统消息和业务运行台。系统级日志/审计/待办/消息不得全部混到平台级页面里。
* 如果一个账号属于多个系统，登录后必须有明确分流策略：只有一个可用系统且无平台管理入口时直接进入该系统的普通运行台；有多个系统时进入系统选择页，显示系统名称、用户在该系统的角色、最近访问、待办数和默认进入按钮；拥有平台管理权限时默认进入平台工作台但必须提供“进入系统/切换系统”入口。缺少多系统选择或默认落点规则时，PM/UIUX/reviewer 必须判定失败。
* 平台层和系统层都必须区分管理态与普通态：平台管理态面向平台管理员维护系统、平台账号、应用、审计和运维；平台普通态只展示用户可进入的系统和个人信息。系统管理态面向系统管理员维护成员、模块、字段、页面、菜单、权限、流程和系统审计；系统普通态就是运行台，只展示被授权模块、待办、文件/导出和个人入口。页面、路由和导航混用管理态/普通态时必须 fail。
* 真实后台 UI 原型和前端实现必须支持左侧导航折叠/展开。折叠态至少保留图标、tooltip、当前系统/工作台识别和可恢复展开入口；不能只做固定宽侧栏。
* 新建、编辑、详情的呈现方式必须一致：若模块采用右侧抽屉，则新增、编辑、详情、提交结果和关键只读详情都应在抽屉体系内完成；只有对象复杂到需要完整工作区时才可使用独立详情页，并必须在 UI/UX 设计中说明原因。不得出现“新增/编辑抽屉，详情无理由独立页面”的交互割裂。
* 动态字段驱动的列表必须包含高级筛选、表头/列配置、排序、分页、保存视图、字段权限影响下的列可见/可导出控制；缺少高级筛选或列配置时，不能判定为主流可用后台。
* 权限设计必须同时表达菜单权限、页面权限、按钮/动作权限、字段可见/可写权限、数据权限和导出/OpenAPI 字段范围权限。UI/UX 原型、前端实现和测试若只表达菜单权限或按钮权限，reviewer 必须判定权限模型不完整。
...
* 主 agent/Orchestrator 不得绕过 `uiux`、Open Design 和 reviewer 直接手写 HTML 并称其为 UI 设计、高保真原型、可用页面或可实现依据。主 agent 手写 HTML 只能标记为 `scratch/structure-draft`，且必须在文件名、评审说明和进度中声明“非 UI/UX 产物、非 Open Design 产物、不可作为前端实现依据”。若用户要求看 UI 效果，必须先选择 `uiux` 角色产出设计，或明确记录 Open Design 账号/模型不可用后的降级原因，再由 reviewer 审查后才能交给用户复核。
* 任何展示给用户的 UI 原型必须附带来源说明：`uiux-agent`、`open-design`、`frontend-implementation` 或 `main-agent-scratch`。来源为 `main-agent-scratch` 时，PM 不得使用“设计”“高保真”“可用”“主流系统”等字样描述，只能作为问题讨论草稿；reviewer 必须默认按失败审查，除非用户明确只要草图。
* UI 原型提交用户前必须先通过最小视觉闸门：不得出现大面积无意义空白、卡片被拉伸成异常高度、导航项混层、平台/系统/普通用户工作区混在同一菜单、列表缺表头/分页/筛选、日志审计非列表化、Flow 与应用合并成一个概念、系统数量多时只靠左侧菜单承载入口。任一项存在，reviewer 必须判定 `fail(ui-quality-gate)`。
* UI/UX 信息架构必须拆成三个工作空间：平台管理工作台、系统管理工作台、系统普通用户运行台。平台管理只能管理平台组织、平台账号/权限、平台级 Flow、平台级应用、平台日志、平台审计、平台消息、平台待办和系统列表；系统管理必须先进入具体系统后才能管理系统成员、部门、角色、字典、模块、字段、页面、菜单、系统内应用配置、流程绑定、系统日志和系统审计；系统普通运行台只能展示被授权模块、待办、消息、常用数据和个人入口。
* 系统数量可能达到上百或上千时，系统入口不得放在左侧导航树中。必须使用系统选择页、系统搜索、最近访问、收藏、角色/状态筛选、分页或虚拟滚动，以及顶部“进入系统/切换系统”入口。左侧导航只承载当前工作空间的功能。
* Flow 和应用在平台层必须作为同级但不同域表达：平台级 Flow 包含流程模板、流程实例、待办审批、流程日志；平台级应用包含应用列表、凭证管理、授权范围、IP 白名单、限流策略、调用日志。系统内也必须区分“流程绑定”和“应用配置”，不得把 Flow 塞进应用配置，也不得把应用做成模块分组。
