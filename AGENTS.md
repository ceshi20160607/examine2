...
* 完整系统验收必须使用一个连续的端到端剧本，而不是多个割裂页面 smoke 拼接：平台管理员登录 -> 创建业务系统 -> 进入系统总览 -> 完成系统设置 -> 建业务模块和字段 -> 发布页面和菜单 -> 普通成员使用运行台新增/查询/编辑/提交数据 -> 配置平台级对外应用并授权系统数据/能力 -> 外部调用成功 -> 审计/日志能追踪。任一环节缺页面、缺权限、缺反馈、缺测试证据，`fullProjectDeployable=false`。
* 普通业务用户验收必须单独执行：普通用户登录后只能看到自己所在业务系统、被授权菜单/模块、被授权字段和自己数据范围内的数据；不得看到管理配置、字段设计、页面发布、权限配置等管理员入口。只用平台管理员跑通不能代表业务用户可用。
* 平台级对外应用验收必须单独执行：对外应用用于业务系统和外部系统通信，可授权业务系统数据、模块动作、字段范围、数据范围，以及平台级流程/文件/消息等平台能力；它不得和系统内业务应用混为同一菜单或同一页面概念。
...
* PRD、API、UI/UX 和前端实现前必须冻结产品术语表；同名词在不同层级有不同含义时必须强制改名并建立映射，例如“系统内业务应用/模块”和“平台级对外应用/OpenAPI 客户端”不得都简称为“应用”。术语未冻结或页面文案混用时，frontend 不得进入实现，reviewer 必须判定 `fail(term-ambiguity)`。
...
* 涉及平台级对外能力时，必须明确它属于平台层还是系统层：平台级对外应用用于管理外部应用、appKey/secret、scope、IP 白名单、限流、调用日志，以及把系统内数据、模块动作、流程或文件能力授权给外部；系统内业务应用只用于组织模块、字段、页面、菜单和运行台。两者菜单、表、接口、权限和 UI 文案必须分离。
