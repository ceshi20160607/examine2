...
* 应用验收必须单独执行：本项目语境中的“应用”默认与“系统”“流程”同级，是系统间调用、系统对外调用和平台能力开放的管理主体。应用可授权某个系统的数据、模块动作、字段范围、数据范围，也可授权流程、文件、消息或后续其它模块能力给外部或其它系统使用。除非用户另行改口，系统内部用于组织字段、页面、菜单和运行台的数据对象不得命名为“应用”，应称为“模块/业务模块/模块分组/运行菜单”等，避免把应用误做成系统内配置菜单。
...
* PRD、API、UI/UX 和前端实现前必须冻结产品术语表；“应用”不得再被默认解释为系统内业务应用。术语表必须明确“系统”“应用”“流程”“模块”“运行台”“外部接入/调用方”的层级关系：系统承载业务配置和运行数据；应用承载系统间/系统对外/外部使用项目能力的调用管理；流程是可被系统内运行和应用对外授权使用的同级能力；模块承载字段、页面、菜单和业务数据。术语未冻结或页面文案混用时，frontend 不得进入实现，reviewer 必须判定 `fail(term-ambiguity)`。
...
* 涉及对外或系统间调用能力时，必须明确应用属于与系统、流程同级的开放能力管理域：应用用于管理调用方、appKey/secret、scope、IP 白名单、限流、调用日志，以及把系统数据、模块动作、流程或文件能力授权给外部或其它系统。系统内配置域只负责模块、字段、页面、菜单、运行台和权限；不得再把系统内配置域命名为“业务应用”。两者菜单、表、接口、权限和 UI 文案必须分离。
