package com.unique.upload.service.impl;
...
public class LocalFileServiceImpl implements FileService {
    ...
    public UploadEntity uploadFile(InputStream inputStream, UploadEntity entity, String bucketName) {
        ...
        checkPath(bucketName + SLASH + key.substring(0, key.lastIndexOf(SLASH)));
        try {
    ...
    public UploadEntity uploadTempFile(InputStream inputStream, UploadEntity entity, String bucketName) {
        String fileName = buildFileName(entity.getFileId(), entity.getName());
        String key = "temp/" + fileName;
        String path = bucketName + SLASH + key;
        checkPath(bucketName + SLASH + "temp/" + fileName.substring(0, fileName.lastIndexOf(SLASH)));
        entity.setPath(key);
        try {
    ...
    public InputStream downFile(UploadEntity entity, String bucketName) {
        ...
        if (!file.exists()) {
            throw new RuntimeException("文件不存在" + file.getAbsoluteFile());
        }
        try {
