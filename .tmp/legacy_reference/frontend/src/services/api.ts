...
export const p1SessionApi = {
  ...
  registerFirstSystem(input: RegisterFirstSystemInput, idempotencyKey: string) {
  ...
  listAuthorizedSystems() {
    return apiRequest<SystemSummary[]>('/api/v1/context/systems')
  },
  switchSystem(systemId: string) {
    return apiRequest<AuthResult>(`/api/v1/context/systems/${encodeURIComponent(systemId)}:switch`, {
      method: 'POST',
    })
  },
  currentContext() {
    return apiRequest<AuthResult>('/api/v1/me/context')
  },
  refresh() {
    return apiRequest<AuthResult>('/api/v1/auth/refresh', { method: 'POST' })
  },
  logout() {
