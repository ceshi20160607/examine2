-- Confirmation-gated creation of one Work task or one personal daily-report
-- draft. AI persistence contains only redacted projections; the Work owner
-- alone can open the authenticated command and owns durable execution replay.
...
CREATE TABLE un_ai_work_proposal (
    ...
    CONSTRAINT ck_ai_work_proposal_operation CHECK (
        operation IN ('WORK_TASK_DRAFT','WORK_DAILY_REPORT_DRAFT')),
    CONSTRAINT ck_ai_work_proposal_hash CHECK (
        plan_hash REGEXP '^[0-9a-f]{64}$'),
    CONSTRAINT ck_ai_work_proposal_json CHECK (
        (preview_json IS NULL OR JSON_TYPE(preview_json) = 'OBJECT')
        AND (result_json IS NULL OR JSON_TYPE(result_json) = 'OBJECT')),
    CONSTRAINT ck_ai_work_proposal_confidence CHECK (
        confidence BETWEEN 0.00000 AND 1.00000),
    CONSTRAINT ck_ai_work_proposal_payload CHECK (
    ...
    CONSTRAINT ck_ai_work_proposal_time CHECK (
...
CREATE TABLE un_ai_work_attempt (
    ...
    UNIQUE KEY uk_ai_work_attempt_key (
        system_id, tenant_id, proposal_id, action, request_key),
    CONSTRAINT fk_ai_work_attempt_root FOREIGN KEY (
        system_id, tenant_id, proposal_id)
        REFERENCES un_ai_work_proposal (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT ck_ai_work_attempt_identity CHECK (
    ...
    CONSTRAINT ck_ai_work_attempt_action CHECK (action = 'CONFIRM'),
    CONSTRAINT ck_ai_work_attempt_key CHECK (
        request_key REGEXP '^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$'),
    CONSTRAINT ck_ai_work_attempt_hash CHECK (
        request_hash REGEXP '^[0-9a-f]{64}$'
        AND (result_hash IS NULL OR result_hash REGEXP '^[0-9a-f]{64}$')),
    CONSTRAINT ck_ai_work_attempt_state CHECK (
        (status = 'EXECUTING' AND finished_at IS NULL
            AND result_hash IS NULL)
        OR (status IN ('SUCCEEDED','FAILED','EXPIRED','STALE','PERMISSION_DENIED')
            AND finished_at IS NOT NULL AND result_hash IS NOT NULL)),
    CONSTRAINT ck_ai_work_attempt_result CHECK (
        result_code REGEXP '^[A-Z][A-Z0-9_]{1,63}$')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE un_ai_work_event (
    ...
    KEY idx_ai_work_event_root (
        system_id, tenant_id, proposal_id, revision, id),
    CONSTRAINT fk_ai_work_event_root FOREIGN KEY (
        system_id, tenant_id, proposal_id)
        REFERENCES un_ai_work_proposal (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_ai_work_event_attempt FOREIGN KEY (
        system_id, tenant_id, attempt_id)
        REFERENCES un_ai_work_attempt (system_id, tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT ck_ai_work_event_identity CHECK (
        id > 0 AND account_id > 0 AND member_id > 0 AND session_id > 0
        AND turn_id > 0 AND policy_version_id > 0 AND provider_id > 0
        AND provider_version >= 0 AND proposal_id > 0
        AND (attempt_id IS NULL OR attempt_id > 0) AND actor_member_id > 0
        AND revision >= 0),
    CONSTRAINT ck_ai_work_event_type CHECK (
        event_type IN ('CLARIFICATION_REQUIRED','PROPOSED','CONFIRMING',
                       'SUCCEEDED','FAILED','REJECTED','EXPIRED','STALE',
                       'PERMISSION_DENIED')),
    CONSTRAINT ck_ai_work_event_state CHECK (
        ...
        AND (from_state IS NULL OR from_state IN (
             ...
             'SUCCEEDED','FAILED','REJECTED','EXPIRED','STALE',
             'PERMISSION_DENIED'))),
    CONSTRAINT ck_ai_work_event_result CHECK (
        result_code REGEXP '^[A-Z][A-Z0-9_]{1,63}$'
...
CREATE TABLE un_work_ai_draft_execution (
    ...
    UNIQUE KEY uk_work_ai_draft_execution_proposal (
        system_id, tenant_id, member_id, proposal_id),
    UNIQUE KEY uk_work_ai_draft_execution_key (
        system_id, tenant_id, member_id, idempotency_key),
    CONSTRAINT fk_work_ai_draft_execution_member_tenant FOREIGN KEY (
        system_id, member_id, tenant_id)
        REFERENCES un_plat_member_tenant (system_id, member_id, tenant_id)
        ON DELETE RESTRICT,
    CONSTRAINT ck_work_ai_draft_execution_identity CHECK (
    ...
    CONSTRAINT ck_work_ai_draft_execution_operation CHECK (
        operation IN ('WORK_TASK_DRAFT','WORK_DAILY_REPORT_DRAFT')),
    CONSTRAINT ck_work_ai_draft_execution_tokens CHECK (
    ...
    CONSTRAINT ck_work_ai_draft_execution_hash CHECK (
        payload_hash REGEXP '^[0-9a-f]{64}$'),
    CONSTRAINT ck_work_ai_draft_execution_result CHECK (
    ...
    CONSTRAINT ck_work_ai_draft_execution_time CHECK (
