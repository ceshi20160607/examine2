-- Permission-based termination extends the existing optimistic Flow state machine.
...
ALTER TABLE un_flow_history_event
    ADD CONSTRAINT ck_flow_history_event CHECK (
        ...
        OR (event_sequence BETWEEN 2 AND 11 AND event_type = 'TERMINATED'
...
SELECT
    ...
    'Terminate pending flow instances',
    'ACTION',
    'ACTIVE',
    UTC_TIMESTAMP(3),
...
WHERE NOT EXISTS (
    ...
    WHERE existing.scope_type = 'SYSTEM'
      ...
      AND existing.permission_code = 'flow.instance.terminate'
);
...
JOIN un_plat_permission permission_row
 ...
 AND permission_row.permission_code = 'flow.instance.terminate'
