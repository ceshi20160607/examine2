-- P4-C5 adds tenant-scoped, transactionally allocated auto numbers and canonical
-- storage for the six server-computed record field types.
...
CREATE TABLE un_module_auto_number_sequence (
    ...
    KEY idx_auto_number_field (
        system_id, logical_module_id, logical_field_id, tenant_id
    )
...
ALTER TABLE un_module_record_unique
    ADD CONSTRAINT ck_ru_supported_type CHECK (
        field_type IN (
            ...
            'PHONE', 'EMAIL', 'URL', 'IDENTITY', 'BARCODE', 'SECRET', 'AUTO_NUMBER'
        )
    );
