...
$requiredFiles = @(
    ...
    'templates/project-phase.md',
    'templates/requirement-package.md',
    'templates/acceptance-case.md',
    ...
    'scripts/test-base-contracts.ps1',
    'scripts/build-framework-manifest.ps1',
    'scripts/sync-base-instance.ps1',
    'skills/compile-project-delivery/SKILL.md',
    'skills/compile-project-delivery/scripts/validate_delivery_plan.py',
    'skills/build-management-ui/SKILL.md',
    'skills/build-management-ui/scripts/validate_ui_contract.py',
    'skills/verify-by-delivery-level/SKILL.md',
    'skills/verify-by-delivery-level/scripts/validate_test_plan.py'
)

if ($Instance) {
    $requiredFiles += 'INSTANCE.md'
    $requiredFiles += 'base.lock.json'
    $requiredFiles += 'framework-manifest.json'
...
$baseContractTests = Join-Path $rootPath 'scripts/test-base-contracts.ps1'
& $baseContractTests -Root $rootPath | Out-Null
if ($Instance) {
    $instanceLock = Get-Content -Raw -Encoding UTF8 (Join-Path $rootPath 'base.lock.json') | ConvertFrom-Json
    $lockedBaseRoot = [System.IO.Path]::GetFullPath((Join-Path $rootPath ([string]$instanceLock.basePath)))
    & (Join-Path $rootPath 'scripts/sync-base-instance.ps1') `
        -BaseRoot $lockedBaseRoot -InstanceRoot $rootPath -Mode Verify | Out-Null
} else {
    $frameworkManifest = Get-Content -Raw -Encoding UTF8 (Join-Path $rootPath 'framework-manifest.json') | ConvertFrom-Json
    if ($frameworkManifest.schemaVersion -ne 1 -or $frameworkManifest.frameworkVersion -ne '3.2.0') {
        throw 'Base framework manifest version is invalid.'
    }
    foreach ($manifestEntry in @($frameworkManifest.files)) {
        $manifestFile = Join-Path $rootPath ([string]$manifestEntry.path)
        if (-not (Test-Path -LiteralPath $manifestFile -PathType Leaf)) {
            throw "Base framework manifest file is missing: $($manifestEntry.path)"
        }
        $manifestActualHash = (Get-FileHash -LiteralPath $manifestFile -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($manifestActualHash -ne $manifestEntry.sha256) {
            throw "Base framework manifest hash drift: $($manifestEntry.path)"
        }
    }
}
if ($Instance -and $Mode -in @('Active', 'Release')) {
    $deliveryContractPath = Join-Path $rootPath 'session/project-delivery.json'
    if (-not (Test-Path -LiteralPath $deliveryContractPath -PathType Leaf)) {
        throw 'Active/release instance validation requires session/project-delivery.json.'
    }
    & python (Join-Path $rootPath 'skills/compile-project-delivery/scripts/validate_delivery_plan.py') `
        $deliveryContractPath --source-root (Split-Path -Parent $rootPath) | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw 'session/project-delivery.json failed the canonical delivery-plan validator.'
    }
}
...
if ($null -eq $stateObject.projectDelivery -or
    $stateObject.projectDelivery.source -ne 'session/project-delivery.json' -or
    $stateObject.projectDelivery.schema -ne 'skills/compile-project-delivery/references/typed-contract.md' -or
    $stateObject.projectDelivery.validator -ne 'skills/compile-project-delivery/scripts/validate_delivery_plan.py' -or
    $stateObject.projectDelivery.status -notin @('not_compiled', 'draft', 'accepted')) {
    throw 'State projectDelivery must declare the typed delivery source, schema, validator and status.'
}
...
if ($stateObject.schemaVersion -lt 6) {
    throw 'State schemaVersion must be at least 6.'
}
if ($stateObject.frameworkVersion -ne '3.2.0') {
    throw 'State frameworkVersion must be 3.2.0 for this Base contract set.'
}
...
if ($stateObject.deliveryCadence.taskEstimateMinMinutes -ne 10 -or
    ...
    $stateObject.deliveryCadence.schedulingPolicy -ne 'disjoint_modules_parallel_by_default' -or
    $stateObject.deliveryCadence.verificationPolicy -ne 'task_affected_then_cycle_once_then_module_page_then_phase_then_project' -or
    $stateObject.deliveryCadence.performancePolicy -ne 'external_optional_after_project_completion') {
...
if ($workstreams.Count -gt 0) {
    ...
    foreach ($workstream in $workstreams) {
        ...
        foreach ($task in @($workstream.tasks)) {
            ...
            foreach ($field in @('id', 'result', 'requirementIds', 'acceptanceCaseIds', 'moduleScope',
                    'writeScope', 'resourceScope', 'dependsOnArtifacts', 'allowedChanges',
                    'forbiddenChanges', 'baseBindings', 'newSemanticsAllowed',
                    'unresolvedDecisionIds', 'estimateMinutes', 'qualityStage',
                    'responsiveScope', 'acceptanceCommand')) {
            ...
            if ([string]::IsNullOrWhiteSpace([string]$task.id) -or
                ...
                @($task.acceptanceCaseIds).Count -eq 0 -or
                @($task.writeScope).Count -eq 0 -or
                @($task.allowedChanges).Count -eq 0 -or
                @($task.forbiddenChanges).Count -eq 0 -or
                @($task.baseBindings).Count -eq 0 -or
                $task.newSemanticsAllowed -ne $false -or
                @($task.unresolvedDecisionIds).Count -gt 0 -or
                ("$($task.id) $($task.result)" -match '(?i)performance|benchmark|load[ _-]?test|stress|k6|jmeter|gatling|百万|千万|压测|并发容量')) {
...
if ($null -ne $stateObject.active.slice) {
    ...
    if ($stateObject.active.slice.targetMinutes -le 0 -or $stateObject.active.slice.targetMinutes -gt 240) {
        throw 'Active slice target must be within 1..240 minutes.'
    }
...
if ($taskExecutions.Count -gt 0) {
    ...
    $allowedTaskKinds = @('enabler', 'implementation', 'integration')
    ...
    foreach ($task in $taskExecutions) {
        ...
        if ($task.estimateMinutes -lt 10 -or $task.estimateMinutes -gt 120) {
            throw "Task $($task.id) must have a 10..120 minute pre-execution estimate."
        }
        ...
        foreach ($contractField in @('owner', 'verifier', 'contractPath', 'requirementIds', 'journeyIds', 'acceptanceCaseIds', 'evidenceManifest', 'evidenceManifestSha256', 'evidence', 'executionMode', 'schedulingReason', 'criticalPathRank', 'integrationOrder', 'estimatedParallelSavingsMinutes', 'estimatedIntegrationCostMinutes')) {
        ...
        if ([string]::IsNullOrWhiteSpace([string]$task.owner) -or
            ...
            @($task.requirementIds).Count -eq 0 -or
            @($task.journeyIds).Count -eq 0 -or
            @($task.acceptanceCaseIds).Count -eq 0) {
            throw "Task $($task.id) must have distinct owner/verifier plus requirement, journey and acceptance-case ids."
        }
        ...
        if ($task.status -in @('in_progress', 'review', 'passed', 'blocked_waiting')) {
            ...
            if (@($task.overrunHistory).Count -gt 0) {
        ...
        if ($task.status -eq 'passed') {
            ...
            $taskEvidenceEntries = @($task.evidence.PSObject.Properties | Where-Object { $null -ne $_.Value })
            if ($taskEvidenceEntries.Count -eq 0) {
                throw "Passed task $($task.id) has no affected-check evidence."
            }
            foreach ($evidenceEntry in $taskEvidenceEntries) {
                if ($evidenceEntry.Name -match '(?i)performance|benchmark|load|stress|k6|jmeter|gatling') {
                    throw "Passed functional task $($task.id) contains forbidden performance evidence."
                }
                Assert-EvidenceRecord $evidenceEntry.Value "Passed task $($task.id) $($evidenceEntry.Name)"
            }
