...
以下四个目录是可直接加载的工程 Skill，并同时提供确定性验证脚本：

| skill | 用途 | 机器验证 |
|---|---|---|
| `compile-project-delivery` | 单一需求源编译为项目阶段、原子需求、验收用例、任务 DAG 和 4 小时周期 | `skills/compile-project-delivery/scripts/validate_delivery_plan.py` |
| `build-management-ui` | 管理系统列表、工具栏、表头、右侧标签详情、仪表盘和页面密度 | `skills/build-management-ui/scripts/validate_ui_contract.py` |
| `implement-backend-module` | Base 优先的后端分层、SQL/Repository 边界、复杂度止损和受影响单测 | `skills/implement-backend-module/scripts/audit_backend_module.py` |
| `verify-by-delivery-level` | task/cycle/module/phase/project 分层验证及项目功能完成后的可选专项边界 | `skills/verify-by-delivery-level/scripts/validate_test_plan.py` |
...
- 项目规划必须先通过 `compile-project-delivery`；用户可见管理页面编码前必须通过 `build-management-ui`；后端实现必须通过 `implement-backend-module` 的 Base/SQL 边界审计；任何测试执行前必须通过 `verify-by-delivery-level`。
- 核心交付计划不包含性能、百万/千万数据或长并发任务；可选性能专项必须在项目功能完成后单独建立。
