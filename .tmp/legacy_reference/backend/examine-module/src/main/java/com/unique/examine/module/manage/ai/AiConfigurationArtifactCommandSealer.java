...
/** Proposal/session/turn/actor/draft-bound AES-GCM artifact envelope. */
@Component
public class AiConfigurationArtifactCommandSealer {
    ...
    @Autowired
    public AiConfigurationArtifactCommandSealer(SensitiveKeyProvider keys) {
        this(keys, new SecureRandom());
    }

    AiConfigurationArtifactCommandSealer(
            SensitiveKeyProvider keys, SecureRandom random) {
        this.keys = Objects.requireNonNull(keys, "keys");
        this.random = Objects.requireNonNull(random, "random");
    }

    AiConfigurationArtifactFacade.SealedCommand seal(
    ...
    private KeyMaterial material(String requestedVersion) {
        var ring = keys.current().orElseThrow(
                AiConfigurationArtifactCommandSealer::keyUnavailable);
    ...
    private static BusinessException keyUnavailable() {
        return new BusinessException(
                "AI_CONFIG_ARTIFACT_COMMAND_KEY_UNAVAILABLE",
                "The configuration artifact command key is unavailable",
                HttpStatus.SERVICE_UNAVAILABLE);
    }

    private static BusinessException invalid() {
        return AiConfigurationArtifactCommandCodec.invalid();
    }
    ...
    record Binding(
        ...
        byte[] aad() {
            ...
            append(output, "examine2:ai-config-artifact:v1");
            ...
            append(output, sessionId);
            append(output, turnId);
