...
| | 模块组 | 对外应用 |
|--|--------|----------|
| 用途 | 运行态顶部模块分组、组内模块导航、排序、可见角色、发布关系 | 外部系统 OpenAPI 接入、appKey/secret、scope、回调、限流、调用来源和审计来源 |
| 配置 | 系统后台 · 模块管理 | 系统后台或平台后台 · 对外应用 |
| 数据归属 | 不作为业务数据父级；只记录模块分组关系 | 不作为模块父级；只记录外部调用来源和授权范围 |
| 后端 | examine-module | examine-app / openapi |

**强约束：**后续文档、API 和建表不能再把系统内模块组称为“业务应用”，也不能让模块数据带 `appId` 表示父级。业务数据归属 `systemId/tenantId/moduleId`；`moduleGroupId` 只用于导航和发布关系；`externalAppId/appKey` 只用于 OpenAPI 调用来源和审计。

## 7. 核心对象关系

```
平台用户 ──扩展为──> 系统成员 ──授予──> 角色 ──控制──> 菜单/动作/字段/数据范围
系统 ──包含──> 模块组 ──组织──> 模块 ──包含──> 字段、页面 schema、动作（含导出）
业务数据 ──归属──> systemId/tenantId/moduleId
对外应用 ──调用──> OpenAPI ──记录──> externalAppId/appKey/traceId/auditLogId
模块 ──发布──> 菜单/页面 ──出现在──> 仪表盘/业务功能
流程（三类）──可绑定──> 模块/事件
```
...
- **platform_admin_root**：平台后台 + 创建车系统
