...
## 2026-06-30 framework engineering recovery lesson

| Symptom | Root cause | Rule |
|------|------|------|
| Multiple rounds of coding and recovery still did not satisfy the user's final target | Correct recovery rules existed under `docs/recovery`, but the reusable `.cursor` framework still allowed task/batch/build evidence to look like completion | Broad objectives must start with `.cursor/architecture/final-goal-framework.md`: final goal ledger -> journey gates -> task cards -> evidence -> user signoff |
| The prototype was copied as a page shell instead of becoming a functional contract | Task cards did not force every prototype area to map to role journey, frontend binding, backend behavior, data, permission, state, and script evidence | Every task card must link to a final goal or journey gate and state non-completion cases explicitly |
| Role skills were assigned, but the integrated product was still broken | Role ownership described who writes, not what final user journey must close | Journey gates own cross-role integration; backend/frontend/dba/test work is accepted only as evidence for the gate |
| The process wasted tokens in repeated rediscovery | The next executable work was not represented as a stable ledger and task cards | When the user reports final-goal failure, update the ledger, journey gate, task cards, and state before coding |
| Engineering final gate and user satisfaction were blurred | Script pass was treated as close enough to signoff | Engineering evidence and `gates.user_script_passed` are separate; only user verification/signoff can close the latter |
| Fresh script stdout and referenced result file disagreed | Some smoke scripts printed the latest result but did not write the fixed evidence path used by docs and final audits | Every smoke script referenced by an evidence document must persist the same machine-readable result it prints; stale result files are acceptance failures |
| Framework guidance still allowed drift after many fixes | Rules were written as text, but there was no audit that forced next executable tasks, evidence contracts, requirement-gap visibility, and signoff separation to remain current | Framework changes must include a machine-readable audit. For this project, run `scripts/final-goal-framework-audit.ps1`; it must fail when `nextTasks` is empty while requirements are partial, when user signoff is implied from engineering evidence, or when task cards lose cross-layer evidence fields |
| The user repeatedly had to ask why the same final goal still was not achieved | Work still drifted toward proving local slices instead of closing every role journey and requirement row; even after coding many pages, the final target was not guarded by a single always-current next-task ledger and requirement coverage gate | Each recovery slice must update the task card, next-execution ledger, requirement coverage ledger, matrix, and state before claiming progress. A slice can be accepted only as engineering evidence, and the next unfinished requirement row must remain visible until user signoff and coverage audits pass |
| Print/export looked implemented because a preview and file id existed | The export boundary was not explicit, so HTML print, browser print, and binary PDF could be blurred into one "done" claim | Print/PDF tasks must state the delivery boundary. If there is no PDF engine, evidence may prove only print-ready HTML with page setup, content type, print CSS, pagination CSS, and browser checks; binary PDF remains open unless implemented or explicitly user-approved as excluded |
| Runtime detail evidence initially passed API checks but browser checks exposed wrong UI-state assumptions | The attachment detail marker was placed on the print tab fallback, and the browser audit required a detail panel even while the edit panel was intentionally open | Browser evidence must validate the active user state: edit workflows assert edit-panel controls, detail workflows assert detail panels, and tab-specific test markers must live on the actual tab content. When a script fails, first distinguish product failure from invalid acceptance criteria, then fix both if needed |
| FRC-1C first run missed the home page and still exposed English readonly copy | The script navigated to `/systems/{id}`, which routes to runtime modules when modules exist, and readonly disabled text came from the backend page-schema service rather than the frontend fallback mapper | Browser scripts must navigate to the exact product surface being accepted, such as `/systems/{id}/dashboard` for home/dashboard. User-facing copy must be verified at its source API as well as in the frontend renderer. |
| Runtime validation looked fixed after filling one page-specific field but still failed on generated required fields | The acceptance flow filled only the new `publicName` field and ignored default required `title` and `status` fields created by module publishing | Validation evidence must prove the whole form contract, not one field. Scripts must read or know all required writable fields in the active schema, fill them before successful save, and assert errors clear only after every required field is satisfied. |
| Normal-member API/schema permissions were correct but the browser still opened a writable create form | Frontend runtime treated backend permission metadata as advisory instead of the rendering contract; the smoke clicked the create button even when the product should disable it | Runtime UI must derive create/edit buttons, form field readonly state, save/draft/submit buttons, and attachment mutation from backend action/field permission. Acceptance scripts should allow correct disabled entrances and only require readonly forms when the entrance is intentionally open. |
| Normal-member platform workbench hid the backend entry but still described platform-admin capabilities and showed create-system as a primary action | Role permission checks were applied to one button but not to the surrounding first-screen copy and action set | Role-specific journey acceptance must check visible text and primary actions against the same role boundary. Hiding an admin route is not enough if the page still explains or promotes admin-only work to a normal member. |
| R44 normal member had a visible module group configured by role ID but runtime navigation showed "no available modules" | System switch context returned role codes under `effectiveRoleIds`, while module group `visibleRoleIds` stored role IDs; admin readback and runtime filtering used different identity shapes | Permission and navigation acceptance must compare the same role identity shape end to end. If both role IDs and role codes are needed, context must carry both or filtering must explicitly normalize them before runtime use. |
| Screenshot-based confirmation still let the process drift from the user's real target | Screenshots were treated as broad confirmation even though they only show the rendered surface; they do not prove requirement coverage, workflow closure, persistence, permissions, or whether the role can finish the job | Coding must start from a requirement confirmation contract. Screenshots are visual evidence only: hierarchy, overflow, copy visibility, and rendered role state. Functional acceptance requires requirement rows, role journeys, API/readback, permission positives/negatives, states, and deployed evidence. |
| A task card can look complete while drifting away from the requirement ledger | The framework checked generic task fields and the active task card, but did not force the task's REQ rows to match the coverage ledger, gap report, and next-execution ledger | Active-task audit must link every current FRC task back to unfinished coverage rows with concrete gaps. If the row is missing, already closed, absent from the gap report, or absent from next-execution work, coding stops and the ledgers are corrected first. |
| A browser smoke failed to open the assistant even though the product entry existed | The generated Node script relied on localized Chinese button text, and the script text was corrupted by command/file encoding before execution | Browser actions must prefer stable selectors, structural position, data attributes, or route/state markers. Localized text remains a copy assertion, not the only action selector. When a browser script fails, first separate product failure from invalid acceptance criteria, then harden the script and rerun. |
| User feedback still reports the deployed product is unusable after many R-batches | Historical PASS rows and screenshots were still allowed to carry too much completion meaning, while tasks were not always selected from unfinished requirement rows and role journeys | Reopen final usable-system acceptance immediately, downgrade old journey PASS rows to engineering evidence, and upgrade the framework/audit before more coding. The next task must start from requirement row -> role journey -> user job -> generated/coded boundary -> deployed evidence. |
| Log pages looked present but operations evidence was fake | Audit log services returned static sample rows, so browser screenshots could pass while real request trace/readback did not exist | Log/operations acceptance must query persisted audit rows and prove traceId/auditLogId detail readback for success and forbidden-failure cases. Static logs are prototype artifacts, not production evidence. |
| The user had to ask for a root-cause summary after many failed continuations | The process kept moving into the next coding slice before explicitly naming why the final target had not been reached. This hid four real causes: the target was measured as slice completion instead of a usable system, tasks were split by page/API instead of user jobs, evidence proved local mechanics instead of cross-layer journeys, and user feedback was not converted into a stronger active-task gate before more coding | When user feedback says the product still cannot be used, the next turn must first summarize the failure cause, write the summary into durable project files, and then continue only from an unfinished requirement row plus role journey. No new slice can claim progress without a stated non-completion boundary and the next unfinished requirement still visible. |
| The framework was upgraded several times but still did not force the final target | V5/V6 separated screenshots and reopened final acceptance, but the active next work could still be another feature-slice aggregation rather than a human-usable role journey; ordinary engineering choices also leaked back to the user as prompts | Add a human-usable system gate: role shell separation, one primary task surface, no stacked competing views, clear copy/tips/states, data readback, permission positives/negatives, generated-vs-coded boundary, and explicit user signoff separation. After "still unusable" feedback, the next task must prefer FRC human acceptance over another feature slice, and the agent must execute inferable engineering choices without asking. |
| Release startup passed TCP Redis probes but backend health still reported Redis DOWN | The current Windows/JDK host can fail Java NIO selector initialization and can also leave Spring Redis/Lettuce unhealthy even when raw Redis TCP/AUTH/PING/SELECT works | Release acceptance must require `/api/v1/health` database/schema/redis all `UP`; raw Redis TCP reachability is only a diagnosis step. If JDK selector fails, record it, use Tomcat NIO2 for the web connector where needed, and keep the batch blocked until the application Redis client health is truly `UP`. |
| R78 passed only after repeated old smoke-script repairs | Permission/navigation and browser evidence scripts mixed role codes with role ids, used menu indexes, temp browser profiles, `--headless=new`, nested PowerShell JSON capture, and form-login waits that were brittle on the current Windows host | Acceptance scripts must use the same role identity shape as backend permission snapshots, prefer stable `data-*` selectors over indexes or localized text, keep browser profiles/scripts inside the workspace, use the proven headless/CDP pattern, parse child results from machine-readable files, and separate product failures from invalid acceptance criteria before coding product changes. |

## 2026-07-08 Platform Application Boundary Lesson

| Symptom | Root cause | Rule |
|---|---|---|
| Platform Application was implemented by reusing the system-switch panel, so the application page became another “enter system” surface | The active task gate allowed route-level repair without enforcing the `temp_flow` platform module taxonomy | Platform Flow and Platform Application are independent platform modules. Application lists/configures application/authorization objects in the current permission scope; it must not render system-switch cards or act as the system entry. System entry remains the system-switch workbench and must create `SystemSwitchContext`. |
| The product still felt confusing after many engineering PASS slices | Page hierarchy and module ownership were not treated as hard acceptance criteria before continuing feature slices | When the user reports confused hierarchy/layout/copy, stop feature-slice continuation and create an IA boundary task linked to `temp_flow`, requirement rows, journey rows, source markers, deployed browser checks, and signoff separation. |
## 2026-07-08 Clean Restart Lesson

| Symptom | Root cause | Rule |
|---|---|---|
| The user reported the project had drifted too far and felt increasingly messy even after many engineering PASS batches | The process kept extending the old recovery line instead of cutting the implementation back to retained requirements, prototypes, and flow sources | When the user calls for a clean restart, archive old implementation to `.oldbk`, preserve only the named source files, reset `.cursor/session/state.json` to clean-rebuild, and treat old code/evidence as reference-only. |
| Flow, Application, workbench, backend, todo, messages, and business modules kept blurring into route/page repairs | The architecture lacked a current hard taxonomy tied to the user's latest boundary explanation | The clean rebuild must start from the taxonomy in `.cursor/architecture/clean-rebuild.md` and `.cursor/session/rebuild/module-boundary-map.md`; coding tasks must map to these boundaries before implementation. |
| Layout repairs were handled page by page and the product still felt confusing | List/detail interaction rules were not promoted to a global acceptance contract | List surfaces use left-side tabs/categories, details use right-side tabs/work areas, and row click opens detail while preserving list context. This is a clean rebuild acceptance rule, not a local styling preference. |
