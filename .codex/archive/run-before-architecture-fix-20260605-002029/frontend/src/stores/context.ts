...
function readContext(): ContextState {
  const text = localStorage.getItem(CONTEXT_KEY);
  if (!text) return {};
  try {
    return JSON.parse(text) as ContextState;
  } catch {
    return {};
  }
}
...
export const useContextStore = defineStore('context', {
  ...
  actions: {
    ...
    async enterSystem(row: AnyRecord) {
    ...
    enrichPayload<T extends Record<string, unknown>>(payload: T) {
      return {
        ...payload,
