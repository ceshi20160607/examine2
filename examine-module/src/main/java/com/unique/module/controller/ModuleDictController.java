...
public class ModuleDictController {
    ...
    */
    ...
    public Result<List> queryFieldAdd(@RequestParam(value = "type", required = false) String type) {
        if (ObjectUtil.isNotEmpty(type)) {
            return Result.ok(moduleDictService.queryField(null));
        }
        return Result.ok(moduleDictService.queryFormField(null));
    }
    ...
    */
    ...
    public Result<List> queryFieldEdit(@PathVariable("id") @ApiParam(name = "id", value = "id") Long id,
        ...
        if (ObjectUtil.isNotEmpty(type)) {
            List<ModuleField> collect = moduleDictService.queryField(id).stream().filter(field -> !field.getFieldName().equals("ownerUserId")).collect(Collectors.toList());
            return Result.ok(collect);
        }
        return Result.ok(moduleDictService.queryFormField(id));
    }
    /**
    * 保存数据
    *
    ...
    */
    @PostMapping("/add")
    @ApiOperation("保存数据")
    public Result<Map<String, Object>> add(@RequestBody ModuleDict crmModel) {
        Map<String, Object> map = moduleDictService.addOrUpdate(crmModel, false);
    ...
    */
    @PostMapping("/update")
    @ApiOperation("修改数据")
    public Result<Map<String, Object>> update(@RequestBody ModuleDict crmModel) {
        Map<String, Object> map = moduleDictService.addOrUpdate(crmModel, false);
        return Result.ok(map);
    }
    /**
    ...
    */
    ...
    public Result<List<ModuleField>> information(@PathVariable("id") @ApiParam(name = "id", value = "id") Long id) {
        ...
        List<ModuleField> information = moduleDictService.information(id);
