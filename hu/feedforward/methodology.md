...
> **来源：** 多轮企业后台 redesign 返工 + RES-001～007 + 跨模型改稿经验提炼。
...
| 用户 | Agent |
|----|-------|
| 写或更新 **`需求.md`** | 读 harness → runbook L0～L4 → 100% P0 HTML |
| （可选）口头不满意 | learning-loop + 重跑 |

见 [contract.md](./contract.md)。
...
## 9. 关联 Skill（Harness 组件）

| 组件 | 路径 |
|------|------|
| 契约 | `feedforward/contract.md` |
| 执行 | `pipeline/runbook.md` |
| 传感器 | `feedback/checklist.md` |
| 汇报 | `feedback/response-contract.md` |
| 学习 | `feedback/learning-loop.md` |
| 配置 | `harness.yaml` |

外部：superpowers **brainstorming**（L0）、**verification-before-completion**（L4 前）。

## 10. 差距分析（摘要）

| 方式 | P0 页表 | P0 HTML | 可 Coding |
|------|---------|---------|-----------|
| 直接画原型 | ~40% | 低 | ❌ |
| 本 Harness | 100% | 100%（L4 必跑满） | ✅ |

根因：跳层、无 glossary、无 config-spec、抽样 HTML。Harness 用 feedforward + pipeline + feedback 三层约束。

## 附录

范例质量基线见 `feedforward/reference-case.md`；试跑见 `examples/nocode-platform/`。
