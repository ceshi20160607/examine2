# 参照案例：企业后台 redesign 质量基线

> **用途：** 用户只给原始需求时，Agent **必须** 对照本文件判断产出是否「够好」，而不是问用户。  
> **来源：** 多轮 redesign 返工经验提炼（平台+系统+运行态、无代码/多租户/API 同族产品）。
...
| 范例 | 路径 |
|------|------|
| 无代码平台 spec 层 | `hu/examples/nocode-platform/` |
| 页表（33 P0） | `hu/examples/nocode-platform/design/design-package.md` |
| 部分 HTML | `hu/examples/nocode-platform/design/prototypes/` |

新项目无此路径时，以 **§1～5 标准** 为准，不降低。
