...
- [ ] 已读 [reference-case.md](./knowledge/reference-case.md)（examine2 最终态标准）

---

## 2. 禁止问用户（一律 AUTO）

以下 **不得** 因「不确定」停下来问用户；按 `defaults.md` + `external-references.md` + `reference-case.md` 自行裁决，写入 `docs/decisions/resolution.md`：
...
examine2 最终 redesign 是 **质量基线**，见 [reference-case.md](./knowledge/reference-case.md)：
...
1. 对照 reference-case + checklist，差项写入 `knowledge/learnings/YYYY-MM-DD-*.md`
2. 可泛化条目追加 `prototype-pipeline-lessons.md`
3. 必要时改 `defaults.md` / `checklist.md`
...
用户只发需求内容或说「按需求做原型」。Agent 见 `需求.md` 或用户消息中的需求文本即 **自动** 读本文件并执行 [rough-to-prototype/SKILL.md](./skills/rough-to-prototype/SKILL.md)。
