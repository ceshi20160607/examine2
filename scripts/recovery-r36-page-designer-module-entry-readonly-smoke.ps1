    ...
    foreach ($systemId in @($script:CreatedSystemIds)) {
        ...
        try {
            $deleted = Invoke-Api -Method 'Delete' -Path "/api/v1/platform/systems/$systemId" -Headers $script:AdminHeaders -Body @{
                reason = 'recovery-r36 page designer cleanup'
                impactScope = 'created_by_current_script'
                idempotencyKey = "cleanup-r36-$script:Suffix-$systemId"
            }
...
$EvidenceDir = Join-Path $RepoRoot 'docs\evidence\recovery\screenshots\r36-page-designer-module-entry-readonly'
$ResultFile = Join-Path $RepoRoot 'docs\evidence\recovery\r36-page-designer-module-entry-readonly-result.json'
$SummaryFile = Join-Path $RepoRoot 'docs\evidence\recovery\r36-page-designer-module-entry-readonly-2026-06-30.md'
...
$System = Invoke-Api -Method 'Post' -Path '/api/v1/platform/systems' -Headers $script:AdminHeaders -Body @{
    systemName = "R36 Page Designer $script:Suffix"
    systemCode = "r36_page_$script:Suffix"
    tenantMode = 1
...
$Module = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/modules" -Headers $script:AdminHeaders -Body @{
    ...
    moduleCode = "r36_request_$script:Suffix"
    name = "R36 Request $script:Suffix"
    status = 1
    description = 'Recovery R36 page designer module entry proof'
}
...
$null = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/modules/$ModuleId/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-r36 module publish'
    idempotencyKey = "module-publish-r36-$script:Suffix"
}
...
$null = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/runtime/modules/$ModuleId/records" -Headers $script:AdminHeaders -Body @{
    fieldValues = @{
        publicName = "R36 Visible Record $script:Suffix"
        secretNote = "R36 Hidden Secret $script:Suffix"
        amountValue = 3600
    }
    childRows = @{}
    attachmentIds = @()
    sourceType = 'R36_PAGE_DESIGNER_READONLY'
}

$PagePayload = @{
    pageCode = 'main'
    pageName = "R36 Runtime Main $script:Suffix"
    ...
    visibleRoleIds = @()
    changeReason = 'recovery-r36 module workflow page designer'
}
...
$PublishedPage = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/modules/$ModuleId/pages/main/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-r36 page publish'
    idempotencyKey = "page-publish-r36-$script:Suffix"
}
...
$NormalLoginName = "r36_page_member_$script:Suffix"
$NormalRegister = Invoke-Api -Method 'Post' -Path '/api/v1/auth/register-with-system' -Body @{
    ...
    email = "r36_page_member_$script:Suffix@example.com"
    password = $Password
    systemName = "R36 Owned $script:Suffix"
    systemCode = "r36_owned_$script:Suffix"
    tenantMode = 1
    templateCode = 'blank'
}
...
$NormalMember = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/members" -Headers $script:AdminHeaders -Body @{
    deptId = $null
    memberName = "R36 Runtime Member $script:Suffix"
    employeeNo = "R36M$((Get-Date).ToString('HHmmssfff'))"
    mobile = "18$((Get-Date).ToString('HHmmssfff'))"
    email = "r36_runtime_member_$script:Suffix@example.com"
    status = 1
...
$null = Invoke-Api -Method 'Post' -Path '/api/v1/platform/system-switch' -Headers $NormalHeaders -Body @{
    systemId = $SystemId
    tenantId = $TenantId
    reason = 'recovery-r36 normal member readonly browser audit'
}
...
$ForbiddenCreate = Invoke-ExpectedForbidden -Method 'Post' -Path "/api/v1/systems/$SystemId/runtime/modules/$ModuleId/records" -Headers $NormalHeaders -Body @{
    fieldValues = @{
        publicName = "R36 forbidden write $script:Suffix"
        secretNote = 'should not write'
        amountValue = 99
    }
    childRows = @{}
    attachmentIds = @()
    sourceType = 'R36_FORBIDDEN_CREATE'
}

$chromePath = Find-Chrome
$debugPort = Get-FreeTcpPort
$script:ChromeProfileDir = Join-Path $env:TEMP "unexamine-r36-chrome-$script:Suffix"
...
$nodeScript = Join-Path $env:TEMP "unexamine-r36-page-designer-$script:Suffix.js"
@'
const fs = require('fs');
const path = require('path');

const baseUrl = process.env.R36_BASE_URL;
const outDir = process.env.R36_EVIDENCE_DIR;
const port = process.env.R36_CDP_PORT;
const systemId = process.env.R36_SYSTEM_ID;
const moduleId = process.env.R36_MODULE_ID;
const moduleName = process.env.R36_MODULE_NAME;
const pageName = process.env.R36_PAGE_NAME;
...
async function run() {
  ...
  const roles = {
    admin: {
      accessToken: process.env.R36_ADMIN_TOKEN,
      refreshToken: process.env.R36_ADMIN_REFRESH,
      accountId: process.env.R36_ADMIN_ACCOUNT_ID || 'admin',
    },
    normal: {
      accessToken: process.env.R36_NORMAL_TOKEN,
      refreshToken: process.env.R36_NORMAL_REFRESH,
      accountId: process.env.R36_NORMAL_ACCOUNT_ID || 'normal',
    },
  };
...
$env:R36_BASE_URL = $BaseUrl
$env:R36_EVIDENCE_DIR = $EvidenceDir
$env:R36_CDP_PORT = [string]$debugPort
$env:R36_SYSTEM_ID = $SystemId
$env:R36_MODULE_ID = $ModuleId
$env:R36_MODULE_NAME = $Module.name
$env:R36_PAGE_NAME = $PagePayload.pageName
$env:R36_ADMIN_TOKEN = $AdminLogin.accessToken
$env:R36_ADMIN_REFRESH = $AdminLogin.refreshToken
$env:R36_ADMIN_ACCOUNT_ID = $AdminLogin.profile.accountId
$env:R36_NORMAL_TOKEN = $NormalLogin.accessToken
$env:R36_NORMAL_REFRESH = $NormalLogin.refreshToken
$env:R36_NORMAL_ACCOUNT_ID = $NormalLogin.profile.accountId

$nodeOutput = & node $nodeScript
$nodeExit = $LASTEXITCODE
if ($nodeExit -ne 0) {
    throw "R36 browser audit failed with exit code $nodeExit. Output: $nodeOutput"
}
...
Assert-True -Condition ($DashboardResult.hasHomeConfigTitle -eq $true -and $DashboardResult.hasModulePageDesigner -eq $false -and $DashboardResult.hasHomeAndPageDesignerTitle -eq $false) `
    -Message 'Dashboard config still mixes module page designer wording into the home configuration surface.'
Assert-True -Condition (@($NormalReadonlyResult.fields).Count -ge 2) -Message 'Normal runtime readonly form did not render expected readable fields.'
...
Assert-True -Condition (@($BrowserAudit.results | Where-Object { $_.overflowX -gt 0 }).Count -eq 0) -Message 'One or more FRC-1B browser routes overflow horizontally.'
...
$Result = [ordered]@{
    status = 'PASS'
    task = 'REC-P0-036'
    frc = 'FRC-1B Page designer module-entry and runtime readonly proof'
    baseUrl = $BaseUrl
    systemId = $SystemId
    moduleId = $ModuleId
    pageCode = 'main'
...
- Module management shows `模块页面设计器`, selected module, selected page, and schema preview together.
