    ...
    foreach ($systemId in @($script:TargetSystemId, $script:NormalOwnedSystemId)) {
        ...
        try {
            $deleted = Invoke-Api -Method 'Delete' -Path "/api/v1/platform/systems/$systemId" -Headers $script:AdminHeaders -Body @{
                reason = 'recovery-R18 normal member real-login runtime cleanup'
                impactScope = 'created_by_current_script'
                idempotencyKey = "cleanup-R18-$script:Suffix-$systemId"
            }
...
$EvidenceDir = Join-Path (Get-Location) 'docs\evidence\recovery\screenshots\r18-runtime-mobile-action-containment'
...
$Target = Invoke-Api -Method 'Post' -Path '/api/v1/platform/systems' -Headers $script:AdminHeaders -Body @{
    systemName = "R18 Runtime Target $Suffix"
    systemCode = "r18_target_$Suffix"
    tenantMode = 1
...
$null = Invoke-Api -Method 'Post' -Path '/api/v1/platform/system-switch' -Headers $script:AdminHeaders -Body @{
    ...
    reason = 'recovery-R18 target setup'
}

$NormalLoginName = "r18_member_$Suffix"
$NormalRegister = Invoke-Api -Method 'Post' -Path '/api/v1/auth/register-with-system' -Body @{
    ...
    email = "R18_member_$Suffix@example.com"
    password = $Password
    systemName = "R18 Owned $Suffix"
    systemCode = "r18_owned_$Suffix"
    tenantMode = 1
    templateCode = 'blank'
}
...
$RuntimeRole = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:TargetSystemId/roles" -Headers $script:AdminHeaders -Body @{
    roleName = "R18 Runtime Member $Suffix"
    roleCode = "R18_RUNTIME_MEMBER_$Suffix"
    roleType = 'CUSTOM'
    status = 1
    description = 'Recovery R18 runtime-only member role'
}

$Group = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:TargetSystemId/module-groups" -Headers $script:AdminHeaders -Body @{
    name = "R18 Business Group $Suffix"
    sort = 10
...
$Module = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:TargetSystemId/modules" -Headers $script:AdminHeaders -Body @{
    ...
    moduleCode = "r18_ticket_$Suffix"
    name = "R18 Ticket Ledger $Suffix"
    status = 1
    description = 'Recovery R18 normal member runtime module'
}
...
$null = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:TargetSystemId/modules/$ModuleId/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-R18 module publish'
    idempotencyKey = "module-publish-R18-$Suffix"
}

$NormalMember = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:TargetSystemId/members" -Headers $script:AdminHeaders -Body @{
    deptId = $null
    memberName = "R18 Runtime Member $Suffix"
    employeeNo = "R18NM$((Get-Date).ToString('HHmmssfff'))"
    mobile = "18$((Get-Date).ToString('HHmmssfff'))"
    email = "R18_runtime_member_$Suffix@example.com"
    status = 1
...
$NormalSwitch = Invoke-Api -Method 'Post' -Path '/api/v1/platform/system-switch' -Headers $NormalHeaders -Body @{
    systemId = $script:TargetSystemId
    tenantId = $TargetTenantId
    reason = 'recovery-R18 normal member switch precheck'
}
Assert-True -Condition (@($NormalSwitch.effectiveRoleIds) -contains "R18_RUNTIME_MEMBER_$Suffix" -or @($NormalSwitch.effectiveRoleIds) -contains 'SYSTEM_MEMBER') -Message "Normal member switch did not include runtime/member role: $($NormalSwitch.effectiveRoleIds -join ',')"
...
$RecordValue = "R18 Browser Ticket $Suffix"

$chromePath = Find-Chrome
$debugPort = Get-FreeTcpPort
$script:ChromeProfileDir = Join-Path $env:TEMP "unexamine-R18-chrome-$Suffix"
...
$nodeScript = Join-Path $env:TEMP "unexamine-r18-mobile-runtime-$Suffix.js"
@'
const fs = require('fs');
const path = require('path');

const baseUrl = process.env.R18_BASE_URL;
const systemId = process.env.R18_SYSTEM_ID;
const accountName = process.env.R18_ACCOUNT;
const password = process.env.R18_PASSWORD;
const recordValue = process.env.R18_RECORD_VALUE;
const outDir = process.env.R18_EVIDENCE_DIR;
const port = process.env.R18_CDP_PORT;
...
async function captureAndMeasure(client, key) {
  ...
  return evaluateJson(client, `
    JSON.stringify((() => {
      ...
      const outsideHorizontal = (item) => item.left < -1 || item.right > vw + 1 || item.width > vw + 1;
      const outsideFirstViewport = (item) => outsideHorizontal(item) || item.top < -1 || item.bottom > vh + 1;
      const runtimeActionButtons = Array.from(document.querySelectorAll('.runtime-page-head > .inline-actions button')).map(rectOf);
      const filterControls = Array.from(document.querySelectorAll('.runtime-filter-panel input, .runtime-filter-panel select, .runtime-filter-panel button')).map(rectOf);
      const runtimePanels = Array.from(document.querySelectorAll('.runtime-module-sidebar, .runtime-main, .runtime-side-panel, .runtime-filter-panel, .runtime-batch-bar, .runtime-table-shell')).map(rectOf);
      const actionOverflow = runtimeActionButtons.filter(outsideFirstViewport);
      const filterOverflow = filterControls.filter(outsideHorizontal);
      const panelOverflow = runtimePanels.filter(outsideHorizontal);
      const blockers = [];
      ...
      if (key.includes('created-record') && !runtimeTable) blockers.push('runtime table missing');
      if (key.includes('runtime') && runtimeActionButtons.length < 4) blockers.push('runtime primary actions missing');
      if (key.includes('runtime') && actionOverflow.length > 0) blockers.push('runtime primary action outside first viewport');
      if (key.includes('runtime') && filterOverflow.length > 0) blockers.push('runtime filter control horizontal overflow');
      if (key.includes('runtime') && panelOverflow.length > 0) blockers.push('runtime panel horizontal overflow');
      if (key.includes('created-record') && !text.includes(${JSON.stringify(recordValue)})) blockers.push('created record value missing');
...
async function run() {
  ...
  `, 15000);
  const results = [];
  results.push(await captureAndMeasure(client, 'mobile-runtime-loaded'));
  await clickRuntimeCreate(client);
  results.push(await captureAndMeasure(client, 'mobile-runtime-create-panel'));
  ...
  `, 8000);
  results.push(await captureAndMeasure(client, 'mobile-admin-denied'));
  ...
  fs.writeFileSync(path.join(outDir, 'runtime-mobile-action-containment-browser.json'), JSON.stringify(output, null, 2));
  if (failures.length > 0) {
        throw new Error(`Runtime mobile containment audit failed: ${failures.map((item) => `${item.key}:${item.blockers.join('|')}`).join('; ')}`);
  }
...
$env:R18_BASE_URL = $BaseUrl
$env:R18_SYSTEM_ID = $script:TargetSystemId
$env:R18_ACCOUNT = $NormalLoginName
$env:R18_PASSWORD = $Password
$env:R18_RECORD_VALUE = $RecordValue
$env:R18_EVIDENCE_DIR = $EvidenceDir
$env:R18_CDP_PORT = [string]$debugPort

try {
    $nodeOutput = & $nodeExe $nodeScript
    if ($LASTEXITCODE -ne 0) {
        throw "Node runtime mobile containment audit exited with code $LASTEXITCODE"
    }
} finally {
    if ($script:ChromeProcess -and -not $script:ChromeProcess.HasExited) {
        Stop-Process -Id $script:ChromeProcess.Id -Force -ErrorAction SilentlyContinue
...
$resultPath = Join-Path $EvidenceDir 'runtime-mobile-action-containment-browser.json'
$browserSummaryRaw = & $nodeExe -e @'
const fs = require('fs');
const path = process.argv[1];
const result = JSON.parse(fs.readFileSync(path, 'utf8'));
const maxOverflowX = Math.max(...result.results.map((item) => Number(item.overflowX || 0)));
console.log(JSON.stringify({
  status: result.status,
  resultCount: result.results.length,
  failureCount: result.failures.length,
  failures: result.failures.map((item) => ({ key: item.key, blockers: item.blockers })),
  maxOverflowX,
  screenshots: result.results.map((item) => item.screenshot),
}));
'@ $resultPath
if ($LASTEXITCODE -ne 0) {
    throw "Node failed to parse browser result: $resultPath"
}
$browserResult = $browserSummaryRaw | ConvertFrom-Json
Assert-True -Condition ($browserResult.status -eq 'PASS' -and [int]$browserResult.failureCount -eq 0) -Message "R18 browser audit failed: $browserSummaryRaw"
...
$result = [ordered]@{
    ...
    screenshotCount = [int]$browserResult.resultCount
    maxOverflowX = [int]$browserResult.maxOverflowX
    cleanup = $script:CleanupResult
}

$resultJson = $result | ConvertTo-Json -Depth 40
$resultFile = Join-Path (Get-Location) 'docs\evidence\recovery\r18-runtime-mobile-action-containment-result.json'
$resultJson | Set-Content -LiteralPath $resultFile -Encoding UTF8

$summaryPath = Join-Path (Get-Location) 'docs\evidence\recovery\r18-runtime-mobile-action-containment-2026-06-29.md'
@"
...
- Frontend create form saved record value $RecordValue and runtime list/detail read it back.
