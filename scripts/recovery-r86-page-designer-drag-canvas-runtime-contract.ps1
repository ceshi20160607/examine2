    ...
    foreach ($systemId in @($script:CreatedSystemIds)) {
        ...
        try {
            $deleted = Invoke-Api -Method 'Delete' -Path "/api/v1/platform/systems/$systemId" -Headers $script:AdminHeaders -Body @{
                reason = 'recovery-R86 page designer drag canvas runtime contract cleanup'
                impactScope = 'created_by_current_script'
                idempotencyKey = "cleanup-R86-$script:Suffix-$systemId"
            }
...
$EvidenceDir = Join-Path $RepoRoot 'docs\evidence\recovery\screenshots\r86-page-designer-drag-canvas-runtime-contract'
$ResultFile = Join-Path $RepoRoot 'docs\evidence\recovery\r86-page-designer-drag-canvas-runtime-contract-result.json'
$SummaryFile = Join-Path $RepoRoot 'docs\evidence\recovery\r86-page-designer-drag-canvas-runtime-contract-2026-07-07.md'
...
$System = Invoke-Api -Method 'Post' -Path '/api/v1/platform/systems' -Headers $script:AdminHeaders -Body @{
    systemName = "R86 Page Designer $script:Suffix"
    systemCode = "r86_page_$script:Suffix"
    tenantMode = 1
...
$null = Invoke-Api -Method 'Post' -Path '/api/v1/platform/system-switch' -Headers $script:AdminHeaders -Body @{
    systemId = $SystemId
    tenantId = $TenantId
    reason = 'recovery-R86 admin setup'
}
...
Assert-True -Condition ($null -ne $SystemSuperAdminRole -and -not [string]::IsNullOrWhiteSpace([string]$SystemSuperAdminRole.roleId)) `
    -Message 'System super admin role id was not found for R86 runtime visibility setup.'

$RuntimeRole = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/roles" -Headers $script:AdminHeaders -Body @{
    roleName = "R86 Readonly Member $script:Suffix"
    roleCode = "R86_READONLY_MEMBER_$script:Suffix"
    roleType = 'CUSTOM'
    status = 1
    description = 'Recovery R86 readonly runtime role'
}

$Group = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/module-groups" -Headers $script:AdminHeaders -Body @{
    name = "R86 Operations $script:Suffix"
    sort = 10
...
$Module = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/modules" -Headers $script:AdminHeaders -Body @{
    ...
    moduleCode = "r86_request_$script:Suffix"
    name = "R86 Request $script:Suffix"
    status = 1
    description = 'Recovery R86 page designer module entry proof'
}
...
$null = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/modules/$ModuleId/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-R86 module publish'
    idempotencyKey = "module-publish-R86-$script:Suffix"
}
...
$GroupPublish = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/module-groups/$($Group.groupId)/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-R86 group publish'
    idempotencyKey = "group-publish-R86-$script:Suffix"
}
...
$null = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/runtime/modules/$ModuleId/records" -Headers $script:AdminHeaders -Body @{
    fieldValues = @{
        publicName = "R86 Visible Record $script:Suffix"
        secretNote = "R86 Hidden Secret $script:Suffix"
        amountValue = 3600
    }
    childRows = @{}
    attachmentIds = @()
    sourceType = 'R86_PAGE_DESIGNER_READONLY'
}

$PagePayload = @{
    pageCode = 'main'
    pageName = "R86 Runtime Main $script:Suffix"
    pageType = 'MODULE_LIST'
    route = "/systems/$SystemId/modules/$ModuleId"
    layoutMode = 'left-list-right-detail'
    components = @(
        @{ componentCode = 'toolbar'; componentType = 'SHORTCUT'; title = 'Page Actions'; dataSource = 'MODULE_ACTIONS'; sort = 10; visible = $true; props = @{ width = 'half'; placement = 'top'; dragCanvas = 'page-designer' } },
        @{ componentCode = 'list'; componentType = 'LIST'; title = 'Runtime Records'; dataSource = 'RUNTIME_RECORDS'; sort = 20; visible = $true; props = @{ width = 'full'; placement = 'main'; dragCanvas = 'page-designer' } },
        @{ componentCode = 'chart'; componentType = 'CHART'; title = 'Amount Chart'; dataSource = 'RUNTIME_RECORDS'; boundFieldCode = 'amountValue'; sort = 30; visible = $true; props = @{ width = 'third'; placement = 'right'; dragCanvas = 'page-designer' } },
        @{ componentCode = 'form'; componentType = 'FORM'; title = 'Readonly Form'; dataSource = 'MODULE_FIELDS'; boundFieldCode = 'publicName'; sort = 40; visible = $true; props = @{ width = 'half'; placement = 'main'; dragCanvas = 'page-designer' } },
        @{ componentCode = 'secret'; componentType = 'DETAIL'; title = 'Hidden Secret'; dataSource = 'RECORD_DETAIL'; boundFieldCode = 'secretNote'; sort = 50; visible = $true; props = @{ width = 'third'; placement = 'right'; dragCanvas = 'page-designer' } }
    )
    visibleRoleIds = @()
    changeReason = 'recovery-R86 module workflow page designer'
}
...
$PublishedPage = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/modules/$ModuleId/pages/main/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-R86 page publish'
    idempotencyKey = "page-publish-R86-$script:Suffix"
}
...
$NormalLoginName = "r86_page_member_$script:Suffix"
$NormalRegister = Invoke-Api -Method 'Post' -Path '/api/v1/auth/register-with-system' -Body @{
    ...
    email = "r86_page_member_$script:Suffix@example.com"
    password = $Password
    systemName = "R86 Owned $script:Suffix"
    systemCode = "r86_owned_$script:Suffix"
    tenantMode = 1
    templateCode = 'blank'
}
...
$NormalMember = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$SystemId/members" -Headers $script:AdminHeaders -Body @{
    deptId = $null
    memberName = "R86 Runtime Member $script:Suffix"
    employeeNo = "R86M$((Get-Date).ToString('HHmmssfff'))"
    mobile = "18$((Get-Date).ToString('HHmmssfff'))"
    email = "R86_runtime_member_$script:Suffix@example.com"
    status = 1
...
$null = Invoke-Api -Method 'Post' -Path '/api/v1/platform/system-switch' -Headers $NormalHeaders -Body @{
    systemId = $SystemId
    tenantId = $TenantId
    reason = 'recovery-R86 normal member readonly browser audit'
}
...
$AdminSchemaLayoutComponents = @($AdminSchema.components | Where-Object { $_.props -and $_.props.dragCanvas -eq 'page-designer' -and -not [string]::IsNullOrWhiteSpace([string]$_.props.width) -and -not [string]::IsNullOrWhiteSpace([string]$_.props.placement) })
$NormalSchemaLayoutComponents = @($NormalSchema.components | Where-Object { $_.props -and $_.props.dragCanvas -eq 'page-designer' -and -not [string]::IsNullOrWhiteSpace([string]$_.props.width) -and -not [string]::IsNullOrWhiteSpace([string]$_.props.placement) })
Assert-True -Condition ($AdminSchemaLayoutComponents.Count -ge 5) -Message 'Admin published schema did not preserve component width, placement, and dragCanvas props.'
Assert-True -Condition ($NormalSchemaLayoutComponents.Count -ge 4) -Message 'Normal runtime schema did not preserve visible component width, placement, and dragCanvas props.'
Assert-True -Condition (@($NormalSchema.components | Where-Object { $_.componentCode -eq 'chart' -and $_.props.width -eq 'third' -and $_.props.placement -eq 'right' }).Count -eq 1) -Message 'Normal runtime schema did not expose chart component layout props.'
...
$ForbiddenCreate = Invoke-ExpectedForbidden -Method 'Post' -Path "/api/v1/systems/$SystemId/runtime/modules/$ModuleId/records" -Headers $NormalHeaders -Body @{
    fieldValues = @{
        publicName = "R86 forbidden write $script:Suffix"
        secretNote = 'should not write'
        amountValue = 99
    }
    childRows = @{}
    attachmentIds = @()
    sourceType = 'R86_FORBIDDEN_CREATE'
}
...
$script:ChromeProfileDir = Join-Path $BrowserWorkDir "unexamine-R86-chrome-$script:Suffix"
...
$nodeScript = Join-Path $BrowserWorkDir "unexamine-R86-page-designer-$script:Suffix.js"
@'
const fs = require('fs');
...
async function run() {
  ...
  await screenshot(client, 'desktop-admin-schema-preview.png');
  result.results.push(await inspect(client, 'desktop-admin-schema-preview'));
...
$env:R86_BASE_URL = $BaseUrl
$env:R86_EVIDENCE_DIR = $EvidenceDir
$env:R86_CDP_PORT = [string]$debugPort
$env:R86_SYSTEM_ID = $SystemId
$env:R86_MODULE_ID = $ModuleId
$env:R86_MODULE_NAME = $Module.name
$env:R86_PAGE_NAME = $PagePayload.pageName
$env:R86_ADMIN_TOKEN = $AdminLogin.accessToken
$env:R86_ADMIN_REFRESH = $AdminLogin.refreshToken
$env:R86_ADMIN_ACCOUNT_ID = $AdminLogin.profile.accountId
$env:R86_NORMAL_TOKEN = $NormalLogin.accessToken
$env:R86_NORMAL_REFRESH = $NormalLogin.refreshToken
$env:R86_NORMAL_ACCOUNT_ID = $NormalLogin.profile.accountId

$nodeOutput = & node $nodeScript 2>&1
$nodeExit = $LASTEXITCODE
if ($nodeExit -ne 0) {
    throw "R86 browser audit failed with exit code $nodeExit. Output: $nodeOutput"
}
...
$SourceSystemAdminText = Get-Content -LiteralPath (Join-Path $RepoRoot 'frontend\src\features\system-admin\systemAdmin.ts') -Raw -Encoding UTF8
$SourceSchemaRendererText = Get-Content -LiteralPath (Join-Path $RepoRoot 'frontend\src\features\schema\schemaRenderer.ts') -Raw -Encoding UTF8
$ReleaseFrontendDir = Join-Path $RepoRoot 'release\unexamine-0.0.1-SNAPSHOT\frontend\assets'
$ReleaseAssetText = if (Test-Path -LiteralPath $ReleaseFrontendDir) { (Get-ChildItem -LiteralPath $ReleaseFrontendDir -Filter '*.js' | ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw -Encoding UTF8 }) -join "`n" } else { '' }
$SourceMarkers = [ordered]@{ dragCanvas = $SourceSystemAdminText.Contains('pageComponentDragCanvas'); draggableRows = $SourceSystemAdminText.Contains('componentDraggable'); width = $SourceSystemAdminText.Contains('componentWidth'); placement = $SourceSystemAdminText.Contains('componentPlacement'); schemaStrip = $SourceSchemaRendererText.Contains('pageSchemaComponentStrip'); schemaWidth = $SourceSchemaRendererText.Contains('schemaComponentWidth'); schemaPlacement = $SourceSchemaRendererText.Contains('schemaComponentPlacement') }
$ReleaseMarkers = [ordered]@{ dragCanvas = $ReleaseAssetText.Contains('pageComponentDragCanvas') -or $ReleaseAssetText.Contains('data-page-component-drag-canvas'); draggableRows = $ReleaseAssetText.Contains('componentDraggable') -or $ReleaseAssetText.Contains('data-component-draggable'); schemaStrip = $ReleaseAssetText.Contains('pageSchemaComponentStrip') -or $ReleaseAssetText.Contains('data-page-schema-component-strip'); schemaWidth = $ReleaseAssetText.Contains('schemaComponentWidth') -or $ReleaseAssetText.Contains('data-schema-component-width'); schemaPlacement = $ReleaseAssetText.Contains('schemaComponentPlacement') -or $ReleaseAssetText.Contains('data-schema-component-placement') }
Assert-True -Condition (@($SourceMarkers.Values | Where-Object { $_ -eq $false }).Count -eq 0) -Message "R86 source marker check failed: $($SourceMarkers | ConvertTo-Json -Compress)"
Assert-True -Condition (-not [string]::IsNullOrWhiteSpace($ReleaseAssetText) -and @($ReleaseMarkers.Values | Where-Object { $_ -eq $false }).Count -eq 0) -Message "R86 deployed asset marker check failed: $($ReleaseMarkers | ConvertTo-Json -Compress)"
...
$AdminSchemaPreviewResult = @($BrowserAudit.results | Where-Object { $_.key -eq 'desktop-admin-schema-preview' }) | Select-Object -First 1
$DashboardResult = @($BrowserAudit.results | Where-Object { $_.key -eq 'desktop-admin-dashboard-config' }) | Select-Object -First 1
$SchemaStripBrowserResults = @($BrowserAudit.results | Where-Object { $_.hasSchemaComponentStrip -eq $true -and $_.schemaComponentCount -ge 4 })
$AdminModuleWidthValues = @($AdminModuleResult.componentWidthValues)
$AdminModulePlacementValues = @($AdminModuleResult.componentPlacementValues)
$AdminModuleHasAllWidths = (@('full', 'half', 'third') | Where-Object { $AdminModuleWidthValues -contains $_ }).Count -eq 3
$AdminModuleHasKeyPlacements = (@('main', 'right') | Where-Object { $AdminModulePlacementValues -contains $_ }).Count -eq 2

Assert-True -Condition ($AdminModuleResult.hasModulePageDesigner -eq $true -and $AdminModuleResult.hasModuleName -eq $true -and $AdminModuleResult.hasPageName -eq $true) `
    -Message 'Admin module workflow did not show page designer, selected module, and selected page together.'
Assert-True -Condition ($AdminSchemaPreviewResult.hasSchemaPreviewPanel -eq $true -and $AdminSchemaPreviewResult.hasSchemaComponentStrip -eq $true -and $AdminSchemaPreviewResult.schemaComponentCount -ge 5) `
    -Message 'Admin schema preview did not render the runtime component strip with layout markers.'
...
Assert-True -Condition ($AdminModuleResult.hasPageComponentDragCanvas -eq $true -and $AdminModuleResult.draggableComponentRowCount -ge 5) `
    -Message 'Page designer drag canvas or draggable component rows did not render.'
Assert-True -Condition ($AdminModuleHasAllWidths -eq $true -and $AdminModuleHasKeyPlacements -eq $true -and $AdminModuleResult.mobilePreviewComponentCount -ge 5) `
    -Message "Page designer width/placement/mobile-preview markers were incomplete: widths=$($AdminModuleWidthValues -join ',') placements=$($AdminModulePlacementValues -join ',')."
Assert-True -Condition (@($SchemaStripBrowserResults).Count -ge 1) `
    -Message 'Runtime schema component strip did not render with layout markers in any audited browser route.'
...
$Result = [ordered]@{
    status = 'PASS'
    task = 'REC-P0-086'
    frc = 'FRC-1 Page designer drag canvas and runtime component contract closure'
    baseUrl = $BaseUrl
    systemId = $SystemId
    moduleId = $ModuleId
    pageCode = 'main'
    ...
    componentWorkbench = [ordered]@{
        ...
        dragCanvas = $AdminModuleResult.hasPageComponentDragCanvas
        draggableRowCount = $AdminModuleResult.draggableComponentRowCount
        widthValues = @($AdminModuleResult.componentWidthValues)
        placementValues = @($AdminModuleResult.componentPlacementValues)
        mobilePreviewComponentCount = $AdminModuleResult.mobilePreviewComponentCount
    }
    schemaContract = [ordered]@{
        adminLayoutComponentCount = $AdminSchemaLayoutComponents.Count
        normalLayoutComponentCount = $NormalSchemaLayoutComponents.Count
        schemaPreviewStrip = $AdminSchemaPreviewResult.hasSchemaComponentStrip
        schemaPreviewComponentCount = $AdminSchemaPreviewResult.schemaComponentCount
    }
    ...
    schemaComponentStripBrowserResultCount = @($SchemaStripBrowserResults).Count
