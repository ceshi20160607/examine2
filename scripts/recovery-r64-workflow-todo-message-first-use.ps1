...
function Start-BrowserAudit {
    ...
    $script:ChromeProfileDir = Join-Path $script:BrowserWorkDir "unexamine-r64-chrome-$script:Suffix"
    ...
    foreach ($systemId in @($script:CreatedSystemIds)) {
        ...
        try {
            $deleted = Invoke-Api -Method 'Delete' -Path "/api/v1/platform/systems/$systemId" -Headers $script:AdminHeaders -Body @{
                reason = 'recovery-r64 workflow todo message cleanup'
                impactScope = 'created_by_current_script'
                idempotencyKey = "cleanup-r64-$script:Suffix-$systemId"
            }
...
trap {
    ...
    $failure = [ordered]@{
        status = 'FAIL'
        productStatus = 'R64_WORKFLOW_TODO_MESSAGE_FIRST_USE_FAILED'
...
$script:ChromeProcess = $null
$script:ChromeProfileDir = $null
$script:BrowserWorkDir = $null
$script:CleanupResult = @('SKIPPED')

$EvidenceDir = Join-Path $RepoRoot 'docs\evidence\recovery\screenshots\r64-workflow-todo-message-first-use'
$script:ResultFile = Join-Path $RepoRoot 'docs\evidence\recovery\r64-workflow-todo-message-first-use-result.json'
$SummaryFile = Join-Path $RepoRoot 'docs\evidence\recovery\r64-workflow-todo-message-first-use-2026-07-02.md'
...
Assert-True -Condition ($Setup.status -eq 'PASS') -Message "R64 setup did not pass: $SetupJson"
...
foreach ($item in @(
    @{ headers = $script:AdminHeaders; reason = 'recovery-r64 admin switch' },
    @{ headers = $RequesterHeaders; reason = 'recovery-r64 requester switch' },
    @{ headers = $ApproverHeaders; reason = 'recovery-r64 approver switch' }
)) {
...
$PublishCheck = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$($Setup.targetSystemId)/flows/$($Setup.flowId)/publish-check" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-r64 flow publish check before browser first-use'
    idempotencyKey = "flow-publish-check-r64-$($Setup.suffix)"
}
Assert-True -Condition ($PublishCheck.passed -eq $true) -Message 'R64 flow publish-check did not pass.'
$Simulation = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$($Setup.targetSystemId)/flows/$($Setup.flowId)/simulate" -Headers $script:AdminHeaders -Body @{
    ...
    idempotencyKey = "flow-sim-r64-$($Setup.suffix)"
}
Assert-True -Condition ($Simulation.passed -eq $true -and $Simulation.runtimeInstanceCreated -eq $false) `
    -Message "R64 flow simulation failed or created runtime instance: $($Simulation | ConvertTo-Json -Depth 40 -Compress)"

$debugPort = Start-BrowserAudit
$NodeScript = Join-Path $script:BrowserWorkDir "unexamine-r64-workflow-todo-message-$($Setup.suffix).js"
$NodeCode = @'
const fs = require('fs');
const path = require('path');

const baseUrl = process.env.R64_BASE_URL;
const port = process.env.R64_CDP_PORT;
const outDir = process.env.R64_EVIDENCE_DIR;
const phase = process.env.R64_PHASE;
const systemId = process.env.R64_SYSTEM_ID;
const recordId = process.env.R64_RECORD_ID;
const todoId = process.env.R64_TODO_ID || '';
const messageId = process.env.R64_MESSAGE_ID || '';
const roles = {
  requester: { accountId: process.env.R64_REQUESTER_ACCOUNT_ID, accessToken: process.env.R64_REQUESTER_TOKEN, refreshToken: process.env.R64_REQUESTER_REFRESH || '' },
  approver: { accountId: process.env.R64_APPROVER_ACCOUNT_ID, accessToken: process.env.R64_APPROVER_TOKEN, refreshToken: process.env.R64_APPROVER_REFRESH || '' },
  admin: { accountId: process.env.R64_ADMIN_ACCOUNT_ID, accessToken: process.env.R64_ADMIN_TOKEN, refreshToken: process.env.R64_ADMIN_REFRESH || '' },
};
...
async function screenshot(client, key) {
  const raw = await evaluate(client, `(() => {
    ...
    const blockerTexts = ['undefined', 'null', 'NaN', '加载失败', '操作失败'];
...
async function submitFromRuntime(client) {
  ...
  await navigate(client, `${baseUrl}/?r64=${Date.now()}#/systems/${systemId}/modules`);
...
async function approveFromTodo(client) {
  ...
  await setViewport(client, { width: 1440, height: 920, mobile: false });
  await navigate(client, `${baseUrl}/?r64=${Date.now()}#/systems/${systemId}/messages`);
  ...
  await evaluate(client, `(() => {
    ...
    input.value = 'Approved through R64 browser todo';
...
function Invoke-BrowserPhase {
    ...
    $env:R64_BASE_URL = $BaseUrl
    $env:R64_CDP_PORT = [string]$debugPort
    $env:R64_EVIDENCE_DIR = $EvidenceDir
    $env:R64_PHASE = $Phase
    $env:R64_SYSTEM_ID = [string]$Setup.targetSystemId
    $env:R64_RECORD_ID = [string]$Setup.recordId
    $env:R64_TODO_ID = [string]$TodoId
    $env:R64_MESSAGE_ID = [string]$MessageId
    $env:R64_ADMIN_TOKEN = [string]$AdminLogin.accessToken
    $env:R64_ADMIN_REFRESH = [string]$AdminLogin.refreshToken
    $env:R64_ADMIN_ACCOUNT_ID = [string]$AdminLogin.profile.accountId
    $env:R64_REQUESTER_TOKEN = [string]$RequesterLogin.accessToken
    $env:R64_REQUESTER_REFRESH = [string]$RequesterLogin.refreshToken
    $env:R64_REQUESTER_ACCOUNT_ID = [string]$RequesterLogin.profile.accountId
    $env:R64_APPROVER_TOKEN = [string]$ApproverLogin.accessToken
    $env:R64_APPROVER_REFRESH = [string]$ApproverLogin.refreshToken
    $env:R64_APPROVER_ACCOUNT_ID = [string]$ApproverLogin.profile.accountId
    $nodeOutput = & 'D:\dev\nodejs24\node.exe' $NodeScript | Out-String
    if ($LASTEXITCODE -ne 0) {
        throw "R64 browser audit phase $Phase failed with exit code $LASTEXITCODE. Output: $nodeOutput"
    }
...
$RequesterApproveDenied = Invoke-ExpectedForbidden -Method 'Post' -Path "/api/v1/systems/$($Setup.targetSystemId)/approval-tasks/$PendingTaskId/approve" -Headers $RequesterHeaders -Body @{
    idempotencyKey = "requester-denied-r64-$($Setup.suffix)"
...
try {
    $DuplicateTodoAction = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$($Setup.targetSystemId)/todos/$($Todo.todoId)/actions/approve" -Headers $ApproverHeaders -Body @{
        idempotencyKey = "todo-r64-duplicate-after-browser-$($Setup.suffix)"
...
$Result = [ordered]@{
    status = 'PASS'
    task = 'REC-P0-064'
    productStatus = 'R64_ACCEPTED_AS_DEPLOYED_ENGINEERING_EVIDENCE_ONLY'
    generatedAt = (Get-Date).ToString('o')
    baseUrl = $BaseUrl
    ...
    trialCredentials = [ordered]@{
        ...
        requesterLoginName = [string]$Setup.requesterAccount
        approverLoginName = [string]$Setup.approverAccount
        password = [string]$Setup.password
    }
    ...
    browserAuditPath = 'docs/evidence/recovery/screenshots/r64-workflow-todo-message-first-use/workflow-todo-message-browser-audit.json'
    evidenceDir = 'docs/evidence/recovery/screenshots/r64-workflow-todo-message-first-use'
    cleanup = $script:CleanupResult
    accepted = $true
}
Assert-True -Condition ($Result.browserOverflowCount -eq 0 -and $Result.browserBlockerCount -eq 0) -Message 'R64 browser audit reported blockers or horizontal overflow.'
$Result | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $script:ResultFile -Encoding UTF8

$Summary = @"
# REC-P0-064 / R64 Workflow Todo Message First-Use And Closure
...
- Terminal state: detail=$($Result.detailTerminalStatus), sidebar=$($Result.approvalSidebarStatus), duplicateTodoAction=$($Result.duplicateTodoActionStatus)
