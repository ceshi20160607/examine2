    ...
    foreach ($systemId in @($script:SystemId, $script:NormalOwnedSystemId)) {
        ...
        try {
            $deleted = Invoke-Api -Method 'Delete' -Path "/api/v1/platform/systems/$systemId" -Headers $script:AdminHeaders -Body @{
                reason = 'recovery-r74 openapi ai external service residual cleanup'
                impactScope = 'created_by_current_script'
                idempotencyKey = "cleanup-r74-$script:Suffix-$systemId"
            }
...
$EvidenceDir = Join-Path $RepoRoot 'docs\evidence\recovery\screenshots\r74-openapi-ai-external-service-error-state-residual'
$ResultFile = Join-Path $RepoRoot 'docs\evidence\recovery\r74-openapi-ai-external-service-error-state-residual-result.json'
$SummaryFile = Join-Path $RepoRoot 'docs\evidence\recovery\r74-openapi-ai-external-service-error-state-residual-2026-07-02.md'
...
$System = Invoke-Api -Method 'Post' -Path '/api/v1/platform/systems' -Headers $script:AdminHeaders -Body @{
    systemName = "R74 Integration $script:Suffix"
    systemCode = "r74_integration_$script:Suffix"
    tenantMode = 1
...
$Module = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/modules" -Headers $script:AdminHeaders -Body @{
    ...
    moduleCode = "r74_contract_$script:Suffix"
    name = "R74 Contract Ledger $script:Suffix"
    status = 1
    description = 'Recovery R74 OpenAPI AI residual module'
}
...
$null = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/modules/$ModuleId/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-r74 module publish'
    idempotencyKey = "module-publish-r74-$script:Suffix"
}
$null = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/module-groups/$($Group.groupId)/publish" -Headers $script:AdminHeaders -Body @{
    reason = 'recovery-r74 module group publish'
    idempotencyKey = "module-group-publish-r74-$script:Suffix"
}

$SecretMaterial = "material-r74-$script:Suffix"
$OpenApiApp = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/openapi/apps" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "openapi-create-r74-$script:Suffix" }) -Body @{
    externalAppCode = "r74_app_$script:Suffix"
    appName = "R74 OpenAPI App $script:Suffix"
    status = 1
    scopes = @('record.data.read', 'record.data.write')
    callbackUrl = "https://callback.example/r74/$script:Suffix"
    ...
    idempotencyKey = "openapi-create-r74-$script:Suffix"
}
...
$OldSecretRefId = [string]$OpenApiApp.openApiSecretRef.secretRefId
$RotateMaterial = "material-rotated-r74-$script:Suffix"
$Rotate = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/openapi/apps/$($OpenApiApp.externalAppId)/rotate-secret" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "openapi-rotate-r74-$script:Suffix" }) -Body @{
    ...
    idempotencyKey = "openapi-rotate-r74-$script:Suffix"
}
...
Assert-True -Condition ($SecretRefId -ne $OldSecretRefId) -Message 'OpenAPI secret rotation did not switch to a new SecretRef.'
$OpenApiHeaders = @{
    'X-OpenAPI-App-Code' = [string]$OpenApiApp.externalAppCode
    'X-OpenAPI-Secret-Ref' = $SecretRefId
    'Idempotency-Key' = "openapi-record-create-r74-$script:Suffix"
}
$ExternalCreate = Invoke-Api -Method 'Post' -Path "/openapi/v1/systems/$script:SystemId/modules/$ModuleId/records" -Headers $OpenApiHeaders -Body @{
    fieldValues = @{ contractName = "OpenAPI Contract $script:Suffix"; contractAmount = 4900; sourceTag = 'openapi-r74' }
    sourceType = 'OPENAPI_R74'
}
...
$OldSecretDenied = Invoke-ExpectedHttpStatus -Method 'Post' -Path "/openapi/v1/systems/$script:SystemId/modules/$ModuleId/records/search" -ExpectedStatus @(401, 403) -Headers @{
    'X-OpenAPI-App-Code' = [string]$OpenApiApp.externalAppCode
    'X-OpenAPI-Secret-Ref' = $OldSecretRefId
} -Body @{ pageNo = 1; pageSize = 1; keyword = $script:Suffix; fields = @('contractName') }

$ReadOnlyApp = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/openapi/apps" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "openapi-readonly-r74-$script:Suffix" }) -Body @{
    externalAppCode = "r74_readonly_$script:Suffix"
    appName = "R74 Readonly OpenAPI App $script:Suffix"
    status = 1
    scopes = @('record.data.read')
    secretMaterialRef = "readonly-material-r74-$script:Suffix"
    requestedSecretVersion = "v1-readonly-$script:Suffix"
    idempotencyKey = "openapi-readonly-r74-$script:Suffix"
}
...
$WrongSecretDenied = Invoke-ExpectedHttpStatus -Method 'Post' -Path "/openapi/v1/systems/$script:SystemId/modules/$ModuleId/records/search" -ExpectedStatus @(401, 403) -Headers @{
    'X-OpenAPI-App-Code' = [string]$OpenApiApp.externalAppCode
    'X-OpenAPI-Secret-Ref' = "wrong-secret-r74-$script:Suffix"
} -Body @{ pageNo = 1; pageSize = 1; keyword = $script:Suffix; fields = @('contractName') }
...
$ModelAuth = Invoke-Api -Method 'Post' -Path '/api/v1/platform/agent/model-authorizations' -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-model-auth-$script:Suffix" }) -Body @{
    authorizationCode = "r74_model_$script:Suffix"
    modelProvider = 'LOCAL'
    modelName = "R74 Local Model $script:Suffix"
    modelCredentialRef = @{ secretRefId = "sec_model_r74_$script:Suffix"; refType = 'MODEL'; version = "v1-$script:Suffix"; displayName = 'R74 model credential' }
    ...
    status = 1
    idempotencyKey = "r74-model-auth-$script:Suffix"
}
...
$PlatformSession = Invoke-Api -Method 'Post' -Path '/api/v1/platform/agent/sessions' -Headers $script:AdminHeaders -Body @{
    ...
    promptVersion = "r74_platform_prompt_$script:Suffix"
    openingQuestion = 'Check platform scope and do not touch system business data.'
    metadata = @{ source = 'r74' }
}
$PlatformDenied = Invoke-Api -Method 'Post' -Path '/api/v1/platform/agent/platform-agent-confirm' -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-platform-deny-$script:Suffix" }) -Body @{
    ...
    targetScope = 'system'
    humanConfirmed = $true
    idempotencyKey = "r74-platform-deny-$script:Suffix"
}
...
$PolicyCode = "r74_policy_$script:Suffix"
$AgentPolicy = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/policies" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-policy-$script:Suffix" }) -Body @{
    ...
    scope = @{
        ...
        policyVersion = "r74_policy_v_$script:Suffix"
    }
    status = 1
    idempotencyKey = "r74-policy-$script:Suffix"
}
...
Assert-True -Condition ($PolicyCheck.passed -eq $true) -Message 'R74 Agent policy publish-check did not pass.'
$AdminSession = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/sessions" -Headers $script:AdminHeaders -Body @{
    authorizationCode = [string]$ModelAuth.authorizationCode
    policyCode = $PolicyCode
    promptVersion = "r74_assistant_prompt_$script:Suffix"
...
$AdminMessage = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/sessions/$($AdminSession.sessionId)/messages" -Headers $script:AdminHeaders -Body @{
    ...
    idempotencyKey = "r74-admin-message-$script:Suffix"
}
...
$WritePreview = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/system-agent-write-confirmations" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-write-preview-$script:Suffix" }) -Body @{
    sessionId = [string]$AdminSession.sessionId
    sourceConversation = 'R74 assistant write preview'
    moduleId = $ModuleId
    recordId = $OpenApiRecordId
    fieldDiffs = @(
        @{ fieldCode = 'sourceTag'; fieldName = 'Source Tag'; beforeValue = 'openapi-r74'; afterValue = 'assistant-reviewed'; writable = $true; disabledReason = $null },
    ...
    idempotencyKey = "r74-write-preview-$script:Suffix"
}
...
$ConfirmedWrite = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/system-agent-write-confirmations/$($WritePreview.confirmation.confirmationId)/confirm" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-write-confirm-$script:Suffix" }) -Body @{ reason = 'R74 confirms scoped assistant write preview'; idempotencyKey = "r74-write-confirm-$script:Suffix" }
$DuplicateConfirmDenied = Invoke-ExpectedHttpStatus -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/system-agent-write-confirmations/$($WritePreview.confirmation.confirmationId)/confirm" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-write-confirm-duplicate-$script:Suffix" }) -ExpectedStatus @(400) -Body @{ reason = 'R74 duplicate confirm should be terminal'; idempotencyKey = "r74-write-confirm-duplicate-$script:Suffix" }
$RejectPreview = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/system-agent-write-confirmations" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-write-reject-preview-$script:Suffix" }) -Body @{
    sessionId = [string]$AdminSession.sessionId
    sourceConversation = 'R74 rejected assistant write preview'
    moduleId = $ModuleId
    recordId = $OpenApiRecordId
    ...
    humanConfirmed = $false
    idempotencyKey = "r74-write-reject-preview-$script:Suffix"
}
$RejectedWrite = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/system-agent-write-confirmations/$($RejectPreview.confirmation.confirmationId)/reject" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-write-reject-$script:Suffix" }) -Body @{ reason = 'R74 rejects unsafe assistant write preview'; idempotencyKey = "r74-write-reject-$script:Suffix" }
$DraftPreview = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/work/agent/work-agent-draft-confirm" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-work-draft-preview-$script:Suffix" }) -Body @{
    sessionId = [string]$AdminSession.sessionId
    sourceConversation = 'Draft work report from R74 assistant'
    draftType = 'DAILY_REPORT_DRAFT'
    draftPayload = @{ title = "R74 Daily Report $script:Suffix"; summary = 'Generated from authorized assistant and OpenAPI context.' }
    ...
    humanConfirmed = $false
    idempotencyKey = "r74-work-draft-preview-$script:Suffix"
}
...
$DraftConfirmed = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/work/agent/work-agent-draft-confirm" -Headers ($script:AdminHeaders + @{ 'Idempotency-Key' = "r74-work-draft-confirm-$script:Suffix" }) -Body @{
    sessionId = [string]$AdminSession.sessionId
    sourceConversation = 'Confirm R74 work report draft'
    draftType = 'DAILY_REPORT_DRAFT'
    draftPayload = $DraftPreview.draftPayload
    sourceSnapshot = $DraftPreview.sourceSnapshot
    humanConfirmed = $true
    idempotencyKey = "r74-work-draft-confirm-$script:Suffix"
}
...
$NormalLoginName = "r74_member_$script:Suffix"
$NormalRegister = Invoke-Api -Method 'Post' -Path '/api/v1/auth/register-with-system' -Body @{
    ...
    email = "r74_member_$script:Suffix@example.com"
    password = $Password
    systemName = "R74 Owned $script:Suffix"
    systemCode = "r74_owned_$script:Suffix"
    tenantMode = 1
    templateCode = 'blank'
}
...
$NormalMember = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/members" -Headers $script:AdminHeaders -Body @{
    deptId = $null
    memberName = "R74 Normal Member $script:Suffix"
    employeeNo = "R74NM$((Get-Date).ToString('HHmmssfff'))"
    mobile = "18$((Get-Date).ToString('HHmmssfff'))"
    email = "r74_member_runtime_$script:Suffix@example.com"
    status = 1
...
$null = Invoke-Api -Method 'Post' -Path '/api/v1/platform/system-switch' -Headers $NormalHeaders -Body @{ systemId = $script:SystemId; tenantId = $TenantId; reason = 'recovery-r74 normal switch' }
$NormalSession = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/sessions" -Headers $NormalHeaders -Body @{
    authorizationCode = [string]$ModelAuth.authorizationCode
    policyCode = $PolicyCode
    promptVersion = "r74_normal_prompt_$script:Suffix"
    openingQuestion = 'Normal member uses assistant in permitted context.'
    metadata = @{ source = 'r74-normal' }
}
$NormalMessage = Invoke-Api -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/sessions/$($NormalSession.sessionId)/messages" -Headers $NormalHeaders -Body @{
    ...
    idempotencyKey = "r74-normal-message-$script:Suffix"
}
...
$NormalOpenApiDenied = Invoke-ExpectedHttpStatus -Method 'Post' -Path "/api/v1/systems/$script:SystemId/openapi/apps" -Headers $NormalHeaders -ExpectedStatus @(403) -Body @{ externalAppCode = "r74_forbidden_app_$script:Suffix"; appName = 'Forbidden App'; status = 1; scopes = @('record.data.read'); idempotencyKey = "r74-normal-openapi-denied-$script:Suffix" }
$NormalPolicyDenied = Invoke-ExpectedHttpStatus -Method 'Post' -Path "/api/v1/systems/$script:SystemId/agent/policies" -Headers $NormalHeaders -ExpectedStatus @(403) -Body @{ policyCode = "r74_forbidden_policy_$script:Suffix"; scope = @{ moduleScope = @('*'); fieldScope = @{ '*' = @('*') }; actionScope = @('admin'); dataScopeExpression = 'ALL'; outboundLimit = @{ maxRows = 1; allowFileExport = $false; allowThirdPartyWebhook = $false }; desensitizePolicy = @{ mode = 'NONE'; maskedFields = @(); preview = @{} }; policyVersion = "forbidden_$script:Suffix" }; status = 1; idempotencyKey = "r74-normal-policy-denied-$script:Suffix" }
...
function Start-BrowserAudit {
    ...
    $script:ChromeProfileDir = Join-Path $env:TEMP "unexamine-r74-chrome-$script:Suffix"
...
$nodeScript = Join-Path $env:TEMP "unexamine-r74-openapi-assistant-$script:Suffix.js"
$nodeCode = @'
const fs = require('fs');
const path = require('path');
const baseUrl = process.env.R74_BASE_URL;
const port = process.env.R74_CDP_PORT;
const outDir = process.env.R74_EVIDENCE_DIR;
const systemId = process.env.R74_SYSTEM_ID;
const suffix = process.env.R74_SUFFIX;
const roles = {
  admin: { accountId: process.env.R74_ADMIN_ACCOUNT_ID, accessToken: process.env.R74_ADMIN_TOKEN, refreshToken: process.env.R74_ADMIN_REFRESH || '' },
  normal: { accountId: process.env.R74_NORMAL_ACCOUNT_ID, accessToken: process.env.R74_NORMAL_TOKEN, refreshToken: process.env.R74_NORMAL_REFRESH || '' },
};
...
async function capture(client, route, viewport) {
  await navigate(client, `${baseUrl}/?r74=${Date.now()}#${route.path}`);
  ...
  const raw = await evaluate(client, `(() => {
    ...
    return JSON.stringify({
      ...
      assistantMojibake: ${JSON.stringify(route.action === 'openAssistant')} ? /[鍔绯闇姝璋鐢宸瀹鑽纭瑙]/.test(document.querySelector('.assistant-drawer')?.innerText || '') : false,
  ...
  if (result.assistantMojibake) result.blockers.push('assistant mojibake visible');
...
async function run() {
  ...
  fs.writeFileSync(path.join(outDir, 'openapi-ai-external-service-error-browser-audit.json'), JSON.stringify(output, null, 2));
...
$env:R74_BASE_URL = $BaseUrl
$env:R74_CDP_PORT = [string]$debugPort
$env:R74_EVIDENCE_DIR = $EvidenceDir
$env:R74_SYSTEM_ID = $script:SystemId
$env:R74_SUFFIX = $script:Suffix
$env:R74_ADMIN_TOKEN = [string]$AdminLogin.accessToken
$env:R74_ADMIN_REFRESH = [string]$AdminLogin.refreshToken
$env:R74_ADMIN_ACCOUNT_ID = [string]$AdminLogin.profile.accountId
$env:R74_NORMAL_TOKEN = [string]$NormalLogin.accessToken
$env:R74_NORMAL_REFRESH = [string]$NormalLogin.refreshToken
$env:R74_NORMAL_ACCOUNT_ID = [string]$NormalLogin.profile.accountId
$nodeOutput = & 'D:\dev\nodejs24\node.exe' $nodeScript | Out-String
if ($LASTEXITCODE -ne 0) {
    throw "R74 browser audit failed with exit code $LASTEXITCODE. Output: $nodeOutput"
}
$BrowserAuditPath = Join-Path $EvidenceDir 'openapi-ai-external-service-error-browser-audit.json'
...
$Result = [ordered]@{
    status = 'PASS'
    task = 'REC-P0-074'
    productStatus = 'R74_ACCEPTED_AS_DEPLOYED_ENGINEERING_EVIDENCE_ONLY'
    ...
    duplicateConfirmDeniedStatus = [int]$DuplicateConfirmDenied.status
...
Assert-True -Condition ($Result.browserBlockerCount -eq 0 -and $Result.browserOverflowCount -eq 0) -Message 'R74 browser audit reported blockers or horizontal overflow.'
...
$summaryLines = @(
    ...
    "- Assistant confirmations: preview=$($Result.writePreviewStatus), confirm=$($Result.writeConfirmedStatus), duplicateConfirmDenied=$($Result.duplicateConfirmDeniedStatus), reject=$($Result.writeRejectedStatus), draftPreview=$($Result.draftPreviewStatus), draftConfirm=$($Result.draftConfirmedStatus)",
    ...
    "- Rotation: oldSecret=$($Result.openApiOldSecretRefId), activeSecret=$($Result.openApiSecretRefId), oldSecretDenied=$($Result.oldSecretDeniedStatus)",
