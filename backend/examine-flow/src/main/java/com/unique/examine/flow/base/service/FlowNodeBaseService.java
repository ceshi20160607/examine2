// <auto-generated>
// Generated from sql/init.sql by examine-generator. Do not hand edit.
// </auto-generated>
package com.unique.examine.flow.base.service;

import com.baomidou.mybatisplus.extension.service.IService;
import com.unique.examine.flow.base.entity.FlowNode;
import java.util.Collection;
import java.util.List;
import java.util.Optional;

/**
 * Generated base service contract for FlowNode.
 */
public interface FlowNodeBaseService extends IService<FlowNode> {

    Optional<FlowNode> findById(Long id);

    List<FlowNode> findAll();

    FlowNode saveEntity(FlowNode entity);

    boolean deleteByIds(Collection<Long> ids);
}
