// <auto-generated>
// Generated from sql/init.sql by examine-generator. Do not hand edit.
// </auto-generated>
package com.unique.examine.flow.base.service;

import com.baomidou.mybatisplus.extension.service.IService;
import com.unique.examine.flow.base.entity.FlowDefinition;
import java.util.Collection;
import java.util.List;
import java.util.Optional;

/**
 * Generated base service contract for FlowDefinition.
 */
public interface FlowDefinitionBaseService extends IService<FlowDefinition> {

    Optional<FlowDefinition> findById(Long id);

    List<FlowDefinition> findAll();

    FlowDefinition saveEntity(FlowDefinition entity);

    boolean deleteByIds(Collection<Long> ids);
}
