...
| TEST-010 | P12 UI 可用性 E2E | test | test | FE-024 | `docs/test_runs/p12-ui-usable-e2e.md` | FE-024 | 否 | 浏览器验证 UI 设计主链路、状态反馈和可用性 | pass/fail 需说明截图或 DOM 断言 | pending |
...
| VAL-008 | P12 clean build | validator | validator | TEST-010 | `docs/build/p12-clean-build.md` | TEST-010 | 否 | 前端 clean build、后端 package 通过；未完成阶段禁止生成部署包 | clean build/package pass | pending |
...
| PKG-001 | 最终部署包 | pm/validator | deploy | REV-008 | `dist/unexamine-full-deploy-20260611-100441-fixed.zip` / `dist/unexamine-full-deploy-20260611-100441-fixed.tar.gz` | REV-008 | 否 | 仅在 `docs/review.json.status=pass` 且 `fullProjectDeployable=true` 后打包 | 包清单核验 pass | done |
...
| P14-TEST-001 | 测试 | test | partial | 剧本 A API 链路通过：建系统、建应用、建模块、字段、页面、发布、运行台记录均通过；浏览器登录、系统总览和运行台关键页面烟测通过，记录见 `docs/test_runs/p14-integrated-api-e2e-20260611.md`、`docs/test_runs/p14-frontend-smoke-20260611.md`。 |
| P14-TEST-002 | 测试 | test | partial | 剧本 B API 链路通过：OpenAPI scope 目录和对外应用创建通过；浏览器平台对外应用中心和系统对外授权表单烟测通过。 |
| P14-TEST-003 | 测试 | test | partial | 剧本 C API 链路通过：平台/系统/OpenAPI 日志接口均可查询；日志浏览器检索流仍待补充。 |
| P14-VAL-001 | 验证 | validator | partial | 前端 `npm.cmd run build` 通过；后端 clean package、最终 build report 和 reviewer 仍待执行。 |
