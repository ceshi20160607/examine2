...
| 清理动作 | 先删除 `node_modules`、`dist`、`tsconfig.tsbuildinfo`，再使用 lockfile clean install |
...
`npm.cmd ci` 成功按 lockfile 安装 TypeScript，`npm.cmd run build` 通过。
...
VAL-002 返工复验通过。前端已具备可复跑的 TypeScript clean build 入口，当前 clean 构建命令为 `npm.cmd ci; npm.cmd run build`。
