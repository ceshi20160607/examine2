...
> 使用 brief：`docs/design/prototype-brief.md`，版本 `1.7.8-work-field-dict-kanban-source`
> 结论：本轮结构修正已落地到原型和 brief；仍需用户确认，未签字前不进入 API 和开发。
...
- `plat_admin` 登录后进入 `platformWorkbench`，打开个人信息后可进入 `platformAdmin`；配置管理中 `AI Agent / 模型授权配置` 抽屉可打开，包含本地模型、外部模型、授权购买、系统继承和审计规则。
...
- 216 个按钮死按钮 0。
- 32 个抽屉入口全部有定义。
- 角色入口包含 `che`、`admin`、`platform_member`、`plat_admin`。
...
- 系统工作仪表盘月历格为 35 个；任务页包含项目任务列表、普通任务和“状态列来自当前项目配置”；日报页包含我的最近日报、手动填写日报和今日自动统计。
