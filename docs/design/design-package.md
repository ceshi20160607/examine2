...
| 业务列表 | 高级搜索、场景、表头、序号勾选、批量、导入、详情抽屉（RES-005） |

---

## 2. MVP 页面总表（共 24 页 + 3 弹窗）
...
| P-SYS-ADMIN-MEMBERS | 成员与角色（含三维权限） | `system/admin-members.html` | P0 |
...
| ID | 场景 | 出现在 |
|----|------|--------|
| M-ADD-MEMBER | 搜索平台用户添加成员 | admin-members |
...
| 3-10 | plat_admin | admin-home → basic → dict → datasource → module-group → module → modeling → dashboard-cfg → members |
...
设计包已齐，请打开 Open Design，粘贴 `prototype-brief.md` 全文作为 brief，输出到 `docs/design/prototypes/v3/`。  
