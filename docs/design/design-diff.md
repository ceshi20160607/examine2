...
## 12. 配置态细粒度规格（config-spec v1.0 · 2026-06-17）

用户反馈：仅有「列表 + 编辑按钮」不够，需明确**按钮清单**、**字段抽屉逐项**（标题、类型下拉、占宽 50%/100%、显隐、唯一、字典/关联映射到模块.字段等）。

| 产物 | 状态 |
|------|------|
| `docs/design/config-spec.md` | ✅ 新增 §1–§12：运行态按钮、建模 6 Tab、字段抽屉分区、映射表 |
| `ui-spec.md` §5.8.3/5.8.4 | ✅ 引用 config-spec |
| `prototype-brief.md` §6.2 | ✅ 字段抽屉 + Tab/.scene/action 最低内容 |
| `docs/design/prototypes/system/admin-modeling.html` | ✅ 演示「车辆状态」抽屉 + 场景/动作 Tab |

**待 redesign 全量验收：** 角色 4 Tab、流程三栏、对外应用映射、仪表盘设计器仍须按 config-spec §9 补全编辑面板。
