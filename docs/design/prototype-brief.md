...
7. **平台 Flow/应用（RES-005）：** 在**平台后台**单独配置；平台 Flow 可通过平台应用对外开放。侧栏 Flow/应用是用户入口，不是配置页。
8. **业务列表（RES-005）：** 高级搜索抽屉、列表场景、序号+勾选列、批量（编辑/转移/删除/导出）、导入、列设置、行点击右侧详情抽屉。
...
| 文件 | 说明 |
|------|------|
| `plat-admin-flow.html` | 平台 Flow 列表+画布入口 |
| `plat-admin-apps.html` | 平台对外应用 + 绑定 Flow + 参数映射 |
| 加厚 `runtime-list.html` | 高级搜索、场景、序号勾选、批量条、导入、列设置、详情抽屉 |
| 加厚 `admin-modeling.html` | 列表场景 + 表头默认列配置区块 |
...
| `runtime-list.html` | **完整运行态壳** + 车辆列表 + **RES-005 高级表格**（见 ui-spec §5.9） |
