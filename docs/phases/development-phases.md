...
| P7-frontend-ui-deploy | 前端真实 UI 与部署包期 | FE-013、TEST-006：前端工程入口、真实页面组件、路由挂载、API 调用闭环、浏览器 smoke/E2E、`dist/` 产物、前后端组合 E2E | 存在 `index.html`、`src/main.*`、真实页面和可部署 `dist/`；后端 jar + 前端 dist 完成组合 E2E。 | in_progress |

## 当前期

当前正在推进：`P7-frontend-ui-deploy`。
...
- TEST-006 待启动，用于验证后端 jar + 前端 dist 的组合 E2E。
