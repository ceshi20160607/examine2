...
- 平台用户只能在平台内创建、导入或受控开通；不能提供普通账号自助开通入口。
- 面向新用户的入口是“创建系统”，创建系统的人自动成为该自建系统管理员，但不获得平台级管理权限。
- 系统内创建应用配置，是为了让该系统使用平台级应用能力和 Flow 能力，实现对外或对内调用，以及模块事件触发流程。
- 平台当前固定基础能力包含组织架构、Flow、应用、日志、审计、待办、消息和统一入口。
- 每个自建系统也具备系统级日志、审计、待办、消息，以及组织、权限、字典、模块、模块分组、系统内应用配置、流程事件绑定和运行台。

新增产物：

- 页面原型：`docs/ui/prototypes/p18-architecture-aligned-ui-prototype.html`
- 复核说明：`docs/ui/prototypes/p18-architecture-aligned-ui-prototype-review.md`
- 本地预览：`http://127.0.0.1:4177/p18-architecture-aligned-ui-prototype.html`

浏览器自检：

- 登录平台 -> 统一入口 -> 平台级 Flow 与应用 -> 系统内应用配置与流程事件 -> 业务运行台：通过。
- 页面中已清理“普通平台注册/注册平台账号”等易误导可见文案，保留“创建系统”入口。

当前裁决：

- `p18-architecture-aligned-ui-prototype.html` 状态为 `draft-for-user-review`。
- 用户复核通过前，frontend 不得基于该原型进入实现。
- `frontendUsable=false`、`fullProjectDeployable=false`、最终打包继续阻塞。

## 2026-06-13 P17 架构再次修正，补充 UI 风格选择板

用户复核再次成立：`p17-low-cost-ui-prototype.html` 对平台和自建系统的理解仍不准确，视觉也只是粗糙结构稿，不是可接受的 UI 设计。

本轮修正后的架构理解：

- 平台类似统一登录/SSO 入口，但不只是 SSO，还承载平台级组织架构、平台权限、平台级应用、平台级流程、平台审计和运维。
- 自建系统是用户创建的业务后台管理空间，用于配置该系统的组织架构、用户部门、角色权限、字典、模块、模块分组、页面、菜单、系统内应用配置、流程事件绑定和运行台。
- 平台级应用是平台能力；系统内应用配置是某个自建系统对平台级应用能力的启用、功能范围、事件绑定、授权范围和入口配置；模块和模块分组不是应用。
- 流程既是平台级能力，也需要能在系统内按模块事件绑定，例如提交、修改、状态变更时走审批。

新增产物：

- 架构修正：`docs/product/p17-platform-system-architecture-correction.md`
- 风格选择板：`docs/ui/prototypes/p17-style-directions.html`
- 本地预览：`http://127.0.0.1:4177/p17-style-directions.html`

PM/Reviewer 当前裁决：

- `p17-low-cost-ui-prototype.html` 仅保留为历史结构草稿，不能作为后续实现依据。
- 下一步应先确认架构和 UI 风格方向，再重做页面级原型。
- 当前推荐风格方向：A 稳重企业控制台为主，吸收 B 的普通用户亲和度。
- `frontendUsable=false`、`fullProjectDeployable=false`、最终打包继续阻塞。

## 2026-06-13 P17 低成本 UI 原型已产出，等待用户复核

本轮按用户要求先产出低成本 UI 原型，只用于确认产品模型、主流程和后台交互方向，不作为最终视觉稿或前端实现冻结稿。

新增产物：

- 原型：`docs/ui/prototypes/p17-low-cost-ui-prototype.html`
- 复核说明：`docs/ui/prototypes/p17-low-cost-ui-prototype-review.md`
- 本地预览：`http://127.0.0.1:4177/`

本轮已重点校正：

- 应用与系统、流程同级，只作为对外/跨系统调用管理主体。
- 系统内对象使用模块、字段、页面、菜单、运行台，不再称为应用。
- 登录分流覆盖平台管理态、系统管理态、普通业务运行台和多系统选择页。
- 左侧导航支持展开/折叠。
- 新建、编辑、详情统一右侧抽屉体系。
- 列表包含高级筛选、表头配置、保存视图、分页和字段权限影响。
- 权限覆盖页面、动作、字段、数据范围和应用调用范围。

浏览器自检：

- 多系统用户登录 -> 系统选择页 -> 进入“车”系统 -> 业务运行台 -> 高级筛选 -> 详情抽屉：通过。

当前裁决：

- `p17-low-cost-ui-prototype.html` 状态为 `draft-for-user-review`。
...
- 过去 PM 只在多个文档中零散表达了目标，仍然会把功能点、构建和局部 E2E 误判为完整系统可用。
- 现在已新增 `docs/product/final-user-goal.md`，作为后续 PM、UI/UX、frontend、test、reviewer 的最高优先级产品目标基线。
- `AGENTS.md` 已补充规则：PM 每次裁决是否完成、是否打包、是否可上线前，必须逐条对照最终目标基线。
- 当前状态仍为 `frontendUsable=false`，P15 包仍为技术试用包；P16 必须先按该基线重做页面级 UI/UX，再允许前端重构和最终包验收。

## 2026-06-12 UI 体验结论撤回

用户复核结论成立：当前 P15 前端不是主流可用系统 UI，也不是普通人能自然理解的完整流程。它仍然更接近工程化配置台和功能拼接页。

PM/Reviewer 当前裁决：
- `docs/review.json.status=fail`
- `frontendDeployable=true` 只代表前端能构建和静态部署，不代表用户可正常使用
- `frontendUsable=false`
- P15 包降级为技术试用包：`trial_package_only_ui_rework_required`
- 不再把当前页面称为“主流样式和流程”
- 后续必须先做页面级 UI/UX 重整，再允许重新打最终交付包

本次复核确认的主要问题：
- 前端主体仍集中在超大 `frontend/src/App.ts`，页面组件化和设计系统不足
- 多数页面仍是表单、卡片、表格直接堆叠，像接口/配置调试台
- 新建入口仍偏向页面内嵌表单，不是主流后台常见的主按钮 + 抽屉/独立新建页/分步向导
- 业务流程没有被页面自然承载，仍需要用户理解“系统、成员、模块、字段、发布、运行台”等后台概念
- 普通用户、系统管理员、平台管理员的工作空间视觉和操作差异还不够清晰
...
更新时间：2026-06-12 13:17

### 2026-06-12 P15 第一轮实现修正

本轮已进入 P15-2 实现修复，但仍不允许打最终包。

- 后端：`/api/v1/platform/my-systems` 已补齐“创建者系统 + 成员加入系统”两类系统，避免普通成员只看到平台空壳。
- 后端：系统成员开通支持在系统成员页直接创建平台账号并加入当前系统，前端无需先绕到平台账号页。
- 后端：新增登录账号写入请求上下文的 Web 过滤器，系统权限快照校验 `accountId + systemId + memberId`，降低前端伪造成员上下文风险。
- 前端：登录后根据“我的系统”和平台权限决定落点；普通成员单系统场景默认进入业务运行台，管理员进入系统总览。
- 前端：系统内导航按权限区分管理员入口和普通业务入口，普通成员不再默认暴露建模、字段设计、发布和权限配置菜单。
- 前端：补充全局退出入口、成员开通卡片、我的系统空态、平台/运行记录分页条。
- 验证：后端 `mvn -pl examine-web -am -DskipTests test` 通过；前端 `npm.cmd run build` 通过并生成 `frontend/dist/`。
- 仍未完成：尚未记录真实浏览器 E2E 剧本“platform_admin 创建车系统 -> 开通 che -> che 登录进入运行台 -> 新增/查询/编辑车辆数据 -> 验证看不到管理入口”。因此 `fullProjectDeployable=false`、`package_gate=blocked` 保持不变。
...
## 2026-06-11 P14 普通用户与 OpenAPI 补证完成

Reviewer P0 问题成立，本轮完成后端补缺和真实接口复验，记录见 `docs/test_runs/p14-ordinary-user-openapi-e2e-20260611.md`：

- 普通业务用户 `p14_member_212202` 通过可配置角色获得运行台菜单、记录查看/新建/编辑/提交/历史权限，字段权限包含 `customer_name`、`phone`、`level`，数据范围为 `SELF`。
- 普通业务用户可在模块 `customer_file_77525602` 新增业务记录 `2065062007648825345`，访问成员管理返回 `403 COMMON_FORBIDDEN`。
- OpenAPI client `2065062008223444993` 使用 AK/SK 成功调用 `POST /openapi/v1/records/query`，`requestId=p14-openapi-212202`，返回记录数 `5`。
- OpenAPI 日志可按 requestId 查询，返回 `total=1`、`statusCode=200`、`signatureResult=PASS`、`scopeResult=PASS`、`rateLimitResult=PASS`。
- 后端修复包括运行菜单同步 RBAC 授权目录、角色字段权限按 `moduleId + fieldCode` 解析、动作/字段选项替换幂等、OpenAPI client 权限快照、OpenAPI 日志 total 修正。

当前结论：P14-PKG-001 仍 blocked，等待 reviewer 复审通过后才允许进入最终打包。

## 2026-06-11 P14 reviewer 复审通过

Reviewer 已复审通过，记录见 `docs/review.json` 和 `docs/issues/verification/development/p14_reviewer_verification.md`。

- `docs/review.json.status=pass`
- `frontendDeployable=true`
- `backendTrialDeployable=true`
- `fullProjectDeployable=true`
- `packageGate=allowed`

当前进入 `P14-PKG-001` 打包。

## 2026-06-11 P14-PKG-001 最终部署包完成

PM 结论：P14 accepted。产品运行模型、集成 UI/UX、前端重组、后端补缺、普通业务用户验收、平台级对外应用验收、日志追踪、clean build、review 和部署包闸门均已闭环。
...
| 后端 | `backend/unexamine.jar`、`backend/start.sh` |
| 前端 | `frontend/index.html`、`frontend/assets/index-BoxiNZTv.css`、`frontend/assets/index-C4ikCUrv.js` |
| nginx 文档 | `docs/nginx-deploy.md`，要求 `/api/` 前缀保留转发 |
| 归档权限 | `tar.gz` 内 `backend/start.sh` 为 `rwxr-xr-x`；zip 元数据为 `100755` |
| 临时目录 | 归档清单未包含 `.codex/tmp/` |

当前结论：`fullProjectDeployable=true`，P14 最终部署包 ready。
