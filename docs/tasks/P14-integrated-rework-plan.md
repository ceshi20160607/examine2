...
| P14-BE-001 | backend | 只补 P14 UI/测试发现的真实后端缺口。 | 后端源码与测试证据 | 已补运行菜单同步 RBAC 授权目录、字段权限解析、配置替换幂等、OpenAPI client 权限快照和 OpenAPI 日志 total。 |
| P14-TEST-001 | test | 执行剧本 A 管理员到普通用户完整链路。 | `docs/test_runs/p14-integrated-api-e2e-20260611.md`、`docs/test_runs/p14-frontend-smoke-20260611.md`、`docs/test_runs/p14-ordinary-user-openapi-e2e-20260611.md` | API/浏览器链路完成建模、发布、运行台记录；普通用户可新增业务记录，访问成员管理返回 403。 |
| P14-TEST-002 | test | 执行剧本 B 平台级对外应用链路。 | `docs/test_runs/p14-integrated-api-e2e-20260611.md`、`docs/test_runs/p14-frontend-smoke-20260611.md`、`docs/test_runs/p14-ordinary-user-openapi-e2e-20260611.md` | OpenAPI client 使用 AK/SK 成功调用系统内模块数据。 |
| P14-TEST-003 | test | 执行剧本 C 审计与恢复链路。 | `docs/test_runs/p14-integrated-api-e2e-20260611.md`、`docs/test_runs/p14-frontend-smoke-20260611.md`、`docs/test_runs/p14-ordinary-user-openapi-e2e-20260611.md` | OpenAPI 日志可按 requestId 查询，返回 `statusCode=200`、`signatureResult=PASS`、`scopeResult=PASS`。 |
...
| P14-PKG-001 | validator | 在 reviewer pass 后生成部署包。 | `dist/unexamine-full-deploy-20260611-213200-p14.zip`、`dist/unexamine-full-deploy-20260611-213200-p14.tar.gz` | 包含前端 dist、后端 jar、start.sh、nginx 文档和 P14 证据；`start.sh` 权限和 `.codex/tmp/` 排除已核验。 |
...
P14 accepted。三条剧本、普通业务用户隔离、平台级对外应用链路、OpenAPI 调用日志、前端 clean build、后端 clean package、reviewer 复审和部署包清单均已通过。
