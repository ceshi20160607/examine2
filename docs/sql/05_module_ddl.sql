...
CREATE TABLE un_module_field (
  ...
  field_type     VARCHAR(32)  NOT NULL COMMENT '字段类型枚举：TEXT|TEXTAREA|NUMBER|MONEY|PERCENT|DATETIME|BOOLEAN|SELECT|MULTI_SELECT|ADDRESS|PERSON|DEPARTMENT|TITLE|SIGNATURE|SERIAL_NO|RICH_TEXT|REF_MODULE|TAG|DATE_RANGE|FILE',
  ...
  validate_type  VARCHAR(32)  NULL COMMENT '校验类型（phone|email|idCard|url|password 等，TEXT 子样式）',
  date_format    VARCHAR(32)  NULL COMMENT '日期格式（DATETIME：yyyy-MM-dd / yyyy-MM-dd HH:mm:ss 等）',
  dict_code      VARCHAR(64)  NULL COMMENT '数据字典编码（SELECT/MULTI_SELECT）',
  ref_model_id   BIGINT       NULL COMMENT '关联目标 modelId（REF_MODULE）',
  ref_display_field VARCHAR(64) NULL COMMENT '关联记录展示 fieldCode（目标 model）',
  relation_module_label VARCHAR(128) NULL COMMENT '关联模块展示名（子表/选择器标题）',
  config_json    JSON         NULL COMMENT '类型扩展配置（displayStyle/listFields/segments/地址级联等）',
