...
class DatabasePreparationTests(unittest.TestCase):
    def test_prepares_combined_and_flyway_sql_from_versioned_sources(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (project / "db/base").mkdir(parents=True)
            (project / "db/update").mkdir(parents=True)
            (project / "db/migration").mkdir(parents=True)
            (project / "db/base/V1__identity.sql").write_text("create table identity_record (id bigint);\n", encoding="utf-8")
            (project / "db/base/V2__runtime.sql").write_text("create table runtime_record (id bigint);\n", encoding="utf-8")
            ...
            self.assertEqual(2, result["baseFiles"])
            ...
            all_sql = (project / "db/all.sql").read_text(encoding="utf-8")
            self.assertLess(all_sql.index("identity_record"), all_sql.index("runtime_record"))
            self.assertLess(all_sql.index("runtime_record"), all_sql.index("audit_record"))
    ...
    def test_rejects_update_version_that_is_not_newer_than_baseline(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (project / "db/base").mkdir(parents=True)
            (project / "db/update").mkdir(parents=True)
            (project / "db/base/V2__baseline.sql").write_text("select 2;\n", encoding="utf-8")
