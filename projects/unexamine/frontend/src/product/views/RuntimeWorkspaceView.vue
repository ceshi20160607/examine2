...
import { productDateTime, userFacingWorkspaceName } from '../presentation'
import type {
  RuntimeField,
  RuntimeDictionary,
...
const transferForm = reactive({ ownerMemberId: undefined as number | undefined, departmentId: undefined as number | undefined, participantText: '', reason: '' })
...
function ownerDisplayName(record: RuntimeRecord) {
  ...
  return record.ownerMemberId === systemContext.value?.memberId ? systemContext.value?.displayName || '我' : '其他负责人'
}
...
function openTransfer(record: RuntimeRecord) {
  ...
  Object.assign(transferForm, { ownerMemberId: record.ownerMemberId, departmentId: record.departmentId, participantText: record.participantMemberIds.join(','), reason: '' })
...
function transferParticipantIds() {
  return [...new Set(transferForm.participantText.split(',').map((item) => Number(item.trim()))
...
<template>
  <div class="runtime-page" :data-refresh-key="props.refreshKey">
      ...
      <template #actions>
        <a-dropdown>
          ...
          <template #overlay><a-menu @click="runPageUtility(String($event.key))">
            ...
            <a-menu-item v-if="hasAction('CREATE') && lifecycleState === 'ACTIVE'" key="ai"><RobotOutlined />使用智能助手创建</a-menu-item>
          </a-menu></template>
        </a-dropdown>
        <a-button v-if="hasAction('CREATE') && lifecycleState === 'ACTIVE'" type="primary" @click="runAction('CREATE', openCreate)"><PlusOutlined />新建{{ activeModule?.moduleName }}</a-button></template>
    </ProductPageHeader>
    ...
    <nav v-if="groupedModules.length" class="runtime-group-tabs" aria-label="业务模块组">
      ...
      <main class="record-panel panel-card">
        <template v-if="activeCode">
          <div class="record-query-bar">
            <a-input-search v-model:value="searchText" allow-clear placeholder="搜索标题、编号和可搜索字段" style="width:280px" @search="applyListQuery"><template #prefix><SearchOutlined /></template></a-input-search>
            <a-button :type="listFilters.length ? 'primary' : 'default'" ghost @click="filterOpen = true"><FilterOutlined />高级筛选<span v-if="listFilters.length">（{{ listFilters.length }}）</span></a-button>
            <a-select :value="activeViewCode || undefined" allow-clear placeholder="选择常用视图" style="width:170px" :options="savedViews.map(view => ({ value: view.code, label: `${view.name}${view.defaultView ? ' · 默认' : ''}` }))" @change="selectSavedView" />
            <a-popover v-model:open="columnOpen" trigger="click" placement="bottomRight">
              ...
              <a-button><SettingOutlined />视图设置</a-button>
            </a-popover>
          </div>
          <div class="record-summary"><div><a-radio-group v-model:value="lifecycleState" button-style="solid" size="small" @change="changeLifecycleState">
          ...
          </a-radio-group><a-segmented v-model:value="tenantScope" size="small" :options="[{ label: '全部', value: 'ALL' }, { label: '本租户', value: 'OWN' }, { label: '共享给我', value: 'SHARED' }]" @change="page = 1; activeViewCode = ''; loadRecords()" /><span>{{ loading ? '正在按当前条件加载…' : `共 ${total} 条` }}</span><small>{{ querySummary }}</small></div><span class="runtime-version-tags"><a-tag color="blue">主版本 v{{ configuration?.versionNumber }}</a-tag><a-tag v-if="configuration?.configuration.tenantExtensionApplied" color="purple">租户扩展 v{{ configuration.configuration.tenantExtensionVersionNumber }}</a-tag></span></div>
          ...
          <a-table class="record-table" :data-source="records" :loading="loading" row-key="id" :pagination="false" :row-selection="rowSelection" :custom-row="rowHandlers" :scroll="{ y: pageSize >= 100 ? 560 : undefined }" :virtual="pageSize >= 100">
            <a-table-column v-for="column in displayedColumns" :key="column.code" :title="column.label" :width="column.width" :fixed="fixedFieldCodes.includes(column.code) ? 'left' : undefined">
              <template #default="{ record }"><template v-if="column.code === 'title'"><strong class="record-title">{{ record.title }}</strong><small>{{ record.recordNumber || '未设置业务编号' }} · {{ userFacingWorkspaceName(record.dataTenantName) }}</small></template>
          ...
          <div class="mobile-record-list" aria-label="业务记录卡片列表" :aria-busy="loading">
            <button v-for="record in records" :key="record.id" type="button" :data-status-tone="statusTone(record.status)" @click="openDetail(record)">
              <header><span><strong>{{ record.title }}</strong><small>{{ record.recordNumber || '未设置业务编号' }} · {{ userFacingWorkspaceName(record.dataTenantName) }}</small></span><ProductStatusTag :status="record.status" /></header>
              <dl>
    ...
    <a-drawer v-model:open="detailOpen" :title="detail?.title" width="520" class="detail-drawer">
      <template v-if="detail"><div class="detail-meta"><span>{{ detail.recordNumber || '未设置业务编号' }}</span><ProductStatusTag :status="detail.status" /><a-tag :color="detail.shared ? 'orange' : 'blue'">数据归属：{{ userFacingWorkspaceName(detail.dataTenantName) }}</a-tag><span v-if="detail.shared" class="detail-share-status">共享状态：<ProductStatusTag :status="detail.shareStatus" /></span></div>
        <a-tabs class="detail-tabs-runtime">
          <a-tab-pane key="basic" tab="基础信息"><a-descriptions :column="1" bordered size="small"><a-descriptions-item v-for="field in detailFields" :key="field.code" :label="field.name"><span class="detail-field-value">{{ displayValue(field, detail.fields[field.code]) }}<a-button type="link" size="small" @click="copyFieldValue(field, detail.fields[field.code])">复制</a-button></span></a-descriptions-item>
          ...
          <a-tab-pane key="relations" tab="关联数据"><a-list v-if="conversionLinks.length" size="small" bordered :data-source="conversionLinks"><template #renderItem="{ item }"><a-list-item><a-button type="link" @click="openConversionLink(item)">{{ item.direction === 'SOURCE' ? '来源' : '目标' }} · {{ item.moduleName }} · {{ item.recordTitle }}</a-button><span>{{ productDateTime(item.convertedAt) }}</span></a-list-item></template></a-list><a-empty v-else :image="Empty.PRESENTED_IMAGE_SIMPLE" description="暂无有权查看的关联数据" /></a-tab-pane>
          <a-tab-pane key="timeline" tab="变更时间线"><a-timeline v-if="timeline.length"><a-timeline-item v-for="entry in timeline" :key="entry.eventId">
              <div class="timeline-heading"><strong>{{ entry.label }}</strong><span>{{ productDateTime(entry.occurredAt) }}</span></div><p>{{ entry.actorDisplayName }}</p>
    ...
    <a-drawer :open="formOpen" :title="editing ? `编辑${activeModule?.moduleName}` : `新建${activeModule?.moduleName}`" width="560" :mask-closable="false" :keyboard="false" @close="requestCloseForm">
      ...
      <a-form layout="vertical"><div class="form-grid"><a-form-item label="标题" required><a-input id="runtime-record-title" v-model:value="form.title" /></a-form-item><a-form-item label="业务编号"><a-input v-model:value="form.recordNumber" /></a-form-item></div>
        ...
        <a-form-item v-for="field in formFields" :key="field.code" :label="field.name" :required="field.required" :data-runtime-field="field.code"
          ...
          <a-input-number v-else-if="['MEMBER', 'DEPARTMENT', 'REFERENCE'].includes(field.fieldType)" v-model:value="form.fields[field.code]" :min="1" style="width:100%" />
    ...
    <a-modal v-model:open="transferOpen" title="转交负责人" :footer="null" :mask-closable="false">
      <a-form layout="vertical"><div class="form-grid"><a-form-item label="新负责人编号" required><a-input-number v-model:value="transferForm.ownerMemberId" :min="1" style="width:100%" @change="transferPreview = undefined" /></a-form-item><a-form-item label="新部门编号"><a-input-number v-model:value="transferForm.departmentId" :min="1" style="width:100%" @change="transferPreview = undefined" /></a-form-item></div><a-form-item label="相关成员编号" extra="多个成员编号用逗号分隔"><a-input v-model:value="transferForm.participantText" @change="transferPreview = undefined" /></a-form-item><a-form-item label="转交原因" required><a-textarea v-model:value="transferForm.reason" :rows="3" :maxlength="1000" show-count @change="transferPreview = undefined" /></a-form-item></a-form>
