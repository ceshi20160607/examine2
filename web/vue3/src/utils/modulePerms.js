import { ref } from 'vue'
import { listModulePermissions } from '../api/module.js'
...
export function createModulePermState() {
  ...
  async function loadModulePerms() {
    try {
      ...
      permKeys.value = new Set(keys.map((x) => String(x || '').trim()).filter(Boolean))
      loadError.value = ''
    } catch (e) {
