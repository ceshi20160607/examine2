...
if (Test-Path $nginxConfig) {
    ...
    Add-Check 'nginx proxies /openapi to backend 9999' ($nginxContent -match 'location\s+/openapi/' -and $nginxContent -match 'proxy_pass\s+http://127\.0\.0\.1:9999;') 'nginx/unexamine.conf'
} else {
    Add-Check 'nginx proxies /api to backend 9999' $false "Missing $nginxConfig"
    Add-Check 'nginx proxies /openapi to backend 9999' $false "Missing $nginxConfig"
}
